// pages/component/signMod/signMod.js
import initCalendar, {
  setSelectedDays,
  disableDay,
  getThisMonthDays
...
Component(app.BTAB({
  ...
  methods: { 
    ...
    init() {
      console.log('initinit1')
      if(this.inited)return
      console.log('initinit2')
      this.inited = true;
      let that = this;
      initCalendar({
        multi: true,
    ...
    sign_show_toggle(e){
      let bool = !!!this.data.sign_show_detail;
      if(!bool){
...
function getOrderActInfo(info){
  ...
  for(let i = 0,len=list.length;i<len;i++){
    ...
    if(item.isReach || (i == 0)){
      ...
      info.nextGetContinueInfo.nextGetContinueDay = item.continueDay - currentContinueDay;
      if((i == len - 1) && info.nextGetContinueInfo.nextGetContinueDay == 0){
        info.curSort += 1;
      }
    }else{
      break;
