...
function loadData(type = 0, dateType) {
    if (this.hasMore[type] && !this.isLoading[type] && this.reflash[type]) {
        ...
        let str = [`cashOutList`]
        if (this.data.pageType == 'commission') {
            str = [`staffCommOrderList`]
        }else if (this.data.pageType == 'share') {
            str = [`getStaffDstbUserReport`]
        }
        ...
        selectLoadData.call(this, str, type, typeParam, endDate,dateType).then(res => {
            if (res.code == 1) {
                const data = res.data || {};
                let dataList = [];
                if (this.data.pageType == 'commission') {
...
function selectLoadData(str, type, typeParam, endDate,dateType) {
    let obj = {};
    obj = {
    ...
    if (str && this.data.pageType == 'commission') { //分销订单
    ...
    } else if (str && this.data.pageType == 'share') { //提现记录
        obj = {
            userToken:  app.LM.userToken || "",
            brandCode: app.Conf.BRAND_CODE,
