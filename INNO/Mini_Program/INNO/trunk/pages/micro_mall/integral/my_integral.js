...
var app = getApp();
Page(app.BP({
    cardCodeTask() { return this.MyViewTask("#cardCode", "cardCode", "showCardCode"); },
    ...
    showPointCaptcha(){
        this.cardCodeTask().then(cmpt=>{
            cmpt.show();
            cmpt.getCaptcha('userPoint');
        })
    }
...
function pointList(label=0) {
    ...
    let params = {
        userToken: app.LM.userToken, // a4e34c131bfbcd4f
...
function checkCaptcha(){
    return app.sysTemConfig('is_show_point_captcha').then(data=>{
        if(data&&data.Value == 1){
            this.setData({
                isShowPointCaptcha:true
            })
        }
    })
}