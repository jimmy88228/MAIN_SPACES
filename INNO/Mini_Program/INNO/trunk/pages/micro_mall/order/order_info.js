...
Page(app.BP({
  ...
  toPayReq() {
    let buyType = this.options.buyType || "order";
    let allOrderEntity = this.allOrderEntity||{};
    console.log('allOrderEntity',allOrderEntity)
    return PayH.UnifiedorderByOrderId(buyType, this.data.order_id,true,true,allOrderEntity).then(e => {
      console.log(e,'支付结果');
      this.setData({
...
function updateBtnContainer(){
  ...
  let container = [
    ...
    },
    {
      ...
      "openType":"share",
      "name":"代付",
      "status": this.data.close_pay && (order_info.orderStatus=='待付款') && (order_info.pay_code=='wxpay' || order_info.pay_code == 'wxin') && !this.data.endOrder && order_info.order_type != 11
    },
    {
      "key":'apply_invoice',
      "tap":'onTap',
