...
Page(app.BP({
  ...
  */
  ...
  funcJump:function(e){
    ...
    if (key == 'distribution') {
    ...
    }else if (key == 'thirdCs' && sys_config.third_cs_open){
      //第三方客服跳转
      ...
      console.log('优惠券',this.coupon_jump_type,this.coupon_jump_appid,this.coupon_jump_path)
  ...
  activeCustomerService(){
    ...
    this.contactStaff && this.contactStaff.initData();
  },
  contactCallBack(e){
    let detail = e.detail || {};
    this.contactGuide = this.contactGuide || this.selectComponent("#contactGuide");
...
function menuDataCallback(menuData, userData){
    ...
    if (assets.my_point && assets.my_point.is_enable == 1) {
      checkLogin.call(this,()=>{getUserPoint.call(this);})
    }
    if (assets.prepaidcard && assets.prepaidcard.is_enable == 1) {
      checkLogin.call(this,()=>{getUserPrepaid.call(this);})
    }
    if (assets.redpack && assets.redpack.is_enable == 1){
      checkLogin.call(this,()=>{getUserRedpackAmount.call(this);})
    }
    if (assets.coupon && assets.coupon.is_enable == 1){
      checkLogin.call(this,()=>{getUsefulCouponCount.call(this);})
    }
    //检测是否开启导购
...
function checkLogin(cb){
  cb && typeof(cb) == 'function' && app.LM.loginAsync(false).then(()=>{
    if(app.LM.isLogin)cb()
  });
} 