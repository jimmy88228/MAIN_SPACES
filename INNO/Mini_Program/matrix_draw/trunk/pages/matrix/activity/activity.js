...
// import iterateeTime from "../iteratee_time.js.js";
...
Component({
        ...
        prizeList: {
            ...
            observer(nV, oV) {
                ...
                if (this.data.activityType === 1) {
                    ...
                    console.log('_prizeList',_prizeList)
                    this.setData({
                        _prizeList
                    });
                } else {
    ...
    methods: {
                    ...
                    for (let info of result) {
                        ...
                        if (info) {
                            setTimeout(() => {
                                ...
                                if (info.isEnded) {
                                    this.setData({
                                        addDeg: (this.data.turntableTime - this.indexRecord + info.currentIndex) * 60,
                                        durationTurn: 1.5
                                    });
                                    // console.log('addDeg',this.data.addDeg,this.data.turntableTime , this.indexRecord , info.currentIndex,this.theDeg)
                                    setTimeout(() => {
                                        if (this.audioCtx1.havedPlay) {
                                            audioRun(this.audioCtx1,'stop')
                                        } else {
                                    ...
                                    setTimeout(() => {
                                        ...
                                        // console.log('addDeg2',this.data.addDeg ,this.theDeg,this.data.turntableTime , this.data.allDeg , this.data.addDeg)
                                        this.setData({
                                            turntableTime: 0,
                                            allDeg: 360,
                                            addDeg: 0,
        ...
        eggStart(e) {
            console.log('砸蛋 eggStart',this.data.disabled)
            if(this.data.disabled)return;
            ...
            allowJoin.call(this, () => {
                ...
                setTimeout(() => {
                    ...
                    getLotteryResult.call(this).then(res => {
                        console.log('getLotteryResult',res);
                        setTimeout(() => {
                            if (this.audioCtx1.havedPlay) {
                                audioRun(this.audioCtx1,'stop')
                            } else {
        ...
        shakeStart() {
            ...
            wx.onAccelerometerChange(res => {
                ...
                if (axisX < res.x || axisY < res.y || axisZ < res.z) {
                    // if(this.data.disabled)return;
                    // setBg.call(this,'shakeStart');
                    calculateTime++;
                    console.log("calculateTime2", calculateTime);
                    // 只有弹窗关闭时候计算
...
function zodiacFunc() {
    ...
    app.EB.listen("zodiac", () => {
        ...
        console.log('zodiacList',zodiacList)
...
function cancelBg(type) {
    let extra = {};
    (type != 'handleAudio' && type != 'failEvent') && (extra.disabled = false);
    this.setData({
        preventBg: false,
        ...extra
    });
...
function handleAudio(priUrl, index = 0, isAutoPlay = false) {
    console.log('进来 handleAudio',...arguments)
    // 兼容方法
    ...
    console.log('进来',audioName,this[audioName])

    this[audioName].onPlay(() => {
        // 说明新接口可以使用了
        console.log('进来 onPlay 当前对象',this[audioName])
        ...
        if (!this[audioName].havedPlay) {
            ...
            checkLoad.call(this);
            // 生肖开始播放
            if (isAutoPlay) app.EB.call("zodiac", this); //只有生肖是isAutoPlay true
            this.watingData = false;
        }
    });
    this[audioName].onError(err => {
        console.info("进来 报错了: " + audioName + "，" + priUrl);
        console.info(err);
        //播放不成功 
        failEvent.call(that,that,isAutoPlay) 
    });
}
...
Array.prototype.shuffle = function () {
    var input = this;
    console.log('打乱1 this',JSON.parse(JSON.stringify(this)));
    ...
    console.log('打乱2 this',JSON.parse(JSON.stringify(this)));
    return input;
}

function failEvent(that,isAutoPlay){
    that.loadAudioData = true;
    checkLoad.call(that);
    // 生肖开始播放
    if (isAutoPlay) app.EB.call("zodiac", that);
    cancelBg.call(that,'failEvent');
    that.watingData = false; 
    // let downLoadTask = wx.downloadFile({
    //     url: priUrl,
    //     success(res) {
    //         console.info("下载音频 downloadFilesucs",res);
