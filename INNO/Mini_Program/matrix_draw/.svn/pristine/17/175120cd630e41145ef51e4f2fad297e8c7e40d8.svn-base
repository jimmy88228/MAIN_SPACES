...
Page.BasePage({
  ...
  bindKeyInput:function(e){
    ...
    // //屏蔽特殊字符
    value = app.StringUtil.safeFilter(value,"address");
  ...
  saveAddress:function(){
    ...
    if (!address_data.consignee) {
      ...
      error_txt="请输入收货人";
    }else if(app.StringUtil.getCharLength(address_data.consignee)>20){
      check_null.consignee_is_null=true;
...
function saveAddressHandle(address_data){
  ...
  if (address_data.address_id){
    reqUrl = "updateUserAddress";
    reqData.addressId = address_data.address_id
  }else{
    reqUrl = "createUserAddress";
  }
