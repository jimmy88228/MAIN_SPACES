...
function SwitchApi({code,setting,data,index,extend,sr_t,Limit_S}) {
  ...
  let p = new Promise((rs, rj) => {
    try{
      ...
      initMsg.hide = true;
      console.log('报错',initMsg)
    }


    if(code == 'text' || code == 'imageAd' || code == 'imageTextNavigate' || code ==  'richText' || code == 'video' || code == 'customerService'||code == 'textScroll'||code == 'goodsList'){ // || initMsg.hide
      initMsg.goApi = false;
