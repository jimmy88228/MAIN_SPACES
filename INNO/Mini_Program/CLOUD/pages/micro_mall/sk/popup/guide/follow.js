...
function switchSmall(isShow){
  ...
  if(!isShow){//缩小
    boxStyle= "right:0;transform: translate(0,20%);transition: all 350ms ease-in-out;";
...
function getGuideFollowWechatInfo(){
  ...
  return app.RunApi.go('SecKillApi', 'getGuideFollowWechatInfo', params, extra).then(res => {
    if(res.code == 1){
      let data = res.data || {};
      this.setData({
        followData: data
...
function pushGuideFollowWechat(){
  ...
  return app.RunApi.go('post','SecKillApi', 'pushGuideFollowWechat', params, extra).then(res => {})
}
