...
Component(app.BTAB({
  ...
  ready(){
    this.setData({
      boxH:`min-height:calc(100vh - ${app.SIH.navPlace||0}px - 90rpx - ${this.data.customTab?app.SIH.isIphoneX?'158rpx':'90rpx':'0px'})`
    })
  },
  methods: {
    onUnloadFn() {
    ...
    reachBottom(type) {  //需要父页面调用原生的 onReachBottom()
      ...
      this[this.current_customName] = this[this.current_customName] || this.selectComponent("#" + this.current_customName);
      if(type == 'callEvent'){
        this[this.current_customName].callEvent();
      }else{
...
function getCustomTabRequest(ops = {}) {
  ...
  return app[apiPack.api][apiPack.url]({
    ...
    if (e.code == 1) {
      ...
      wx.nextTick(()=>{
        that.current_customName = "custom" + that.currentId;
...
function getItemData(_options, isTabPage) {
  ...
  if (isTabPage) {
    ...
    this.current_customName = "custom" + page_id;
  }
  //
  // this.couponAssist = this.couponAssist || this.selectComponent("#couponAssist");
