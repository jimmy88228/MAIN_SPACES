...
Page(app.BP({
    ...
    onLoad: function(options) {
        ...
        this.params = {
            ...
            sortBy: "desc", //降序 
            goods_brand_ids: options.brand_ids ? [options.brand_ids] : [],
          };
        checkSalesVolume.call(this);
        search.call(this); //搜索商品
        loadProperty.call(this); //初始化筛选
    },
    onShow() {
...
function loadProperty(search = false) { //筛选信息
    let that = this;
    ...
    return app.GoodsApi.getGoodsPropertyList({
        ...
        if (e.code == 1) {
            ...
            if (!that.tab) {
                that.tab = that.selectComponent("#tabs");
            }
            that.tab.resetFnc();
...
function search() { //最终loadData接口
    ...
    return app.GoodsApi.searchGoodsList({
        ...
        if (e.code == 1) {
            ...
            this.page = page || 1;
            // let goods_list1 = this.data.goods_list1 || [];
            // let goods_list2 = this.data.goods_list2 || [];
            // let goods_tagList = this.data.goodsTagList;
            // if (list.length == 0) {
            //     this.hasMore = false;
            //     if (this.page == 1) {
            //         this.setData({
            ...
            //     // goods_list = list;
            // } else {
            //     // goods_tagList = goods_tagList.concat(tagList);
            // }
            // let key1 = `goods_list1[${this.page - 1}]`;
            // let key2 = `goods_list2[${this.page - 1}]`;
            // this.setData({
            //   [key1]: temp1,
            //   [key2]: temp2,
            //   // goodsTagList: goods_tagList,
            //   // goods_list: goods_list
            // })
            // this.ctx.append(goods_list);
            // console.log(this.data.goods_list1,"goods_list1");
            // console.log(this.data.goods_list2,"goods_list2");
            let noData = false
            if(list.length == 0){
              this.hasMore = false;
              if (this.page == 1){
