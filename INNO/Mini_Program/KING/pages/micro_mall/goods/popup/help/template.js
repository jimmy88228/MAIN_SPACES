  ...
  initData(type = '', x = 0,y = 0,width = 0, height = 0){
    ...
    if(type=='image'){
      return {
        ...
        category:'image',   //标识
        mode: 'aspectFit',  // widthFix （图片mode）
  ...
  initNewGoodsTemplate(allData={}){
    ...
    if (goodsInfo.price) {
      ...
      if (goodsInfo.min_price != goodsInfo.max_price && goodsInfo.max_price != "0") {
        price.text = "¥" + goodsInfo.min_price + "-" + goodsInfo.max_price + (P_Conf_Bool && goodsInfo.min_integral > 0 ? " + 积分" + goodsInfo.min_integral : "");
      } else {
        // 文字  单价格 加粗
        if (!!!goodsInfo.min_price) {
          price.text = "¥" + goodsInfo.price;
        } else {
          price.text = "¥" + goodsInfo.min_price;
        }
      }
      arr.push(price);
