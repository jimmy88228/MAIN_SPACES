// var DateDown = require("../../../../utils/DateDown.js");
...
Page(app.BP({
    ...
    chooseShareType(data) {
        ...
        activity_info.max_price = activity_info.max_deposit_price;
    ...
    getPresaleGoodsDetail.call(this).then(res => {
        ...
        .then(e => {
            ...
            if (staffInfo.isStaffDstbData) {
                ...
                return this.commission.getCommission("PRESALE", goodsId, this.options.activity_id);
            }
        }).catch(e => {})
    listen.call(this);
    this.setData({
...
function getPresaleGoodsDetail() {
    return app.PreSaleApi.getPresaleGoodsDetail({
        params: {
        ...
        if (res.code == 1) {
            ...
            // if (!activity_info.isNormal && MyDate.parse(activity_info.pre_end_time) <= MyDate.parse(activity_info.server_time)) {
            //     this.setData({
            //         activity_end: true
            //     })
            // }else{
            //   let stime = activity_info.server_time,etime = activity_info.pre_end_time;
            //   if(MyDate.parse(activity_info.pre_begin_time) > MyDate.parse(activity_info.server_time)){
            //     etime = activity_info.pre_begin_time;
            //   }
            //   let check = checkTime.call(this,activity_info); //返回倒计时文案对象
            //   if (check.timeDown) {
...
function getPresaleGoodsProductList() {
    return app.PreSaleApi.getPresaleGoodsProductList({
        params: {
