...
function stringToCode128(text) {
    ...
    function codesForChar(chr1, chr2, currcs) {
        ...
        if (shifter != -1) {
            result.push(shifter);
            // result.push(codeValue(chr2));//修改原因：原因是code128C是纯数字条码，但是必须是偶数位。当出现奇数位是，需要在最后一位前用code128B进行转码声明，但是js中却错误的插入了chr2也就是-1对应的codeValue：62，也就是code128B中62对应的码字_，导致出现奇数位最后一位的错误。
