  ...
  let p = new Promise((rs, rj) => {
    ...
    if (!data[index].itemConfig) { //广告
      ...
      for (let item in details) {
        ...
        this.goodsData[id].img_path = details[item].img_path || false;
      };
      if (arr.length <= 0) {
...
function marqueeView(data = [], config = {}, details = {}, index = 0, tab = false) {
  // console.log('滑动初始化',this.data.cl_curr,index)
  if (!tab) {
  ...
  marquee && marquee.initScroll(data, config, '', 'classify', details);
}
