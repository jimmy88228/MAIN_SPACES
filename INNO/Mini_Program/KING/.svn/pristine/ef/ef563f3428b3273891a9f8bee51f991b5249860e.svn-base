...
const GOSO = {
  ...
  "api_domain": {
    ...
    "PAYAPI": "https://gosomktapi.igoso.com.cn",
...
function getSetConfig(config = {}){
  ...
  else if (config.brandCode == "SINGLE_DOG" || config.brandCode == "CA" || config.brandCode == "SLZL" || config.brandCode == "YNTY" || config.brandCode == "QD" || config.brandCode == "BYGD" || config.brandCode == "SA" ) {
    brandConfig = {
      ...config,
      ...SINGLE_DOG
    };
