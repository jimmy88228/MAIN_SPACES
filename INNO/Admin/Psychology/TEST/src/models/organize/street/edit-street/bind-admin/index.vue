<template>
  <div>
    <Modal v-model="modalShow" :width="1100" class="hide-close street-bind-admin-modal" @on-visible-change="confirmBindAdmin">
      ...
      <template slot="state" slot-scope="{ row, index }">
        <i-switch v-model="row.admin_state" :before-change="()=>{return beforeChangeState(index, row)}" :true-value="1" :false-value="0" size="large" :loading="row.stateLoading" v-hasAction="'street_maintenance_adminstate'">
      ...
      <template slot="handle" slot-scope="{ row, index }">
          <div class="operate-area">
            <a class="operate" @click="editAdmin(row.admin_id)" v-hasAction="'street_maintenance_adminupdate'">编辑</a>
            <Poptip
                confirm
                title="确定删除该管理员？"
                placement="left"
                @on-ok="removeItem(row.admin_id, index)" v-hasAction="'street_maintenance_adminremove'">
                <a class="operate">删除</a>
            </Poptip>
            <a class="operate" @click="resetPAW(row.admin_id)" v-hasAction="'street_maintenance_adminrest'">重置密码</a>
          </div>
      </template>
      </Table>
      <rewrite-page
...
export default {
  ...
  methods:{
    showModal({ streetId, confirm }){
      this.modalShow = true;
      this.searchForm.street_id = Number(streetId) || 0;
      this.confirmCallback = confirm;
      this.loadData();
    },
    onLoadData(page, extraData) {
      return this.$MainApi.streetMaintBindAdmin({
        data: {
            ...this.searchForm,
            ...extraData,
        },
    ...
    addAdmin(){
      this.editTitle = "添加人员";
      let streetId = this.searchForm.street_id;
      this.$refs["editAdminRef"] && this.$refs["editAdminRef"].showModal({ streetId });
    },
    editAdmin(adminId){
      this.editTitle = "编辑人员";
    ...
    removeItem(adminId, index){
      ...
      return this.$MainApi.streetMaintAdminRemove({
        data: {
            admin_id: adminId
        },
      })
    ...
    resetPAW(adminId){
      ...
      return this.$MainApi.streetMaintAdminRest({
        data: {
            admin_id: adminId,
        },
      })
    ...
    setAdminState(adminId, adminState){
      ...
      return this.$MainApi.streetMaintAdminState({
        data: {
            admin_id: adminId,
