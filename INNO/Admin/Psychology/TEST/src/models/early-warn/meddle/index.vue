    ...
    <hold-layout :isFull="true">
        ...
        <rewrite-table ref="myTable" class="full-table" :columns="columns" :data="list" :loading="tableLoading" >
            ...
            <template slot="getrank" slot-scope="{ row, index }">
                {{(row.getrank && row.getrank.level_name) || (row.warning_type == 'change' ? "已解除" : "--")}}
            </template>
...
export default {
    ...
    methods: {
        ...
        addRecord(){
            this.$refs["editMeddleRef"] && this.$refs["editMeddleRef"].showDrawer({userId: this.searchForm.user_id});
        },
        checkRecord(row){
            this.$Modal.info({
                title: "评估建议",
