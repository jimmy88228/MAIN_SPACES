    ...
    <div class="organize-index-area">
      ...
      <div class="tree-table">
        <div class="tree-table-tr tree-table-header">
          ...
          <div class="tree-table-td tree-t-m">注册人数</div>
            ...
            <div class="search-item tree-table-tr" :title="item._parentName.join('/') + '/' + item.title" v-for="(item) in c_searchData" :key="item.id">
                ...
                <div class="tree-table-td tree-t-r">
                    <div class="operate-area">
                        ...
                        <a class="operate" :class="{'limit-level': item.total_level > 3}" @click="editOrganize(item, 'group')" v-hasAction="[item.total_level < 4, 'structure_structure_add']">增加小组</a>
                    </div>
                </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    editOrganize(item, type) {
      ...
      let organizeInfo = {};
      if(type == 'group'){
        if(item.total_level > 3){
          this.$Message.warning("最多只能增加三级组织");
          return;
        }
