<template>
  <div @click="selectItem" class="material-list-item pointer" :class="[isLongStyleType?'long-type':'normal-type']">
    <div class="img-box reletive" :class="{ init: !item.cover }">
      <div v-if="item.cover" v-bgStyle :style="'background-image:url(' + item.cover + ');'" alt="" class="img-cover flex-s0" ></div>
      <div class="default-box flex-c-c" v-if="!item.cover && isLongStyleType">
        <div class="img-default flex-s0" v-bgStyle  :style="'background-image:url(' + DefaultImg[type] + ');'"  alt="" v-if="DefaultImg[type]"></div>
      </div>
      <div class="title-box m-l-10" v-if="isLongStyleType">
        <div class="title">{{item.title}}</div>
         <div v-if="fromType == 'contentRepository'" class="tip-box m-t-10 C_7f">
          <div class="flex-s-c" v-if="item.supplierName">
      ...
      <div class="check-box" v-if="selected">
        <div class="check"></div>
      </div>
...
export default {
  ...
  methods: {
    selectItem() {
      console.log('selectItem',this.index,this.item)
      if(this.disabled){
        this.$Message.warning("该选项不可操作");
        return;
      }
