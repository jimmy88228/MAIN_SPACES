...
export default {
  ...
  props: {
    ...
    limitLevel: Number | String, // 限制可选层数
    showDefault: Boolean,
    defaultData: {
      type: Object,
      default:()=>{
      ...
      if (type == "organize") {
        return OrganizeH.loadData(reqData.type).then((res) => {
          if (res.code) {
            ...
            if(this.showDefault){
              items.unshift(this.defaultData)
            }
      ...
      if (data instanceof Array) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (isNeedLoop && data[i].children && data[i].children.length > 0) {
            ...
            _parent.push(data[i].value);
            data[i].children = this.dataHandle(data[i].children, _parent);
          }
        }
        return data;
      }
