...
export default {
  ...
  computed: {
    ...
    search_list(){
      let search_list = this.asyncList.filter(item=>{
        if(this.inputVal){
          return item.member_name.indexOf(this.inputVal) != -1 || this.inputVal.indexOf(item.member_name) != -1
        } else {
          return true;
        }
      })
  ...
  methods: {
    showModal(selectData) {
      ...
      this.searchForm.structure_id = 0;
      if (!this.isAuto) {
        this.init();
        this.loadData();
    ...
    organizeChange(detail) {
      detail = detail || {};
      let row = detail.row;
      this.searchForm.structure_id = row.id;
      this.loadData();
    },
    confirm() {
