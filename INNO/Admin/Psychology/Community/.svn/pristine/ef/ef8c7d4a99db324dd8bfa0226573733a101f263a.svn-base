<template>
  ...
  >
    ...
    <vue-scroll class="scroll-box">
      <div>
        <div class="flex padding-box">
          <div class="ac-name">{{acInfo.name}}</div>
          <a class="p-l-15" @click="createCode({name:acInfo.name},'act')" v-hasAction="'appraisal_activity_qr_code'">活动码</a>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      baseInfo:{
        'lecturesList': {
          codeId:"specialLecture",
          codeIds:"specialLectures",
          loadDataReq:"specialLectureOrganizeList",
          codeReq:"specialLectureBatchQrCode",
          pageLink: "pages/course-lecture/course-details/course-details",
        },
        'studyTask':{
            codeId:"studyTaska",
            codeIds:"studyTasks",
            loadDataReq:"studyTaskOrgnList",
            codeReq:"studyTaskBatchQrCode",
  ...
  methods: {
    showModal({info,type}) {
      this.type = type || "";
      this.acInfo.name = info.name || info.activity_name || "";
      this.id = parseInt(info.id || 0);
      this.loadData().then(()=>{
    ...
    createCode(row,type) {
        ...
        let params = {
          structureId: row.id || 0,
        }
        if(this.type == 'surveyAct'){
        ...
        this.$UIModule({
            mode: "code-view",
            props: {
                codeTip: `<p class="big-tip">${this.acInfo.name||""}</p>` + `${'<p class="tip">'+(row.structure_name||"")}`,
            },
            options: {  
                path: item.pageLink, 
                params,
    ...
    createCodeReq() {
        ...
        let req = this.baseInfo[this.type].codeReq || "";
        return this.$MainApi[req]({
              data: {
                  ids,
                  activityid: this.id,
