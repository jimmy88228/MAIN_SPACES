...
export default {
  ...
  methods: {
    ...
    viewDetails(row, state) {
      ...
      let query = {
        activityId: this.activityId || "",
        structureId: row.structure_id || (row.is_view == 2 ? -1 : ""),
        structureName: row.structure_name || (row.is_view == 2 ? "未知" : ""),
        modelName: activityInfo.info && activityInfo.info.model_name || "",
        is_view: row.is_view,
        prentStructureIds: row.is_view == 2 ? [-1] : (row.prent_structure_ids || []),
      }
