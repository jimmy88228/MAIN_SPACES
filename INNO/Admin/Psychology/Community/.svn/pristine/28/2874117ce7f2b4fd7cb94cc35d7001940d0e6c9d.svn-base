    ...
    <div>
      <Form ref="organizeInfoForm" :model="organizeInfo" :label-width="90" :rules="ruleValidate">
        <FormItem label="上级部门" :prop="isCheckParentIds ? 'parentIds' : ''">
          ...
          :limitIds="organizeInfo.id ? [organizeInfo.id] : []" :reqData="organizeReqData" 
          class="base-320" 
          type="organize" 
          v-model="organizeInfo.parentIds" 
          valueKey="id" 
          labelKey="structure_name" 
          :limitLevel="4"
          @change="changeOrganize"></data-cascader>
        </FormItem>
...
export default {
  ...
  methods: {
    ...
    confirmReq() {
      ...
      let parentIds = organizeInfo.parentIds || []
      let pid = 0;
      if(parentIds.length > 0){
        pid = Number(organizeInfo.parentIds.slice(-1)[0]);
      } else {
        pid = organizeInfo.pid || 0
      }
