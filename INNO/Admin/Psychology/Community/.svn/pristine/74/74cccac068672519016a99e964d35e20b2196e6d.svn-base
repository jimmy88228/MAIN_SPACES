<template>
    <div class="form-box broadcast-md">
        <div class="form-title bold">广播模块组件</div>
        <mdForm type="broadcast" :styleBox="styleBox" :pageInfo="pageInfo" :dynamic_setting="dynamic_setting" :compInfo="compInfo" @styleSwitch="styleSwitch" @cardClick="cardClick" @addGroup="addGroup" @setDynamic="setDynamic"></mdForm>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        init() {
            ...
            if (!this.module_data.hasOwnProperty('broadcastGroup')) {
                this.$set(this.compInfo.module_data, "broadcastGroup", []);
            }
            console.log('初始化2 dynamic_setting + module_data',JSON.parse(JSON.stringify(this.compInfo)))
        },
        // 添加分组
        addGroup() {
            this.compInfo.module_data.broadcastGroup.push({
                name: "分组名",
                broadcast: [],
            });
        },
        ...
        removeGroup(index) {
            this.$Modal.confirm({
                ...
                cancelText: "取消",
                onOk: () => {
                    this.$delete(this.module_data.broadcastGroup, index);
                },
            });
        },
        // 指定文章图片列表的变动，回调
        onModsListChange(obj) {
            this.$set(this.module_data.broadcastGroup[obj.groupIndex], "broadcast", obj.data);
        },
        // card 点击事件
        cardClick(index) {
