...
export default {
  ...
  methods: {
                    ...
                    .then((res) => {
                        if (res.code) {
                            ...
                            if (data instanceof Array) {
                                ...
                                items = data.items || [];
                                to_items = data.to_items||null;
                            }
                            return this.handleData(items, "init",dataType,to_items);
                            // 缓存
    ...
    handleData(items, type,dataType,to_items) {
        let _items = [];
        if (type == "init") {
            if (!(items instanceof Array) && items instanceof Object) {
                for (let i in items) {
                    let item = {};
                    if(to_items){ //to_items匹配
                      item = {
                        ...(to_items.find && to_items.find(to_item=>to_item.id == i) || {}),
                      }; 
                    }
