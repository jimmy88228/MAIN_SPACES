<template>
    <div class="dimension-dropDown-area">
            ...
            <div class="dimension-dropDown-input"  ref="" :class="{ 'multiple-dropDown-input': multiple && selectView.length, 'is-focus': isFocus }" @click="getFocus">
                <span v-if="multiple">
                    <span class="inline-b" v-for="(item, index) in selectView" :key="index">
                        <Tag :fade="false" class="m-r-5 m-b-5" closable  @on-close="removeValue(item.id)">{{item.dimension_name}}</Tag>
                    </span>
                </span>
                ...
                <div class="dropdown-icons">
                    <Icon :size="16" class="dropdown-icon dropdown-close" @click.stop="clearData()" v-if="selectView.id" style="margin-right: -15px;" type="md-close-circle" />
                ...
                <vue-scroll class="dropDown-scroll" v-bar :style="viewStyle">
                    <div>
                        ...
                        <div v-if="showNum == 0" class="invalid text-c" style="padding: 30px 0px;">
                            暂无维度<template v-if="canEdit">，可自行添加</template>
                        </div>
                    </div>
                </vue-scroll>
        ...
        <editDimension :modelId="modelId" ref="editDimensionRef"></editDimension>
    </div>
</template>
                    ...
                    for(let i = 0; i < dimensionData.length; i++){
                        ...
                        if(result != -1){
                            selectView.push({
                                ...
                                name: dimensionData[i].dimension_name,
                                ...dimensionData[i]
                            });
