      ...
      <div class="edit-cont" v-bar>
        <div class="edit-cont-area">
          <div class="flex-b-c edit-cont-title" :class="{ 'is-handle': superviseInfo.state == 1 }">
            <div>{{superviseInfo.create_time}}</div>
            <div>{{superviseInfo.state_str}}</div>
          </div>
          <Form :label-width="100" :model="superviseInfo" ref="formDataRef">
            <FormItem label="EAP专员">{{superviseInfo.name}}</FormItem>
            <FormItem label="联系电话">{{superviseInfo.mobile_phone}}</FormItem>
            <FormItem label="咨询方向">{{(superviseInfo.get_field && superviseInfo.get_field.field_name) || '--'}}</FormItem>
            <FormItem label="督导方式">{{(superviseInfo.get_consultant_service && superviseInfo.get_consultant_service.service_name) || '--'}}</FormItem>
          ...
          <div class="deal-detail" v-if="superviseInfo.state == 1">
            ...
            <div class="detail-row">
              ...
              <div class="row-r">{{superviseInfo.consult_date}}</div>
            </div>
            <div class="detail-row">
              <div class="row-l">督导方式</div>
              <div class="row-r">{{superviseInfo.get_consultant_confirm_service && superviseInfo.get_consultant_confirm_service.service_name}}</div>
            </div>
            <div class="detail-row">
          ...
          <div class="edit-foot">
            <Button type="primary" :loading="handleLoading" @click="setHandle" v-if="superviseInfo.state != 1">分配督导</Button>
...
export default {
  ...
  methods: {
    ...
    getView(id) {
      if (!Number(id) && Number(id) != 0 || !this.drawerShow) return;
    ...
    setHandle() {
      this.$emit("handle", this.superviseInfo);
    },
