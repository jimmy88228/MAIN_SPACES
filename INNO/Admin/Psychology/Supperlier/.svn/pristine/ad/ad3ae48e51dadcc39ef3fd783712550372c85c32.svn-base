    ...
    <div class="organize-tree-area">
      <div class="tree-area" v-bar>
        <div style="padding-right: 10px;">
          <div v-show="!searchq">
            <Tree :data="c_organizeList" :multiple="multiple" :expand-node="expandNode" :select-node="selectNode" :show-checkbox="showCheckbox" :check-strictly="true"  @on-select-change="toggleSelect"></Tree>
          </div>
          <div v-show="searchq" class="search-area">
            <div class="search-item flex-s-c" :class="[searchClick?'searchClick':'', searchClick && (searchSelectId == item.id) ? 'selected':'']" :title="item.parentName.join('/') + '/' + item.title" v-for="(item) in c_searchData" :key="item.id">
              ...
              <div @click="searchClick?toggleSelectClick(item):''">
                ...
                :disabled="item.disabled || (isRelation && item.pChecked) || (limitMain && item.id == '0' && !item.checked)"
...
export default {
  ...
  methods: {  
    getData(selectData, extra = {}) {
      ...
      console.log('初始化',selectData,this.c_checkData)
      this.loading = true;
