<template>
    <div>
        <Modal v-model="isShowApplet" class="hold-modal-zindex" :width="320" class-name="applet-list-modal" @on-visible-change="onVisibleChange">
            ...
            <!-- <div>选择平台</div>
            <div>
                <Select v-model="appletInfo.platformId" @on-change="changePlatform">
                    <Option v-for="(item, index) in platformData" :value="item.id" :key="item.id">{{item.name}}</Option>
                </Select>
            </div>
            <div>选择小程序</div> -->
            <div>
                    ...
                    <TabPane label="小程序码" name="xcxCode">
                        <div class="img-view" id="img-view">
                            ...
                            <div class="code-tip theme-c" ref="codeTipRef" v-if="codeTip" :style="getTipStyle">
                                <p v-html="codeTip"></p>
                                <p class="code-appname-tip" v-if="appletInfo.appname">小程序：{{appletInfo.appname}}</p>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        getAppletData(){
            // 请求小程序列表需要带上 type  //1 eap平台小程序 2 心理咨询小程序 
            // 自定义页面 默认为1， 咨询师传 2
            return this.$MainApi
                .getAppletList({
                   data: {
                    ...this.listParams instanceof Object ? this.listParams : {},
                    // type: this.appletInfo.platformId 
                   }
                }).then((res)=>{
                    if(res.code){
                        let data = res.data || {};
