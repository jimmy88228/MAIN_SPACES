<template>
    <div class="form-box rich-text-md">
        <div class="form-title bold">富文本模块组件</div>
        <mdForm type="rich-text" :pageInfo="pageInfo" :commonInfo="commonInfo" :styleBox="styleBox" :dynamic_setting="dynamic_setting" :compInfo="compInfo" @styleSwitch="styleSwitch" @cardClick="cardClick" @addGroup="addGroup" @setDynamic="setDynamic"></mdForm>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        init() {
            ...
            if (!this.module_data.hasOwnProperty('richTextGroup')) {
                this.$set(this.compInfo.module_data, "richTextGroup", []);
            }
            console.log('初始化2 dynamic_setting + module_data',JSON.parse(JSON.stringify(this.compInfo)))
        },
        // 添加分组
        addGroup() {
            this.compInfo.module_data.richTextGroup.push({
                name: "分组名",
                richText: [],
            });
        },
        ...
        removeGroup(index) {
            this.$Modal.confirm({
                ...
                cancelText: "取消",
                onOk: () => {
                    this.$delete(this.module_data.richTextGroup, index);
                },
            });
        },
        // 指定文章图片列表的变动，回调
        onModsListChange(obj) {
            this.$set(this.module_data.richTextGroup[obj.groupIndex], "richText", obj.data);
        },
        // card 点击事件
        cardClick(index) {
