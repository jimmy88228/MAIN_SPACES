...
<template>
    <md-view type="rich-text" :compInfo="compInfo" :inTab="inTab"></md-view>
</template>
...
export default {
    ...
    computed: {
        ...
        module_data(){
            ...
            if (!module_data.articlesGroup) {
                this.$set(this.compInfo.module_data, "articlesGroup", [])
            }
            let articlesGroup = module_data.articlesGroup || [];
            let currTab = this.dynamic_setting.currTab || "";
            let currIndex = Number(currTab.replace(/[^\d]/g, "")) || 0;
            let imgL = articlesGroup[currIndex] && articlesGroup[currIndex].articles && articlesGroup[currIndex].articles.length; 
    ...
    methods: {
        ...
        swiperChangeEvent(isAdd = true) {
            ...
            let articlesGroup = this.module_data.articlesGroup || [];
            let dataL = articlesGroup && articlesGroup.articles.length || 0;
            let i = isAdd ? 1 : -1;
            if (dataL > 0) {
                cur = cur + i;
                if (isAdd) {
