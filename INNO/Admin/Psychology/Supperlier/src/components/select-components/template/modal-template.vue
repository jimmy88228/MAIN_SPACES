<template>
    <div>
        <Modal v-model="isVisible" :class-name="'template-modal ' + modalClass" :title="title" :width="modalWidth" :mask-closable="isCloseMask" @on-visible-change="handleModal" transfer>
            <div class="container">
                <div class="left" v-if="!hideSelect">
                    <Card class="left-card">
                        <div class="card_header p-t-10 p-b-10 m-b-10">
                            <p class="mode fs-12"> 已选项 <span class="notice">[{{ type === "radio" ? "单选模式" : "多选模式" }}]</span> </p>
                            <p class="fs-12" v-if="limitSelectNum > 0"> [{{ selectedData.length + "/" + limitSelectNum }}] </p>
                        </div>
                ...
                <div class="right">
                    ...
                    <div :style="'height:' + defaultHeight + 'px;overflow-y:auto'">
                        <slot name="select-ui">
                            <template>
                                <Table :loading="tableLoading" :height="defaultHeight" row-key="id" :columns="tableColumn" :data="list" ref="myTable" @on-select-cancel="handleSelectCancel" @on-select="handleSelect" @on-select-all="handleSelectAll" @on-select-all-cancel="handleCancelAll"></Table>
                            </template>
                        </slot>
                    </div>
