...
<rewrite-drawer class="edit-admin-drawer" v-model="drawerShow" :width="418">
  ...
  <editMember ref="editMemberRef" :formData="adminInfo" type="edu_street"></editMember>
  <div slot="footer">
...
export default {
  ...
  methods:{
    ...
    loadData(adminId){
      ...
      return this.$MainApi.streetMaintAdminInfo({
        data: {
            admin_id: adminId
        },
        other: {
    ...
    updateMemberData(){
      ...
      let req = Number(adminInfo.admin_id) ? 'streetMaintAdminUpdate' : 'streetMaintAdminAdd';
      this.pageLoading = true;
