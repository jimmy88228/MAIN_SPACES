...
export default {
        ...
        if(nV instanceof Array){
          ...
          gradeData.map((item, index)=>{
            if(index == gradeData.length - 1){
              item.warnPercent = item.totalWarn ? lastWarn.toFixed(2) + "%" : "0.00%";
              item.healthPercent = item.totalHealth ? lastHealth.toFixed(2) + "%" : "0.00%";
              item.userPercent = item.totalUser ? lastUser.toFixed(2) + "%" : "0.00%";
            } else {
              ...
              lastWarn = lastWarn.sub(parseFloat(item.warnPercent))
  ...
  methods:{
    ...
    setPercent(val, total){
      ...
      return ((val.div(total)) * 100).toFixed(2) + '%'
    },
    initEvent(){
      //加法   
