...
const LoginManager = new Vue({
    data: {
        mIsLogin: false,
        // mIsNeedResetPwd: false, // 是否需要重置密码
        mUserInfos: {},
        mLoginToken: "",
        mAdminRoleData: []
    },
    computed: {
        ...
        isNeedResetPwd(){
            return (this.mUserInfos && this.mUserInfos.state == 0) || false
        },
        userInfos() {
    ...
    methods: {
        login(account, password, verifyKey, config = {}) {
        ...
        loginByVcode(mobilePhone, vCode, customerId){
            return this.$MainApi.loginByVcode({
                data: {
                ...
                if (res.code) {
                    if(res.select_state != 1 && res.select_state != 2){
                        this.loginDataHandle(res);
                    }
                    return res;
                }
