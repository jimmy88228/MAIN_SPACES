...
class ModuleManager {
    ...
    menusLoop(menusSrc, parent, menusMap){
        menusSrc = menusSrc || [];
        menusMap = menusMap || {};
        let menus = [];
        if(menusSrc instanceof Array && menusSrc.length > 0){
            for(let i = 0; i < menusSrc.length; i++){
                let menuItem = menusSrc[i] || {};
                let menu = {
                    name: menuItem.name,
                    title: menuItem.title,
                    icon: menuItem.icon,
                    sort: this.sortSrc[menuItem.name] || 0,
                ...
                if(menuItem.childs instanceof Array && menuItem.childs.length > 0){
                    menu.childs = this.menusLoop(menuItem.childs, menu, menusMap).menus;
                } else {
                    menu.to = menuItem.to;
                }
                let menusMapItem = JSON.parse(JSON.stringify(menu));
                delete menusMapItem.childs;
                menusMap[menuItem.name] = menusMapItem;
                menus.push(menu);
            }
        }
        return {
    ...
    createMenus() {
        ...
        // let menusMap;
        // let groups = {};
        let { menus, menusMap } = this.menusLoop(this.menusSrc);
        console.log("insta menus", menus, "menusMap", menusMap);
