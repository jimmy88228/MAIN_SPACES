    ...
    <div class="bg-1 act-report">
            ...
            <div class="report-scroll">
              <div class="report-scroll-stay">
                <vue-scroll @handle-scroll="handleScroll" ref="vueScrollRef">
                  <div class="p-r-20 " style="padding-bottom: 35px;">
                    <div class="bg-2 report-item flex-b-c" v-for="(item, index) in actList" :key="item.id">
                      <div class="report-item-l">
                        ...
                        <p class="progress-txt">{{item.state == 3 ? '已结束' : '测评进度'}} {{item.schedule_info.join_member}}/{{item.schedule_info.has_all_join_member}}</p>
                      </div>
        ...
        <div class="bg-1 average-area">
          <div class="item-tip flex-b-c m-b-10">
            <div>
              <p>各年级测评平均分</p>
              <p class="item-sub-tip">统计最近一场的活动数据</p>
            </div>
            <div>
    ...
    dataHandle(data){
      ...
      for(let key in list){
        ...
        for(let i = 0; i < item.length; i++){
          let grade_name = item[i].grade_name;
          if(xName.indexOf(grade_name) == -1){
            xName.push(grade_name);
          }
        }
      }
      // 根据中文排序年级
      ...
      for(let key in list){
        ...
        for(let i = 0; i < item.length; i++){
          let grade_name = item[i].grade_name;
          let index = xName.indexOf(grade_name);
          chartItem.data[index] = item[i].grade_mean;
          maxY = Math.max(maxY, item[i].grade_mean);
        }
          ...
          tooltip: {
              ...
              formatter(params){
                if(!params.seriesName){
                  return `${params.name}&nbsp;&nbsp;&nbsp;<span style="font-weight: bold;">${params.value}</span>`
                } else {
                  return `${params.seriesName}<br /> ${params.name}&nbsp;&nbsp;&nbsp;<span style="font-weight: bold;">${params.value}</span>`
                }
              }
          },
          grid: {
