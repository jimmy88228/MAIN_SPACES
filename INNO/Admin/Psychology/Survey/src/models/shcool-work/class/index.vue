<template>
    <hold-layout :isFull="true">
        <rewrite-area class="flex-b-c">
            <div></div>
            <div>
                <Button @click="editClass()" icon="md-add" v-hasAction="'class_maintenance_add'" >创建班级</Button>
                <Button @click="batchCreate" icon="md-cloud-upload" v-hasAction="'class_maintenance_batch_import'">批量创建</Button>
            </div>
        </rewrite-area>
        <Table ref="myTable" class="full-table"  :columns="columns" :data="list" border :loading="tableLoading">
            ...
            <template slot="handle" slot-scope="{ row, index }">
                ...
                <div class="operate-area">
                    <a class="operate" @click="editClass(row)" v-hasAction="'class_maintenance_edit'">编辑</a>
                    <a class="operate" @click="bindAdmin(row, index)" v-hasAction="'class_maintenance_bindadmin'">绑定班主任</a>
                </div>
            </template>
        </Table>
