<template>
  <hold-layout :isFullHeight="true" layout-content-custom-style="display:flex;min-height:0" layout-custom-style="min-height:0;">
...
import HoldLayout from '@/components/view-components/hold-layout/index.vue';
          ...
          return this.$MainApi.contentList({
            ...
            try {
              if(res.code){
                ...
                list = data.list||[];
                list = list.map(item=>{
                  let id = item[`${this.curItem.type}Id`] || item['id'] || 0
                  item = Object.assign({},
                  {...item},
                  {
                    id: id,
