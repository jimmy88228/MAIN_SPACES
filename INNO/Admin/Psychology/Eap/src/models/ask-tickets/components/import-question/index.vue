  ...
  <custom-modal class="hold-modal-zindex import-problem-modal" ref="modal" :width="952" :footerHide="true" :closable="true">
    <div class="import-problem-cont">
      <div class="import-title">
        <div>选择批量创建类型</div>
        <Select v-model="importType" class="base-select m-b-10">
        ...
        <p>3、题目无选项直接空一行，会默认识别为文本型题目(选择批量类型为问答题时，填写的选项无效) </p>
      </div>
        ...
        <div class="operate-item" v-bar>
          <div class="problems-view">
            <div class="problems-item" v-for="(pItem, pIndex) in problemsData" :key="pIndex">
              <div class="w-break">{{pItem._title}}</div>
              <template v-if="importType == 'single_choice'">
                <RadioGroup v-if="pItem.option_data.length">
              ...
              <template v-else-if="importType == 'multi_choice'">
                <CheckboxGroup v-if="pItem.option_data.length">
                  <div v-for="(item, index) in pItem.option_data" :key="index" v-if="item.option_content">
                    <Checkbox :disabled="true"><span class="w-break">{{item._option_content}}</span></Checkbox>
                  </div>
                </CheckboxGroup>
              </template>
              <template v-else-if="importType == 'q_a'">
                <custom-input type="textarea" class="question-textarea"></custom-input>
              </template>
            </div>
          </div>
        </div>
        ...
        for(let i = 0; i < problems.length; i++){
          ...
          if(optionsList[0]){
            let problem = {
              ...
              title: optionsList[0].substring(0,this.titleLimitNum) || "",
              _title: optionsList[0] || "",
              option_data: option_data,
              random_option: 0,
              is_must: 1
            }
            if(!limitTitleTip && this.titleLimitNum && problem._title.length > this.titleLimitNum){
