<template>
  ...
  >
    <div class="title-box flex-b-c" slot="header">
      ...
      <div class="close relative pointer" @click="dismiss">
        <div class="row absolute translate-center"></div>
        <div class="col absolute translate-center"></div>
      </div>
    </div>
    <div class="scroll-box" v-bar>
      <div>
        <div class="flex padding-box">
          ...
          <a class="p-l-15" @click="createCode({},'act')" v-hasAction="'appraisal_activity_qr_code'">活动码</a>
        </div>
        ...
        <div class="list padding-box flex-b-c" v-for="(item,index) in acInfo.join_data" :key="index">
          <div class="name">{{item.structure_name}}</div>
...
export default {
  ...
  methods: {
    ...
    onLoadData(page, extraData) {
      ...
      return this.$MainApi.organizationalList({
        data: {
          activityid: id,
          ...extraData,
          pageSize: 1000,
        ...
        if (res.code) {
          ...
          this.acInfo.join_data = items;
          console.log("organizationalList", res);
        }
        return res
      });
    },
    ...
    createCode(row={},type) {
        ...
        this.$UIModule({
            mode: "code-view",
            props: {
                codeTip: `<p class="big-tip">${this.acInfo.activity_name||""}</p><p class="tip">${row.structure_name||""}</p>`
            },
            options: {
    ...
    createCodeReq() {
        ...
        return this.$MainApi
          .batchQrCode({
              ...
              other: { isShowMsg: true , isErrorMsg: true},
          })
          .then((res) => {
              let data = res.data;
