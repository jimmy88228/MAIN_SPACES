...
<template>
    <md-view type="video" :compInfo="compInfo" :inTab="inTab"></md-view>
</template>
...
export default {
    ...
    computed: {
        ...
        module_data(){
            ...
            if (!module_data.videosGroup) {
                this.$set(this.compInfo.module_data, "videosGroup", [])
            }
            let videosGroup = module_data.videosGroup || [];
            let currTab = this.dynamic_setting.currTab || "";
            let currIndex = Number(currTab.replace(/[^\d]/g, "")) || 0;
            let imgL = videosGroup[currIndex] && videosGroup[currIndex].videos && videosGroup[currIndex].videos.length; 
    ...
    methods: {
        ...
        swiperChangeEvent(isAdd = true) {
            ...
            let videosGroup = this.module_data.videosGroup || [];
            let dataL = videosGroup && videosGroup.videos.length || 0;
            let i = isAdd ? 1 : -1;
            if (dataL > 0) {
                cur = cur + i;
                if (isAdd) {
        ...
        checkEmpty(module_data){
            let group = module_data.videosGroup||[];
            return group.length>0 && group[0] && group[0].videos && group[0].videos.length>0 || false
        },
    },
    watch: {},
    mounted() {
