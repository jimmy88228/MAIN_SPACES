<template>
    <div class="form-box article-md">
        <div class="form-title bold">文章模块组件</div>
        <mdForm type="article" :styleBox="styleBox" :dynamic_setting="dynamic_setting" :compInfo="compInfo" @styleSwitch="styleSwitch" @cardClick="cardClick" @addGroup="addGroup" @setDynamic="setDynamic"></mdForm>
...
export default {
    ...
    methods: {
        ...
        init() {
            ...
            if (!this.module_data.hasOwnProperty('articlesGroup')) {
                this.$set(this.compInfo.module_data, "articlesGroup", []);
            }
            console.log('初始化2 dynamic_setting + module_data',JSON.parse(JSON.stringify(this.compInfo)))
        },
        // 添加分组
        addGroup() {
            this.compInfo.module_data.articlesGroup.push({
                name: "分组名",
                articles: [],
            });
        },
        ...
        removeGroup(index) {
            this.$Modal.confirm({
                ...
                cancelText: "取消",
                onOk: () => {
                    this.$delete(this.module_data.articlesGroup, index);
                },
            });
        },
        // 指定文章图片列表的变动，回调
        onModsListChange(obj) {
            this.$set(this.module_data.articlesGroup[obj.groupIndex], "articles", obj.data);
        },
        // card 点击事件
        cardClick(index) {
