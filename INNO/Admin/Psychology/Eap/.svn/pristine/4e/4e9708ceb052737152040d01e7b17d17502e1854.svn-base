...
export default {
    ...
    methods: {
         ...
         * }
        */
        _initTreeData(data,  _selectData = [], extra = { parentName: [], parentIds: [], pChecked: false}) {
            extra = extra || {};
            let parentName = extra.parentName || []; 
            let parentIds = extra.parentIds || [];
        ...
        _searchTreeCheck(state, item) {
            if (state) {
                ...
                let dIndex = this.ids.indexOf(Number(item.id));
                this.c_checkData.splice(dIndex, 1);
            }
        },
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    switch (key) {
                        ...
                        case "selected":
                            ...
                            break;
                        case "checked":
                            if (item.id == (curr && curr.id) || item.parentIds.indexOf(curr && curr.id) != -1) {
                                ...
                                if (item.id == (curr && curr.id)) { //定位到点击的元素
                                    ...
                                    // console.log('c_checkData',item.id,curr.id,curr[key]?'操作元素的下级':'删除选择合集中的元素',item.id == (curr && curr.id) , item.parentIds.indexOf(curr && curr.id) != -1, idIndex , JSON.parse(JSON.stringify(this.c_checkData)))
                                }
                            }
                            break;
                        default:
