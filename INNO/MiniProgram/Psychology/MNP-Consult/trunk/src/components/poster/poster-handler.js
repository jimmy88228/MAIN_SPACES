			...
			return new Promise((rs, rj)=>{
				if(posterDom) {
				  ...
				  .exec((res) =>{
				    ...
				    canvas.width = width * SaveMult;
				    canvas.height = height * SaveMult;
				    ctx.fillStyle = "#fff";
		      ...
		      switch(type){
		        ...
		        case "text":
		          ...
		          ctx.textBaseline = "top";
							let left = css.left * dpr;
							if(css.textAlign == 'center'){
								left = (canvas.width/SaveMult - ctx.measureText(viewData[i].text).width) / 2;
							}
		          ctx.fillText(viewData[i].text, left, css.top * dpr);
							// console.log('centercenter',canvas.width/SaveMult,ctx.measureText(viewData[i].text).width,left)
		          break;
		        case "image":
		          if(_url) {
		            let img = canvas.createImage();
		            ...
		            img.onload = function(){
		              ...
		              ctx.drawImage(img, offsetX, offsetY, css.width * dpr, css.height * dpr)
								...
								// console.log('imageimage',css.left * dpr, css.top * dpr,css.width * dpr, css.height * dpr)
		          }
		          break;
		        case "code":
		          if(_url) {
