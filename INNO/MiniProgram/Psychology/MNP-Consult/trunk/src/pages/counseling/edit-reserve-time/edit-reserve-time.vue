<template>
  <view :class="!isEmpty ? 'bg_f7' : 'bg_fff'">
    <view v-show="!showLoading" class="reserve-time">
      <page-nav :isFull="false">
        <template slot="title"> 配置时段 </template>
      </page-nav>
      <view v-if="!isEmpty">
        <editDateSelect :serviceTime="serviceTime" :time-all-data="timeAllData" :selectedTime="selectedTime"
    ...
    <view class="loading-view flex-c-c" v-show="showLoading">
...
<script>
  ...
  const pageOption = Page.BasePage({
    ...
    methods: {
      // 初始化日期选择
      init() {
        let thirtyDate = dateUtil.getFollowUpDate(this.serviceTime,30);
      ...
      getConsultantSchedule() {
        ...
        let startDateObj = dateUtil.getDate(serviceTime, 0) || {}
        let endDateObj = dateUtil.getDate(serviceTime, 29) || {}
