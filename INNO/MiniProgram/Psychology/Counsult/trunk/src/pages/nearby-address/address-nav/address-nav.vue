<template>
  <view class="adress-nav">
    <page-nav>
      <template slot="title"> 社会心理服务导航 </template>
    </page-nav>
    <view v-if="showImg != -1">
          ...
          <view class="store_item" :data-store_sub="index" @click="getAddrMap" :data-item="item"
            ...
            <view class="service_group" v-if="item.psychicHotline || item.residentBookPhone">
              <view class="heart_service" v-if="item.psychicHotline">
                <image :src="setStaticAddress('/nearby-address/heart_white.png')" mode="heightFix" />
                电话心理咨询预约
              </view>
              <view class="store_service" v-if="item.residentBookPhone">
                <image :src="setStaticAddress('/nearby-address/store_white.png')" mode="heightFix" />
                现场心理咨询预约
              </view>
            </view>
            <view class="store_info flex-s-c font-22 m-t-22">
              <image :src="setStaticAddress('/nearby-address/address.png')" mode="aspectFit" />{{item.address}}</view>
            <view class="store_info flex-s-c font-22" v-if="item.telephone">
              <image :src="setStaticAddress('/nearby-address/telephone.png')" mode="aspectFit" />{{item.telephone}}
            </view>
            <view class="store_info flex-s-c font-22">
              <image :src="setStaticAddress('/nearby-address/time.png')" mode="aspectFit" />{{item.serviceTime}}</view>
          </view>
