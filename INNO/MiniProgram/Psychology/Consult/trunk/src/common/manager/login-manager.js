...
class LoginManager {
  ...
  loginAsync(showLoading) {
    ...
    this._loginHold = this.getWxSessionIdAsync().then((sessionId)=>{
      ...
      setTimeout(()=>{
        ...
        console.log('loginAsync finally',this._loginHold);
      }, 50)
    })
  ...
  loginBsnAsync(showLoading, isCheck){ // isCheck 只是检测，不存信息
    ...
    this._loginBsnHold = Func._LoginBsn.call(this,showLoading).then((res)=>{
      ...
      // 直接获取关联人信息
			...
			setTimeout(()=>{this._loginBsnHold = null}, 50)
		})
    return this._loginBsnHold;
  }
	//检测授权用户注册
	getTokenRegisterAsync(showLoading, cache = true,params={}) {
    console.log('getTokenRegisterAsync',this.isLogin)
	  if(this.isLogin && cache){
		...
		return this.getWxSessionIdAsync().then((sessionId)=>{
			return this.registerAsync(showLoading, sessionId,params).then(userToken => {
        console.log('注册成功',userToken,this._userToken,this.isLogin)
			  return {userToken, cache: false};
			});
		})
	}
  //异步授权用户注册
  registerAsync(showLoading, sessionId,params) {
    console.log('registerAsync')
		if(this._regHold) return this._regHold;
    this._regHold = Func._registerAsync.call(this,showLoading, sessionId,params).finally(()=>{
				setTimeout(()=>{this._regHold = null}, 50)
      })
    ...
    this._regBHold = Func._registerBsnAsync.call(this, showLoading, id).then((res)=>{
      ...
      // 直接获取关联人信息
			IM.getUserInfoByToken();
				...
				setTimeout(()=>{this._regBHold = null}, 50)
    })
  ...
  updateUserProfile(showLoading){
    ...
    this._profileHold = this.getWxSessionIdAsync().then((sessionId)=>{
      ...
      setTimeout(()=>{this._profileHold = null}, 50)
    })
  ...
  createWxSessionId(showLoading) {
    console.log('createWxSessionId')
		if(this._cwxsHold) return this._cwxsHold;
    this._cwxsHold = Func._createWxSession.call(this,showLoading).finally(()=>{
        setTimeout(()=>{this._cwxsHold = null}, 50)
    })
    return this._cwxsHold;
  }
  //创建sessionId
  bindMobilePhone(showLoading) {
    console.log('bindMobilePhone')
		if(this.bdpHold) return this.bdpHold;
    this.bdpHold = Func._bindMobilePhone.call(this,showLoading,this.sessionId).finally(()=>{
        setTimeout(()=>{this.bdpHold = null}, 50)
    })
    return this.bdpHold;
  }
	// 刷新sessionId
...
const Func = {
  ...
  _Login(showLoading, sessionId){ 
			return userLogin(showLoading, sessionId,this).then((res)=>{
				if(res.code){
  ...
  _registerAsync(showLoading, sessionId,params){ 
    return register.call(this,showLoading, sessionId,params);
  },
  ...
  _bindMobilePhone(showLoading,sessionId){
    return bindMobilePhone(showLoading,sessionId)
  }
}
...
//授权token请求
function userLogin(showLoading, sessionId,that) {
...
//用户注册
function register(showLoading, sessionId,params){
...
function createSession(showLoading, code) {
  return Http(Apis.createSession,{
    data: {
      code: code
    },
    other: {
...
//绑定手机
function bindMobilePhone(showLoading,sessionId) {
  return Http(Apis.bindMobilePhone,{
    data: {
      sessionId,
    },
    other: {
