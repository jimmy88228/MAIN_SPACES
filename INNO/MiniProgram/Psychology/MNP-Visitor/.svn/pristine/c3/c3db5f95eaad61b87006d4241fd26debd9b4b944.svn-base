<template>
	<view class="index-page">
		<!-- <view class="address-area" v-if="address">{{address}}</view> -->
	...
	const pageOption = Page.BasePage({
		...
		methods: {
			...
			reFreshInfo() {
				...
				return //暂时不需要显示address
				this.authUserInfo = app.IM.authUserInfo || {};
				let structureLen = this.authUserInfo?.structureWithParents.length || 0
				this.address = this.authUserInfo?.structureWithParents[structureLen - 1]?.structureName || ''
			},
		},
	})
