...
class LoginManager {
  ...
  loginBsnAsync(showLoading, isCheck){ // isCheck 只是检测，不存信息
	...
	getTokenRegisterAsync(showLoading, cache = true) {
      ...
      if(Conf.isVisitor){
        return this.registerByTouristAsync(showLoading, sessionId).then(userToken => {
  ...
  // 异步游客用户注册
  registerByTouristAsync(showLoading, sessionId) {
		if(this._regByTouristHold) return this._regByTouristHold;
    this._regByTouristHold = Func._registerByTourist.call(this,showLoading, sessionId).finally(()=>{
				setTimeout(()=>{this._regByTouristHold = null}, 500)
      })
    return this._regByTouristHold; 
  }
	// 获取关联用户
...
const Func = {
  ...
  _registerByTourist(showLoading, sessionId){ 
    return registerByTourist.call(this,showLoading, sessionId);
  },
  uniLogin(provider){
    //#ifdef MP
...
// 游客登录
function registerByTourist(showLoading, sessionId){
  return Http(Apis.registerByTourist,{
    data:{
      sessionId: sessionId,
    },
    other:{
