  ...
  >
    <template v-slot:content>
      ...
      >
        ...
        <image v-show="hasDraw" :src="posterImg" mode="aspectFit" :style="canvasStyle" class="img-poster" :class="{'anim':anim}" />
...
const BaseComp = Page.BaseComp({
  ...
  computed:{ 
      ...
      // console.log('canvas',`width:${width}rpx;height:${height}rpx`)
  ...
  methods: {
    showModal(posterMsg) {
      ...
      if(this.posterMsg.width || this.posterMsg.height){
        // this.posterMsg.width && (this.width = this.posterMsg.width);
        this.posterMsg.height && (this.height = this.posterMsg.height);
      }
      this.$nextTick(()=>{
        this.$refs.popup.show();
        this.initCanvas();
