...
const TCR_USER_INFOS_KEY = "TCR_USER_INFOS";
const BSN_USER_INFOS_KEY = "BSN_USER_INFOS";
const LOGIN_CERTIFICATE = "LOGIN_CERTIFICATE"
    ...
    this._loginHold = this.getWxSessionIdAsync().then((sessionId) => {
      return Func._Login.call(this, showLoading, sessionId).then((data) => {
        ...
        if (token && token != oldToken) {
          return IM.getAuthUserInfoByLogin().then(err => {
...
saveOpenId(openId) {
  ...
  logout(type = 'userToken') {
    // #ifdef H5
    StorageH.remove(LOGIN_CERTIFICATE);
    // #endif
  ...
  removeLoginData(type = 'userToken') {
    ...
    if (type == 'all' || type == 'tcrUserInfos') {
      IM.removeUserInfo("tcrUserInfos")
    }
    if (type == 'all' || type == 'bsnUserInfo') {
      IM.removeUserInfo("bsnUserInfo")
    }
  }
}
