      ...
      <view v-if="!isEmpty">
        ...
        <view class="report-list-area relative p-l-15 p-r-15 p-b-32">
          ...
          <view v-for="(listItem, itemIndex) in list" :key="itemIndex">
            <animateCustom animation-class="animate-fade-in-right" :animation-delay="item.showTime" v-for="(item, i) in listItem" :key="i" >
              <template slot="content">
                <view class="report-item" @click="jump" :data-p-index="itemIndex"
                  :data-url="`pages/report-info/report-info?recordId=${item.recordId}`">
                  <oriImage :src="staticAddress+reportItemIcon" customStyle="width: 724rpx;height: 194rpx; z-index: 99;" class="absolute"></oriImage>
                  <!-- <image :src="staticAddress+reportItemIcon" class="report-item-background" /> -->
                  <view class="report-item-info flex-b-s">
                    <view class="relative">
                      <image :lazy-load="true" class="report-item-image" :src="item.logo ? item.logo : staticAddress+activityIcon"
