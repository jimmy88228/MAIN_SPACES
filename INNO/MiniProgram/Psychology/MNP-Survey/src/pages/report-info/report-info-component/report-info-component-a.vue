  ...
  <view class="report-info" v-if="hasData">
    ...
    <view class="report-area p-l-25 p-r-25 p-b-32">
      ...
      <view class="report-score-table m-b-25">
        ...
        <view class="flex C_333 font-22 p-l-20 p-r-20" v-for="(item, i) in analyze.ruleList" :key="i">
          ...
          <view class="report-score-table-row2 p-t-20 p-b-20 p-l-20 p-r-20">{{
            hasPoint ? item.coefficientPoints.toFixed(2) : item.coefficientPoints
          }}</view>
      ...
      <view class="analyze-item m-t-20 p-l-32 p-r-32" v-for="(item, i) in analyze.ruleList" :key="i">
  ...
  const pageOption = Page.BasePage({
    ...
    computed:{
      hasPoint(){
        ...
        let ruleList = analyze.ruleList || [];
        let hasPoint = false
        for(let i = 0;i < ruleList.length;i++){
          let item = ruleList[i];
          let coefficientPoints = item.coefficientPoints;
          hasPoint = String(coefficientPoints).includes('.')
          if(hasPoint) break
        }
        return hasPoint
      }
