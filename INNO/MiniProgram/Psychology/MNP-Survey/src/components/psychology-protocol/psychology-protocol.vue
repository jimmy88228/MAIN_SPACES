        ...
        <view class="psychology-protocol-info">
          <view>
            <view class="psychology-protocol-bg bold">
              <view class="psychology-protocol-title font-28 m-t-15 m-b-20">{{showName}}保密协议</view>
              <scroll-view scroll-y class="psychology-protocol-detail">
                <text class="p-b-32 font-24">
                    本协议是保障学校心理关爱活动正常开展的基本框架，为了保障学生的个人隐私不被泄露，您的关爱工作能顺利有效地进行，请仔细阅读以下内容并确认表示同意。

                    保密条款
                    一、{{showName}}应本着尊重、保护学生个人隐私的原则,对心理关爱过程中的有关信息,包括个案记录、测评报告、信件、录音和其他资料,均属保密信息,都应在严格保密的情况下进行保存。
                    二、{{showName}}必须严格遵守心理关爱工作的保密原则,对学生个人心理档案的相关资料进行严格保密。
                    三、上述保密资料,除您本人之外的任何人（包括直属领导或非直属领导、其他老师、其他同事）均不得查阅。
                    四、保密例外（包括但不限于）:
                    1.当您发现您负责关爱的学生可能存在伤害自身或伤害他人的严重风险时；
                    2.您负责关爱的学生有致命的传染性疾病且可能危及他人生命健康时；
...
<script>
  ...
  const BaseComp = Page.BaseComp({
    ...
    computed:{
      showName(){
        let roleType = this.tcrUserInfo && this.tcrUserInfo.roleType ||'';
        let result = "";
        switch (roleType) {
          case 'class_teacher':
            result = "班主任"
    ...
    methods: {
      showModal() {
        this.tcrUserInfo || (this.tcrUserInfo = app.IM.tcrUserInfo||{});
