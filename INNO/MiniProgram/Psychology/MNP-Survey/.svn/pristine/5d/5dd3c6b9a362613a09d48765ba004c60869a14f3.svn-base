<template>
  <view class="rank-area" :style="{ 'background-color': actInfo.backgroundColor }">
    ...
    <view class="rank-header-area">
      <image class="rank-header" :src="actInfo.rankingImg || staticAddress + '/game/different/rank-header.jpg'"
        mode="widthFix" />
      ...
      <view class="rank-list">
        <scroll-view v-show="!showLoading" class="rank-scroll" :scroll-y="true">
          <view class="rank-item" :class="{'curr-rank-item':  item.userRanking}" v-for="(item, index) in rankListView"
            :key="index">
            <template v-if="!item.userRanking">
              <view class="item-left">
                <view class="item-top-area" v-if="index < 3">
                  <image class="top-area-img"
                    :src="staticAddress + '/game/different/top' + (Number(index) + 1) + '.png'" mode="widthFix" />
                </view>
            ...
            <view class="item-cont">
              <view class="item-name-area" :style="{'padding-left':item.userRanking && '30rpx'}">
                <view class="item-name" :class="{'invalid': !item.userId}">{{item.userId ? item.nickName : '虚位以待'}}
                </view>
                <view class="edit-img-point" @click="changeNameAction(index, item)">
                  <image v-if="item.userRanking" class="edit-name-img"
                    :src="staticAddress + '/game/different/edit.png'" />
                </view>
              </view>
  ...
  const pageOption = Page.BasePage({
    ...
    methods: {
      ...
      setLike(index, item) {
        ...
        if (!Number(options.gameActivityId)) {
          return Promise.reject()
        }
        if (this.liking) {
          return
