<template>
  <view class="caring-staff" @click="pageClick">
    <page-nav>
      <template slot="title">
        <view>关爱人员</view>
      </template>
        ...
        <view class="flex-s-c caring-staff-info flex-wrap">
          ...
          <template v-if="item.exit != 1">
            ...
            <template v-if="item.referral > 0 && item.referral != 3">
              <view class="organization-type referrals" @click.stop="showTips(i)">
                {{getReferralText(item)}}
                <view  :class="!item.showTips?'hideTips':''" class="referral-tips C_fff">{{getReferralText(item)}}</view>
              </view> 
            </template>
          </template>
        </view>
        <view class="flex-b-c" v-if="item.organizeType != 0"> 
          <view class="button flex-c-c" @click.stop="turnPage" :data-item="item" data-mode="intervene-record" :data-key="['memberName','userId','structureId','warningLevel']">
            <image :src="staticAddress+interventionRecordIcon" mode="widthFix" />
            <view class="C_555 font-22">干预记录</view>
          </view>
          <view class="button flex-c-c" @click.stop="turnPage" :data-item="item" data-mode="archives" :data-key="['memberName','userId']">
            <image :src="staticAddress+mentalFileIcon" mode="widthFix" />
            <view class="C_555 font-22">档案详情</view>
          </view>
          <view class="button flex-c-c" :class="{'disabled':item.referral > 0 && item.referral != 3 }" @click.stop="turnPage" :data-item="item" data-mode="referrals" :data-key="['memberName','userId','structureId']">
...
import StorageH from "@/common/helper/storage-handler.js";
  ...
  const pageOption = Page.BasePage({
    ...
    methods: {
      ...
      // 关闭tips
      pageClick(){
        if(!this.hasTipsShow) return
        let list = this.caringList;
        list.forEach((item,i)=>{
            item.showTips = false
        })
        this.hasTipsShow = false;
        this.list = list;
      }
    }, 
  })
        ...
        .organization-type {
          ...
          .referral-tips{
            ...
            &:after{
              ...
              transform: translateX(-50%);
              content: '';
              border-top:20rpx solid rgba($color: #000000, $alpha: 0.6);
              width:0px;
              height:0px;
