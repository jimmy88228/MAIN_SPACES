  ...
  <view class="report-info bg_f9fcfe" v-if="hasData">
    ...
    <view class="report-item" v-for="(item,i) in analyze.reportDetailBase" :key="i">
      ...
      <template v-else-if="item.reportStyle == 'rutter'">
        <reportInfoRutter ref="reportInfoRutter" :analyze="item" :has-data="hasData"></reportInfoRutter>
      </template>
...
<script>
  import reportInfoA from "@/pages/report-info/report-info-component/report-info-component-a.vue"
  import reportInfoB from "@/pages/report-info/report-info-component/report-info-component-b.vue"
  import reportInfoC from "@/pages/report-info/report-info-component/report-info-component-c.vue"
  import reportInfoRutter from "@/pages/report-info/report-info-component/report-info-component-rutter.vue"
  import reportInfoSectional from "@/pages/report-info/report-info-component/report-info-component-sectional.vue"
  ...
  const pageOption = Page.BasePage({
    ...
    methods: {
      loadData() {
        ...
        let api = userId ? this.$Apis.getReportDetailByAdmin : this.$Apis.getReportDetail
        let formData = {
          recordId
        }
        if(userId){
          formData = Object.assign(formData,{userId})
        }
