	...
	const pageOption = Page.BasePage({
		...
		methods: {
			init() {
				...
				// #ifdef MP
				ScanCode.scan("normal").then((data) => {
					this.initLogin(data);
				})
				// #endif
			},
			...
			afterCheckCode(data,platformINfo = {}) {
				return this._checkLogin(null, false).then((login) => {
					...
					console.log(data && data.pagePath, "data && data.pagePath")
					if (data && data.pagePath) {
						this.redirectAction("/" + data.pagePath + "?customType=normal&" + utils.paramsByJson(data
							.sceneOption))
					} else {
