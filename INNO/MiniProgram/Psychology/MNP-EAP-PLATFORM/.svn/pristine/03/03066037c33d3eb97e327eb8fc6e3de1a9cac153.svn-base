...
export default {
	data(){
		return {
			...
			isMP:PLH.isMP,
			mixShowPage:false,
			TabbarSafeArea:(150 - 40),
			TabbarSafeAcArea:150,
	...
	methods: {
		...
		_checkLastLogin(){
			return PLM.setLastLoginCustomer().then((res)=>{
				console.log(res,"checklastLogin")
				if(res.appCode){
		...
		// 登录
		_checkLogin(callback, isPrueCheck){
			return LM.loginAsync(false, isPrueCheck).then(()=>{
