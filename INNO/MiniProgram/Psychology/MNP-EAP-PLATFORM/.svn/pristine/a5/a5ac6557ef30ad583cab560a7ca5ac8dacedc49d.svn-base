<template>
  <view class="index-page" :style="{'background-color':backgroundColor}">
    <custom-page :scroll-height="scrollHeight" :safeArea="TabbarSafeAcArea" :customType="customType" ref="customPageRef"
      :nav-full="true" :navIsTransparent="true"></custom-page>
    <share ref="shareModule"></share>
    <cuTabbar :showSXIndex="showSXIndex" ref="tabbar" selectedKey="platformIndex"></cuTabbar>
  </view>
</template>
...
<script>
  ...
  const pageOption = Page.BasePage({
    ...
    onLoad() {
      SSXI.getShowSXIndexConfig().then((data) => {
        this.showSXIndex = data
      })
    },
    onReady() {
      uni.$on("changeSXIndexConfig", () => {
        SSXI.getShowSXIndexConfig().then((data) => {
          this.showSXIndex = data
        })
        this.init();
    ...
    methods: {
      ...
      getPage() {
        return this.$Http(this.$Apis.getPageDetail, {
          ...
          this.$refs["customPageRef"].initData();
        })
      },
    },
  })
