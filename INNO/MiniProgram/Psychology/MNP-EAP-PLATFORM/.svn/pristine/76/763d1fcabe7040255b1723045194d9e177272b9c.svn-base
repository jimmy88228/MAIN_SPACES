	...
	<view class="carousel-area">
		<template v-if="showStyle < 3">
			<swiper :style="'height:'+swiperH+'px;'" class="swiper-area" :autoplay="dynamicSetting.autoPlay" :circular="true"
				...
				<swiper-item class="swiper-item" @click="jumpAction(item.link)" v-for="(item, index) in imageList" :key="index">
					<view class="img-item">
		...
		<template v-else>
			<view class="float_swiper" :current="swiperCurrent" @touchstart="swiperTouchS" @touchend="swiperTouchE">
				<view class="float_swiper_item" @click="jumpAction(item.link)" v-for="(item, index) in imageList" :key="index"
...
<script>
	...
	const pageOption = Page.BasePage({
		...
		computed: {
			...
			imageList(){
				let images = this.viewData.images || [];
				let customType = this.customType || "";
				images.forEach(item => {
					item.link.customType = customType
				});
				return images
			}
		},
