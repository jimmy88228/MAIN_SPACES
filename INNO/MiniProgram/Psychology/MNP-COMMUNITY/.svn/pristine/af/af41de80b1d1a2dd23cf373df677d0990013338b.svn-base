<template>
  <view class="course-video">
    ...
    <video @play="stopAudio" :enable-play-gesture="true" :enable-progress-gesture="true" :show-center-play-btn="false" id="video" @timeupdate="videoTimeUpdate" autoplay class="video-style"
      ...
      <view class="course-info flex-b-c">
        <view class="curse-info-detail">
          ...
          <view v-if="timeStr.trim().length>0 && !isFinish" class="curse-info-detail-time flex-s-c">
            <image class="icon-time m-r-10" :src="setStaticAddress('/course-video/time.png')" mode="aspectFit" />
        ...
        <template v-if="showPunch">
          ...
          <template v-else>
            <image class="punch-success shrink0" :src="setStaticAddress('/course-video/punch-success.png')"
              mode="aspectFit" />
          </template>
        </template>
      </view>
    ...
    <view class="course-catalog">
      <view class="course-catalog-count font-22 C_7f m-b-10">课程目录 {{subsections.length || ''}}节</view>
      <template v-if="subsections">
        ...
        <scroll-view 	:scroll-with-animation="true" @scroll="scrollHorList" class="course-catalog-list" :scroll-into-view="'id'+scrollId" scroll-x>
          <view :id="'id'+item.id" class="course-catalog-item" v-for="(item,index) in subsections" :key="index">
            <view class="item-cont bg_f8 relative" :data-c-index="index" :data-c-item="item" @click="clickItem">
              ...
              <view class="font-26 clamp2" style="white-space: pre-wrap;"
                :class="[item.click?'C_52b200 bold':item.needlock?'C_7f':'C_333']">{{item.contentName}}</view>
              <image v-if="item.needlock" class="lock-icon absolute" :src="setStaticAddress('/course-video/lock.png')"
                mode="aspectFit" />
            </view>
          </view>
      ...
      <template v-slot:content>
        <view class="popup-content" :style="{height:`${popupHeight}px`}">
          <view class="popup-title flex-b-c">
            ...
            <image @click="hideBench" :src="setStaticAddress('/course-video/close_popup.png')" mode="aspectFit" />
          </view>
...
<script>
  ...
  const pageOption = Page.BasePage({
    ...
    methods: {
      ...
      autoPunchCard(time = 1) {
        let intervalTime = time * 60000
        if (this.autoPunchInterval) {
          this.playedTime = 0;
          clearInterval(this.autoPunchInterval);
          this.autoPunchInterval = null;
        }
      ...
      clickItem(e) {
        ...
        if (clickItem.contentType == 'audio') {
          ...
          courseManage.punchCard(formData).then(res => {
            this.jumpAction(
              `/pages/course/course-audio/course-audio?id=${clickItem.relateId}&courseId=${courseId}`
            )
          })
          ...
          this.getSubInfo(ops)
          this.loadContentDetail(clickItem.id)
          this.playedTime = 0;
