        ...
        <scroll-view @scroll="scrollHorList" class="course-catalog-list" :scroll-into-view="'id'+scrollId" scroll-x>
          <view :id="'id'+item.id" class="course-catalog-item" v-for="(item,index) in subsections" :key="index">
            <view class="item-cont bg_f8 relative" :data-c-index="index" :data-c-item="item" @click="clickItem">
              ...
              <view class="font-24 clamp2" style="white-space: pre-wrap;"
                :class="item.click?'C_52b200':'C_7f'">{{item.contentName}}</view>
...
<script>
  import courseModule from "@/components/course-lecture-module/course-lecture-module"
  import oriPopup from "@/components/ori-comps/popup/ori-popup";
  import courseManage from "@/common/manager/course-lecture-manage.js";
  ...
  const pageOption = Page.BasePage({
    ...
    methods: {
      ...
      loadContentDetail(contentId, init = false) {
        return this.$Http(this.$Apis.getLectureCourseContent, {
          data: {
      ...
      clickItem(e) {
        ...
        if (clickItem.contentType == 'audio') {
          ...
          if (!clickItem.isFinish) {
            this.jumpAction(
              `/pages/activities/exam-lecture/answer/answer?activityId=${activityId}&contentId=${clickItem.id}&courseId=${courseId}&modelId=${clickItem.relateId}`
            )
          } else {
            this.jumpAction(
              `/pages/activities/exam-lecture/answer/exam-record?activityId=${activityId||0}&contentId=${clickItem.id||0}&courseId=${courseId||0}&modelId=${clickItem.relateId||0}`
            )
          }
        }
        this.hideBench()
