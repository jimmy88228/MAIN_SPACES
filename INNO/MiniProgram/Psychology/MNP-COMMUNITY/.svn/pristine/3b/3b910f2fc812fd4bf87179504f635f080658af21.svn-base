    ...
    <template v-slot:body>
      ...
      <view class="exam-record">
        ...
        <view class="exam-mark-box flex-c-c flex-col">
          ...
          <view class="exam-tips">当前得分<text v-if="info.passMark && info.passMark != 0">（{{info.currentStatus==1?'已':'未'}}达标）</text></view>
        </view>
        <view class="exam-msg-box C_7f">
          <view class="exam-score p-b-20">
            <span>测验总分</span><span class="p-l-20">{{info.totalMark||0}}分</span><span v-if="info.passMark && info.passMark != 0">（达标分数{{info.passMark||0}}）</span>
          </view>
...
.exam-record{
  ...
  .record-item-btn{
    background-color: rgba($color: $uni-main-color, $alpha: 0.1);
    color: $uni-main-color;
    border-radius: 8rpx;
    width: 150rpx;
    height: 66rpx;
