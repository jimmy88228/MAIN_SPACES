            ...
            <view class="progress-box flex-s-c" :style="'height:' + navHeight + 'px;'">
              ...
              <view class="progress">
                <progress :percent="getPercent(isFinish?answerCount:answerCount + 1, answerCount + restQuestions)"
                  active active-mode="forwards" :duration="10" stroke-width="6" border-radius="8"
...
<script>
  ...
  const pageOption = Page.BasePage({
    ...
    methods: {
      init() {
        ...
        this.loadData(1, lastPageSize).then(() => {
          if (!this.acInfo?.lastQuestion?.nextQuestionId && this.acInfo?.lastQuestion?.questionId) {
            this.searchTargetQuestion(this.acInfo?.lastQuestion?.questionId, this.acInfo?.lastQuestion?.fromQuestionId)
            this.isFinish = true;
            this.setSubmitBtn()
          } else {
      ...
      getActInfo() {
        return this.$Http(this.$Apis.getLectureExamModel, {
          data: {
      ...
      loadData(pageIndex = 0, pageSize = 0) {
        ...
        return this.$Http(this.$Apis.getLectureExamQuestionList, {
          data: {
      ...
      submit(index) {
        ...
        return this.$Http(this.$Apis.submitExamLecture, {
          data: {
      ...
      submitFinish() {
        ...
        return this.$Http(this.$Apis.finishExamLecture, {
