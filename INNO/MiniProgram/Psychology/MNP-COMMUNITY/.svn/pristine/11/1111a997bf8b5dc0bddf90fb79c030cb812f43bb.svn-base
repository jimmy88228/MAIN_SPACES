...
<script>
  ...
  const pageOption = Page.BasePage({
    ...
    methods: {
      loadData(){
        return this.$Http(this.$Apis.getLectureCourseExamRecordInfo, {
          data: {
      ...
      reExam(){
        ...
        this.jumpAction(`/pages/activities/exam-lecture/answer/answer?activityId=${this.options.activityId||0}&contentId=${this.options.contentId||0}&courseId=${this.options.courseId||0}&modelId=${this.options.modelId||0}&comeFrom=record`)
      },
      check(e){
        this.jumpAction(`/pages/activities/exam-lecture/answer/check-scores?activityId=${this.options.activityId||0}&examRecordId=${e.examRecordId||0}`)
      },
    },
    onLoad(options){
