<template>  
    <!-- uni-app原生 -->
    <picker :mode="mode" :range-key="rangeKey" @columnchange="columnchange" @cancel="cancel" @change="pickerChange" :value="picker_value" :range="range">
        <!-- 自定义内容 -->
        <view>
...
const pageOption = Page.BaseComp({
  ...
  methods: {
    ...
    cancel(e){
      let picker_value = this.picker_value;
      this.picker_value = "";
      this.$nextTick(()=>{
