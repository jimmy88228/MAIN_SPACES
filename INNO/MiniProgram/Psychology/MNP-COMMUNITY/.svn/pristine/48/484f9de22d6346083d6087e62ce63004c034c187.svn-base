    ...
    <template v-else>
      ...
      <template v-if="showCourse">
        <view class="bg-top">
          <view class="course-title">
            <view class="font-32 C_fff bold m-b-25">{{formatDetail.courseName||''}}</view>
      ...
      <template v-else>
        <view class="has-no-data flex-c-c flex-col">
          ...
          <view class="font-28 C_7f">访问的讲座已结束</view>
  ...
  const pageOption = Page.BasePage({
    ...
    computed: {
      ...
      showCourse() {
        ...
        let showCourse = true
        if (courseDetail.id) {
          if (courseDetail.joinState == 2 || courseDetail.joinState == 3) {
            showCourse = false
          }
        } else {
          showCourse = false
    ...
    methods: {
      ...
      clickItem(e) {
        console.log(e, "点击事件")
        if (this.courseDetail.joinState == 0) {
          SMH.showToast({
            title: "该课程活动尚未开始"
          })
          return
        ...
        if (clickItem.contentType == 'audio') {
          ...
          this.jumpAction(
            `/pages/course-lecture/course-audio/course-audio?id=${clickItem.relateId}&courseId=${courseId}`
          )

        } else if (clickItem.contentType == 'video') {
          this.jumpAction(
            `/pages/course-lecture/course-video/course-video?activityId=${activityId}&courseId=${courseId}&contentId=${contentId}&cIndex=${e.cIndex}&sIndex=${e.sIndex}&chaptersIndex=${e.chaptersIndex}`
          )
        } else if (clickItem.contentType == 'exam') {
          if (!clickItem.isFinish) {
            this.jumpAction(
              `/pages/activities/exam-lecture/answer/answer?activityId=${activityId}&contentId=${contentId}&courseId=${courseId}&modelId=${clickItem.relateId}`
            )
          } else {
            this.jumpAction(
              `/pages/activities/exam-lecture/answer/exam-record?activityId=${activityId||0}&contentId=${contentId||0}&courseId=${courseId||0}&modelId=${clickItem.relateId||0}`
            )
          }
        }
      }
