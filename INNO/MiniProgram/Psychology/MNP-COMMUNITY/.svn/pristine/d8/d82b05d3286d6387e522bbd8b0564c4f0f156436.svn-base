<template>
  <view class="eval-item bg_fff">
      ...
      <view class="class-info flex-b-c">
        ...
        <view class="class-info-detail flex1">
          ...
          <template v-if="dataInfo.joinType == 1 || dataInfo.limitTime == 0">
            <view class="learning-count-progress m-b-10" v-if="dataInfo.hadLearnCount>0">
              <view class="learning-count-value" :style="{width:dataInfo.learningPrecent+'%'}"></view>
            </view>
            <view class="learning-count font-20"  v-if="dataInfo.hadLearnCount>0">已学习{{dataInfo.hadLearnCount}}/{{dataInfo.contentCount}}</view>
            <view class="C_B2 font-20"  v-else>未学习</view>
          </template>
    ...
    <view v-if="dataInfo.joinType != 2" class="eval-item-bottom flex-c-c C_7f font-26" @click="learnNow">
      进入课程
    </view>
  </view>
</template>
