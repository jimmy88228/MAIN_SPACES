          ...
          <view v-for="(item, index) in pItem" :key="item.activityId">
            ...
            <view class="item" @click="jump" :data-state="item.joinState" data-p-index="pIndex" :data-url="
              ...
              <view class="msg-box flex-s-s">
                <view class="relative">
                  <oriImage customStyle=" width: 220rpx; height: 165rpx; border-radius: 10rpx;" :src="item.coverPic ? item.coverPic : staticAddress+activityIcon"
                    @error="imgerror($event, index, pIndex)" class="f-shrink-0" mode="aspectFill" />
                </view>
