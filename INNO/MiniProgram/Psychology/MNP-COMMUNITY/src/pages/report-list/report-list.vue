      ...
      <view v-if="!isEmpty">
        ...
        <view class="report-list-area relative p-l-15 p-r-15 p-b-32">
          ...
          <view v-for="(listItem, itemIndex) in list" :key="itemIndex">
            <view class="report-item animate-fade-in-right" :style="{'animation-delay':`${item.showTime}s`}"
              v-for="(item, i) in listItem" :key="i" @click="jump" :data-p-index="itemIndex"
              ...
              <view class="report-item-info flex-b-s">
                <view class="relative">
                    <oriImage customStyle=" width: 119rpx;height: 119rpx;border-radius: 16rpx;" :src="item.logo ? item.logo : staticAddress+activityIcon"
                    @error="imgerror($event, i, itemIndex)" mode="aspectFill"></oriImage>
