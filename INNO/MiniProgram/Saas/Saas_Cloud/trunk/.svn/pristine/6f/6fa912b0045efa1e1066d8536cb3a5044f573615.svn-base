  ...
  methods: {
          ...
          switch (this.data.loginType){
            ...
            default:
              ...
              if (this.data.isLogin) {//已经登录
                ...
                .then(agreementPopShow => agreementPopShow ? Promise.reject("已调起会员协议弹窗，跳出") : Promise.resolve()) // 注册协议流程
                .then(checkIsLogin.bind(this)) // 检查是否登录和注册流程
    ...
    bindPhone(bindResult,data){
      ...
      let timer = setTimeout(() => {
          ...
          this.loginPhone.checkLogin({},"noNeed",this.data.showMsgBool,this.afterChangeStoreData);
          clearTimeout(timer);
        },300)
      } else{//已经绑过手机
...
function checkIfNeedConfirmAgreement(){ // 返回Promise 带true说明会拉起"会员协议"弹窗, 带false或reject则不会拉起
  try {
    ...
    if (this.ignoreAgreement){ // 不需要走会员协议流程
