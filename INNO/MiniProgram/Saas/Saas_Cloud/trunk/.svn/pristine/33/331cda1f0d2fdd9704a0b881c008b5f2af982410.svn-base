...
function initData() {
    app.CL_StoreCommApi.getStoreCommConfig({
        params: {
        ...
        if (res.code == 1) {
            const data = res.data || {};
            this.mustbe_integer = data.cashoutMustbeInteger || 0;
            remark = data.llimitCashoutRemark||"";
            if(parseFloat(data.maxCashout) > 0 || parseFloat(data.maxCashout) == 0){
              this.max = data.maxCashout
            }else{
              this.max = MAX;
            }
            if(parseFloat(data.minCashout) > 0 || parseFloat(data.minCashout) == 0){
              this.min = data.minCashout;
            }else{
              this.min = MIN;
            }
        } else {
