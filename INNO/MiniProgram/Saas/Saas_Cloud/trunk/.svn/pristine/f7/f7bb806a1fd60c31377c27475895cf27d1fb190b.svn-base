...
Page(app.BP({
    ...
    jump(e) {
        ...
        if (jumpNum == 0) {
            wx.navigateTo({
                url: '/pages/micro_mall/employee_center/distribution_brokerage/distribution_details/distribution_details',
            })
        } else if (jumpNum == 1) {
            // let storage = app.StorageH.get(STORAGE_USER_INFOS_KEY) || {};
            // if(storage.needVerify == 1){
            //     let params = {
            //         func_type:"STORE_FILL_IDCARD",
            //         fromRoute:"brokerage",
            ...
            // }else{
            //     wx.navigateTo({
            //         url: `/pages/micro_mall/employee_center/distribution_brokerage/distribution_take_out/distribution_take_out?balance=${this.data.dataDetail.account_balance || 0}`,
            //     })
            // }
            wx.navigateTo({
                url: `/pages/micro_mall/employee_center/distribution_brokerage/distribution_take_out/distribution_take_out?balance=${this.data.dataDetail.account_balance || 0}`,
            ...
            wx.navigateTo({
                url: `/pages/micro_mall/employee_center/distribution_orders_lists/distribution_orders_lists?currentIndex=${0}&type=record`,
            })
        }
    },
}))
...
function loadData() {
    app.CL_StoreCommApi.getStaffInfo({
        params: {
          "brandCode": app.Conf.BRAND_CODE,
          "userToken": app.LM.userToken || "",
          "manageStoreId": app.LM.storeInfo.manageStoreId || app.StoreH.storeId
