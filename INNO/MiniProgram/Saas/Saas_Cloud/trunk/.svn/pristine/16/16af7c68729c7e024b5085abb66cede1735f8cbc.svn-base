...
Page(app.BP({
  ...
  onLoad: function (options) {
    ...
    this.firstTimePayByCode = options.firstTimePayByCode;
    this.setData({
  ...
  onShow: function () {
    preApiHandler.call(this).then(res=>{
      getOrderInfo.call(this, this.orderId);
    })
  ...
  toComment: function () {
    ...
    wx.navigateTo({
      url: '../comment/mobile_order_comment/mobile_order_comment?order_id=' + orderId,
    })
  },
    ...
    wx.showModal({
      ...
      success: function (info) {
        if (info.confirm) {
          ...
          return app.CL_BuyApi.receiveOrderGoods({
            data: {
              orderId: orderId,
            },
            other: {
  ...
  toPayReq() {
    return PayH.UnifiedorderByOrderId("order",this.orderId).then(e => {
  ...
  toPayByQrCode(){
    return PayH.unifiedCloudShopOrderByCode("order", this.orderId)
      .then(code_url => {
        this.payQrCode = this.payQrCode || this.selectComponent("#payQrCode");
  ...
  jump_order: function (e) {
    ...
    wx.navigateTo({
      url: '/pages/micro_mall/shipping_info/shipping_info?invoiceNo=' + invoiceNo + '&orderId=' + this.orderId,
    })
  },
  ...
  handle_br(e) {
    ...
    console.log('show',this.data.show_br,this.data.barcodeCard)
  ...
  onTap(e){ 
    ...
    */
    ...
    if (type =="modify_address"){
      ...
      getOrderInfo.call(this, this.orderId).then(res=>{
        if(res.code=='1'){
          let data = res.data||{};
      ...
      wx.navigateTo({
        url: '/pages/micro_mall/shipping_info/shipping_info' + '?orderId=' + this.orderId,
      })
      ...
      for(let i in b_s_data){ //查找当前选中的商品
        ...
        if(b_s_data[i] && goodsList[i].canReturn && goodsList[i].returnNumber != goodsList[i].sendNumber){
          bool = true; 
          groupIndex = groupIndex?groupIndex + ',' + goodsList[i].groupIndex : goodsList[i].groupIndex ; 
        }
      }
      ...
      let url = `/pages/micro_mall/order/apply_return?order_id=${order_info.orderId}&groupIndex=${groupIndex}&valet=${order_info.is_valet_order}&is_user=${order_info.is_order_user}`;
      ...
      if(item.canReturn && item.returnNumber != item.sendNumber){
      ...
      wx.navigateTo({
        url: `/pages/micro_mall/invoice/issue_manager/issue_manager?type=invoice&order_type=${type}&sn=${order_info.orderSn||""}&price=${price}`,
      })
    }else if(type=='detail_invoice'){
      let order_info = this.data.order_info||{};
      wx.navigateTo({
        url: `/pages/micro_mall/invoice/invoice_detail/detail?id=${this.data.electricEntity&&this.data.electricEntity.taskId||0}&sn=${order_info.orderSn||""}&price=${order_info.order_amount}`,
      })
...
function getOrderInfo() {
  ...
  if (!orderId) {
    ...
    console.log(this.orderId,'---',this.options)
    return
  };
...
function getOrderPayStatus() {
  return app.CL_BuyApi.checkOrderPay({
    ...
    if (e.code == "1") {
      if (e.data == 1) {
        getOrderInfo.call(this);
        this.payQrCode && this.payQrCode.dismiss && this.payQrCode.dismiss()
      } else if (e.data == 0) {
...
function extendReceiveConfirm(){
  ...
  if (!order_info.orderId) return;
  if (menuInfo.extendReceiptDay > 0){
    let that = this;
    this.pageDialog = this.pageDialog || this.selectComponent("#pageDialog");
    this.pageDialog.setTitle("");
    this.pageDialog.setTouchCancel(false);
    this.pageDialog.setCentent("确认延长" + menuInfo.extendReceiptDay + "天收货时间？");
    this.pageDialog.setTwoBtn(
      {
        name: "取消",
        tap: function () {
      ...
      },
      {
        name: "确定",
        tap: function () {
          extendOrderReceiptTimeReq.call(that, order_info.orderId);
          that.pageDialog.dismiss();
        }
      }
    ).show();
...
function extendOrderReceiptTimeReq(id){
  if(!id) return;
  return app.CL_BuyApi.extendOrderReceiptTime({
    data:{
      orderId: id
    },
    other:{
...
function updateBtnContainer(){
  ...
  let container = [
    ...
    },
    {
      ...
      "tap":'cancelOrder',
      "name":"取消订单",
      "status": menuInfo.canCancel == 1 && menuInfo.isApplyCancel != 1//this.data.showCancelAll && !this.data.sub_order && (order_info.cancleActionStatus==1 && (this.data.close_cancel_btn != 1 || order_info.paystatus_Id == "0")) && order_info.order_type != 8
    },
    {
      "key":'cancel_apply',
      "tap":'cancelOrder',
    ...
    },
    {
      ...
      "tap":'toPayByQrCode',
      "name":"代付码",
      "status": (menuInfo.canQrcodePay) && order_info.orderStatus == '待付款'
    },
    // {
    //   "key":'pay_for',
    //   "tap":'',
