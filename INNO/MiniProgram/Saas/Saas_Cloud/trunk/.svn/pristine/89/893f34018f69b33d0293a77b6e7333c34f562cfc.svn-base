...
const conf = {
   ...
   */
  renderCalendar(curYear, curMonth, curDate) { //计算上下月,计算当前月
  ...
  calculateNextMonthGrids(year, month) {
    ...
    if (+lastDayWeek !== 6) {
      ...
      for (let i = 1; i <= len; i++) {
        lastEmptyGrids.push({
          i,
          isCanGetGift: (signData.signActivityRuleGiftList && signData.signActivityRuleGiftList.length && signData.activity_id) && i == signData.nextMonthGetGiftDay, 
        });
      }
      setData({
  ...
  calculateDays(year, month, curDate) {
    ...
    let next = curDay + (signData.sign_status == 1 ? signData.needSignDay - 1 : signData.needSignDay);
    for (let i = 1; i <= thisMonthDays; i++) {
      let week = conf.getDayOfWeek(year, month, i);
      days.push({
        year,
        month,
        day: i,
        choosed: false,
        week,
        showName: GetDayOfWeekName[week],
        isToday:year == curYear && month == curMonth && i == curDay,
        isCanGetGift: (signData.signActivityRuleGiftList && signData.signActivityRuleGiftList.length && signData.activity_id) && year == curYear && month == curMonth && i == signData.nextGetGiftDay,
      });
    }
...
export const getThisMonthDays = (year,month) => {
  return conf.getThisMonthDays(year,month);
};
...
function checkCanGetGift(i){
  // return i + (signData.sign_status == 1 ? signData.needSignDay - 1 : signData.needSignDay) + 
}
