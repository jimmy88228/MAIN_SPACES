...
Page(app.BP({
    ...
    viewTips(e) {
        wx.navigateTo({
            url: `/pages/micro_mall/employee_center/apply_for_staff/staff_tip/dure?agreement=${this.dure_agreement}`,
        })
    },
...
function windowShowHide(type) {
    if (type != 'jump') {
        ...
        type == 'success' && (refreshStaff.call(this));
    }
    this.setData({
...
function toPay(order_id) {
    var that = this;
    if (order_id) {
        return PayH.UnifiedorderByOrderId({type:"staff",order_id})
        .then(e => {
...
function refreshStaff(){ //刷新staffInfo
