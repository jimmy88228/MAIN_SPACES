...
function checkChannel(ops_params = {}){
  ...
  if (SceneChannel[scene] || scene) {
    ...
    // console.log('进来 场景值属于渠道场景值',channelType)
    createLogSession = true;
    createLogSessionExtend = true;
  } 
  //3--是否与上次进入的scene场景值一致
  if (!this.lastScene || ((this.lastScene != scene) && scene)) {
    // console.log('进来 与上次进入的scene场景值不一致',scene,this.lastScene)
    createLogSession = true;
    createLogSessionExtend = true;
  }
  ...
  //4--是否分享进入
  if (query.fromUser) {
    console.log('进来 分享进入',query.fromUser)
    createLogSession = true;
    createLogSessionExtend = true;
  }
  ...
  // console.log('进来 检测基本的信息',createLogSession,createLogSessionExtend,{channelType,channel},this.channel);
  ...
  let p = new Promise((rs,rj)=>{
    // console.log('进来 检测channelType',channelType);
    if (channelType) {
      if (channelType == 'CUSTOM' || query.channelType) { //扫码进入
        // console.log('进来 扫码进入',channelType,query.channelType);
        createLogSessionExtend = true; 
        ...
        SHP.getParams(["options"]).then(params => {
            ...
            query = ph.query;
            createLogSessionExtend = true;
            // console.log('进来 扫码进入参数获取',ph,query);
            return rs();
        })  
      }
      else if (channelType == 'GROUP_SHARE') { //群分享
        // console.log('进来 群分享',channelType);
        WxGH.getShareTicket(ops_params.shareTicket)
          .then(res => {
            // console.log('群分享结果', res);
            if(res){
              channel = res;
              createLogSessionExtend = true;
            }else{ // 这里是做兼容，如果实在拿不到群id，最后还是要把 渠道类型 和 渠道值 带过去
              channel = scene;
              createLogSessionExtend = true;
            }
            return rs();
          })
          .catch(err => { // 这里是做兼容，如果实在拿不到群id，最后还是要把 渠道类型 和 渠道值 带过去
            console.log("WxGH.getShareTicket err ->", err);
            channel = scene;
            createLogSessionExtend = true;
            return rs();
          })
        ...
        // console.log('进来 公众号',channelType,channel);
        if (channel){
          createLogSessionExtend = true;
        }else{
          channelType = "";
...
class LogManager {
  ...
  setChannel(ops_params) {
    // console.log('进来 setChannel',ops_params.scene,this.lastScene,ops_params);
    if (!ops_params.path){
      let page = getCurrentPages().pop();
    ...
    createChannel.call(this, ops_params || {}).then(res=>{
      ...
      //   this.createSession();
      // }
      // console.log('进来 createChannel then',res);
