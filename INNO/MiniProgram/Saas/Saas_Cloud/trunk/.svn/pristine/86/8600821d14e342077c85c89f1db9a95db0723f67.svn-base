...
Page(app.BP({
    ...
    onLoad: function(options) {
        ...
        this.authorizeData();
        checkStorePhoneNumberVisibility.call(this)
        //
    ...
    getAddrMap(e) {
        ...
        let storeItem = store_data[store_sub] || {};
        let addr = storeItem.address || '';
        let city = storeItem.area_name || '';
        let distinct = storeItem.distinct || '';
        let lat = parseFloat(storeItem.lat) || 0;
        let lon = parseFloat(storeItem.lon) || 0;
        let phone = storeItem.phone || '';
        let store_id = storeItem.id || '';
        let store_name = storeItem.name || '';
        let latlon = lat + "," + lon;
        let place = lat + "," + lon;
        ...
        if (clickType == "selected") { //查看门店
          ...
          storeItem.key_word = keyword;
          app.StorageH.set("select_store", storeItem);
          app.SMH.showToast({
            "title":"选择成功"
          })
          ...
          let openTimeDesc = storeItem.openTimeDesc || "";
          let enableSelfGet = storeItem.enableSelfGet || 0;
...
function checkStorePhoneNumberVisibility(){ // 读配置，店铺联系电话是否显示
  return app.sysTemConfig("applet_side_view_store_phone_number").then(res => {
    this.setData({
      storePhoneShow: res.Value && res.Value == 1 || false
    })
    return Promise.resolve(res)
  })
}
