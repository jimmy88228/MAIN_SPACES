  ...
  checkLoctionFn() {
    ...
    let p = new Promise(function(rs, rj) {
      if (that._loc_obj && that._loc_obj.checked) { //检测过 
        if (route && StoresForA[route] && StoresForA[route]['check']) { 
      ...
      // let callBack = (res) => {
      //   if (route && StoresForA[route] && StoresForA[route]['check']) {
      //     return that.checkStoresForA(that, rs, rj, route);
      //   }
      //   return rs(that._loc_obj);
      // };
      // that.authorizeId = setTimeout(() => {
      //   clearTimeout(that.authorizeId);
      //   AS.checkAuthorize(Check_Name, callBack, callBack, true); //授权封装函数  已同步更新setStatusObj
      // }, delayTime);
    });
    return p;
  }
