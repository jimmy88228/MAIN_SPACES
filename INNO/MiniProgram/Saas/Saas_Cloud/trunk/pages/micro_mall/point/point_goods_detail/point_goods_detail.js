...
Page(app.BP({
    ...
    onShow: function() {
        ...
        initShippingInfo.call(this, this.data.select_goods);
...
function onShowEvent(ops){
    if (app.StoreH.storeId){ // 解决第一次进入小程序是这个页面，没有拿到商店id时会弹窗报错的问题，可去掉这个判断
        getPointMkGoodsDetail.call(this, ops).then(res=>{
            ...
            return this.commission.getCommission("POINTMKT", ops.goodsId, ops.mkGoodsId, true);
          })
          listen.call(this);
    }
}
...
function getPointMkGoodsDetail(options = {}) {
    ...
    return app.CL_PointApi.getPointMkGoodsDetail({
            params: {
...
function checkBuyPointMkGoods() {
    let select_goods = this.data.select_goods||{};
    return app.CL_PointApi.checkBuyPointMkGoods({
        params: {
...
function pointMkGoodsCheckout(select_goods) {
    ...
    if (!this.isLoading) {
        ...
        return app.CL_PointApi.pointMkGoodsCheckout({
            data: {
                mkGoodsId: this.data.baseInfo.mk_goods_id,
                goodsId: this.data.baseInfo.goods_id,
                productId: select_goods.productInfo.product_id,
