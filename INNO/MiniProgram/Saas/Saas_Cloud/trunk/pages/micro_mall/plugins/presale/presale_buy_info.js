...
Page(app.BP({
    ...
    createOrder: function() {
        ...
        app.CL_PreSaleApi.presaleDepositAddOrder({
            data: reqData,
            other: {
                isShowLoad: true
            }
    ...
    handlePay(order_info){
        ...
        return PayH.UnifiedorderByOrderId(pay_type,order_id)
        .then(e => {
          if (e.code == "1") {
            let pay_info = e.data;
            //微信支付
            WxApi.requestPayment({
              ...
              setLock.call(this);
              if(currentCoupon.use_coupon && currentCoupon.use_coupon.length>0){
                initCoupon.call(this,'init');
              }
              that.loading = false;
            }).catch(e => {
...
function apiFn(reqData, check = false) {
    if (!check) {
        if (this.req_api_name == 'getPresaleOrderData' || this.req_api_name == 'getPresaleDepositSettlement') {
            return app.CL_PreSaleApi[this.req_api_name]({
                params: reqData,
                other: {
                    isShowLoad: true
                }
        ...
        reqData.bonusId = 0;
        return app.CL_PreSaleApi.presaleOrderTailUsePointCoupon({
            data: reqData,
            other: {
                isShowLoad: true
            }
...
function logThePresaleDetail(){
  ...
  return app.CL_PreSaleApi.logThePresaleDetail({
    data:{
...
function getPayStatus(type=""){
    return app.CL_PreSaleApi.getPreSalePayStatus({
        params:{
