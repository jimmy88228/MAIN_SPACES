    ...
    properties: {
      allData: {
        ...
        observer(n, o) {
          ...
          if (opKind != this.temOpKind || opKind == 'STAFF_GOODS' || opKind == 'STAFF_ACTIVITY' || opKind == 'STORE_STAFF_GOODS' || opKind == 'STORE_STAFF_ACTIVITY') {
            this.setData({
              QRCode: "",
              tempQR: {},
            })
    ...
    methods: {
      ...
      canvasArrEvent(code, userHead) {
        ...
        if (!imgUrl) {
          ...
          if (info.opKind == "STAFF_ACTIVITY" || info.opKind == "STAFF_GOODS" || info.opKind == 'STORE_STAFF_ACTIVITY' || info.opKind == 'STORE_STAFF_GOODS') {
        ...
        if (goodsCanvas && (info.opKind == "STAFF_ACTIVITY" || info.opKind == 'STORE_STAFF_ACTIVITY')) {
        ...
        initTemplate.call(this, allData).then(tempArr => {
          if (info.opKind != "STAFF_ACTIVITY" || info.opKind != "STORE_STAFF_ACTIVITY" || !(draw.template == "custom" && info.gen_barcode_style == 0 && this.saveActive != 0)) {
            drawArr = tempArr || [];
            drawArr = tempArr || [];
...
function codeInitDraw(ctx, baseW, res, baseInfo, canvasId) {
  ...
  if (!baseInfo.codeDiy && info.gen_barcode_style !=2 && ((info.opKind != "STAFF_ACTIVITY" || info.opKind != 'STORE_STAFF_ACTIVITY') || (info.opKind == "STAFF_ACTIVITY" || info.opKind == 'STORE_STAFF_ACTIVITY') && (this.saveActive == 0 || info.gen_barcode_style == 1))) {
...
function checkInfo(data = {}) {
  ...
  if (this.storeInfo.staff_code) {
    data.scene = data.scene || {};
    if (this.storeInfo.staff_code != data.scene.storeStaffCode) {
      data.scene.storeStaffCode = this.storeInfo.staff_code
    }
  }
...
function initTemplate(allData = {}) {
  ...
  } else if(draw.template == 'activity'){
    return checkShareConf.call(this, "activity").then(check_res => {
      allData.info = allData.info || {};
      allData.info.ShareConf = this.ShareConf || {};
      return DrawTemplate.initActivityTemplate(allData).then(res => {
        return Promise.resolve(res || []);
      })
    })
  } else {
...
function triggerActionLog(data = {}, type) {
  ...
  switch (opKind) {
    ...
    case "STAFF_ACTIVITY":
      ...
      break;
      case "STORE_STAFF_GOODS":
      case "STORE_STAFF_ACTIVITY":
        position = "STORE_STAFF_GOODS";
        // staff_page_id = scene.page_id || (info.customJump == 1 && goodsInfo.page_id) || 0;
        if (type == "show") {
          name = "GOODS_POSTER"
          if (info.tabCurrent == 1) {
            name = "ACTIVITY_POSTER"
            position = "STORE_STAFF_ACTIVITY";
          }
        } else if (type == "save") {
          name = "GOODS_POSTER_SAVE"
          if (info.tabCurrent == 1) {
