  ...
  app.BTAB({
    ...
    methods:{
      ...
      scrolltolowerCallback(e){
        ...
        // if(coupon_list[current].hasMore){
        //   limitLoad.call(this);
        // }else{
        //   app.SMH.showToast({
        //     isNoData: true
      ...
      onShowUseLimit: function(e) {
        ...
        console.log("activieBonusId",activieBonusId)
    },
    ...
    beforeUseCoupon(e){
      ...
      let that = this;
      if (selectData.bonusType == 4 && this.idsArr.length > 0 && !this.data.choiceIds[selectData.bonus_id]) {
    ...
    useCoupon: function(e) {
        ...
        couponOption.bonusIds = this.idsArr;
...
function loadCoupon(params){
  couponByJieSuan.call(this,params).then(res=>{
    let data = res.data || {};
...
function couponByJieSuan(couponOption){
  ...
  return app.CL_BuyApi.getCheckoutBonusList({
    data:{
...
function installData(data,type){
  ...
  for (let i in couponList) {
    ...
    let fromTime = couponList[i].useStartDate && couponList[i].useStartDate.replace(/\-/g, "/");
    let toTime = couponList[i].useEndDate && couponList[i].useEndDate.replace(/\-/g, "/");
  ...
  selectInfo.realityMoney = data.totalAmountAfter || 0.00;
  selectInfo.totalMoney = data.totalAmountBefore || 0.00;
