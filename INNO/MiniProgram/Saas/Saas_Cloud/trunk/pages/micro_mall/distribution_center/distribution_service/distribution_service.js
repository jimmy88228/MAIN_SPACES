...
Component(app.BTAB({
  ...
  methods: {
    jump(e) {
      ...
      if (url) {
        if (url == "share" || url == "gree_hand") {
          getStaffInfo.call(this).then(res => {
            let page_id = url == "gree_hand" ? this.data.staff_cfgInfo && this.data.staff_cfgInfo.cloup_shop_guide_page : res.cloup_shop_page_id;
            wx.navigateTo({
