...
const PAGE_TYPE = "BONUS_ACTIVITY";
const app = getApp();
Page(app.BP({
  data: {
    activity_info: {},
  ...
  onLoad: function(options) {
    ...
    let detail_share = this.data.brand_info.icon_url + "micro_mall/detail_share.png";
    this.setData({
      g_home: g_home,
      detail_share
    }) 
  ...
  confirmBuy() {
    ...
    let params = {
      activityId,
      staffId: app.PH && (typeof app.PH.paramsJson === "function") && app.PH.paramsJson("storeStaffCode", true) && app.PH.paramsJson("store_staff_id") || 0, // 店员
    }
    if (!this.order_id) {
      app.RunApi.go('post', 'ActApi', 'createBuyBonusOrder', params).then(res => {
        ...
        if (res.code == 1) {
          ...
          if (order_id) {
            ...
            wxSubscribe.subscribeGlobal({...this.subConfig, relatedId: order_id}).finally(() => goPay.call(this, order_id))
