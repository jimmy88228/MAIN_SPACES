...
Page(app.BP({
  data: {
    ...
    bonus_sales_tabs:[],
    brok_tabs:[],
    manageStoreList: [], // 店长可切换的店铺列表
    currentStore: {}, // 当前门店
  },
  storeInfo: {},
  onLoad: function(options) {
    let name = options && options.name || "店铺分享";
  ...
  handleManageStoreChange(e) {
    const index = e.detail.value;
    const manageStoreList = this.data.manageStoreList || [];
    const storeItem = manageStoreList[index];
    if (storeItem.id === this.data.currentStore.id) return 
    this.selectManageStore(storeItem, true)
  },
  selectManageStore(storeItem = {}, needReflesh = false) {
    if (storeItem.id){
      const manageStoreId = storeItem.id
      this.setData({currentStore: storeItem})
      this.storeInfo.manageStoreId = manageStoreId
      this.storeInfo.staff_id && app.LM.setStoreInfo(this.storeInfo)
      needReflesh && onShowEvent.call(this)
    }
  }
}))

function checkUserStaffType(){
  return app.LM.checkIfStore().then(data => {
    if (data && data.staff_id){ // 是店长
      this.storeInfo = data || {};
      if (data.staff_type == 1) return getStaffManageStore.call(this)
    }
...
function getStaffManageStore(){
  if (this.storeInfo && this.data.manageStoreList && this.data.manageStoreList.length) return Promise.resolve({list: this.data.manageStoreList})
  return app.CL_StoreCommApi.getStaffManageStore({
    params: {
    ...
    if (res.code == "1" && res.data && res.data.list){
      ...
      this.setData({manageStoreList: list})
      list.length && this.selectManageStore(list[0])
      return Promise.resolve(res.data)
    }
    return Promise.reject(res)
  }).catch(err => {
    console.log("获取店长管理的店铺列表失败", err)
    return Promise.reject(err)
  })
}

function loadData() {
  return app.CL_StoreCommApi.getStaffInfo({
    params: {
      "brandCode": app.Conf.BRAND_CODE,
      "userToken": app.LM.userToken || "",
      "manageStoreId": this.storeInfo.manageStoreId || app.StoreH.storeId
...
function end() {
  ...
  if (this.bonusSales) {
    this.bonusSales && this.bonusSales.end();
  }
  let _timer = setTimeout(() => {
    ...
    this.setData({
      ...
      bonusSalesOnhide: this.bonusSalesOnhide
    })
  }, 200)
}
...
function getMyStaffInfo() {
  if (this.myStaffInfoRes) return Promise.resolve(this.myStaffInfoRes) // 只需要请求一次，切换店铺不需要重复请求
  let params = {
    brandCode: app.Conf.BRAND_CODE,
  }
  let extra = {
  ...
  return app.RunApi.go('CL_StoreCommApi', 'getMyStaffInfo', params, extra).then(res => {
    if (res.code == '1') {
      let data = res.data || {};
...
function checkShow() {
  let p = new Promise((rs, rj) => {
    if(!this.pr_all){
      let p1 = getMyStaffInfo.call(this);
  ...
  return p.then(res => {
    ...
    let bonus_sales_val = [dt.buybonus_total_sales, dt.buybonus_month_sales, dt.buybonus_week_sales]
    let sales_tabs = [{nm:"累计销售"},{nm:"月销售"},{nm:"周销售"},/*{nm:"邀请奖励",key:"invite"}*/];
    let bonus_sales_tabs = [{nm:"累计销售"},{nm:"月销售"},{nm:"周销售"}];
    this.salesOnhide = ['0.00','0.00','0.00','0.00'];
    this.bonusSalesOnhide = ['0.00','0.00','0.00','0.00'];
    ...
    if (data_1.can_visit_commission_duty == 1) {
      ...
      if(this.commission_mod && this.commission_mod.isOpen != '0'){
        commission_mod_show = true;
        broker_val = [dt.total_income, dt.month_income, dt.week_income, dt.account_balance, dt.staff_income || 0];
        brok_tabs = [{nm:"累计收益"},{nm:"本月收益"},{nm:"本周收益"},{nm:"当前收益",key:"cash"}];
        dt.staff_type == 1 && brok_tabs.push({nm:"店员收益"}); // 店长增加显示"店员收益"模块
    ...
    return Promise.resolve([broker_val,sales_val,bonus_sales_val]);
  })
}
...
function bonusSalesInit(val){
  !this.init_T_S && (this.init_T_S = 1400) || (this.init_T_S != 400 && (this.init_T_S = 400));
  this.bonusSales = this.bonusSales || this.selectComponent('#bonus_sales') || '';
  if (this.bonusSales) {
    this.label_s2 = setTimeout(() => {
      this.bonusSales.init(val);
      clearTimeout(this.label_s2);
    }, this.init_T_S)
  }
}
