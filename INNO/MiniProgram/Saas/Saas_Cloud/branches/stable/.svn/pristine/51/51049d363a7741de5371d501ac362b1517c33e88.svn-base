const { LogMap } = require("../../../../common/manager/log-map");
...
function loadData() {
    if (!this.loading) {
        this.loading = true;
        return app.CL_BargainApi.getUserHagglePriceActivityList({
            params: {
                pageIndex: this.page,
...
function cancelOrder(that,id=0,dialog) {
    ...
    that.lock = true;
    return app.RunApi.go('POST','CL_BargainApi','cancelUserHagglePriceActivity',{userActivityId:id}).then(res=>{
        if(res.code == "1"){
            setTimeout(()=>{
                that.lock = false;
            },500)
