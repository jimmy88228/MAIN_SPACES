...
function apply_Staff(reqData) {
    if (!reqData.encryptedData || !reqData.iv || !app.LM.userKey) return Promise.reject("");
    // console.log('reqData',{
    //     "apply_name": this.data.value_name,
    ...
    // })
    // return Promise.reject();
    return app.CL_DistrApi.applyStaffDstbInfoNoPhone({
        data: {
          "apply_name": this.data.value_name,
          "encryptedData": reqData.encryptedData,
          "iv": reqData.iv,
            ...
            if (e.code == "1") {
                ...
                WxApi.requestPayment({
                    ...
                    createPolling.call(that, COUNT, DURATION, () => {
                        return app.CL_DistrApi.pay_Result({
                            params: {
                                // brandCode: app.Conf.BRAND_CODE,
                                orderId: order_id
                            },
