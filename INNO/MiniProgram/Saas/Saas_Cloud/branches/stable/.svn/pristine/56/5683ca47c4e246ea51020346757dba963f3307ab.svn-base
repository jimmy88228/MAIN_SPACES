// components/calendar/signContinuous/signContinuous.js
...
function initCirPoint(info){
  info = info || this.data.info||{};
  let detailList = info.detailList||[];
  let prizeLevel = detailList.length;
  let curLevel = info.currentSort; 
  let curProgress = info.currentContinueDay;
  let nextGetContinueInfo = info.nextGetContinueInfo||{};
  let allProgress = nextGetContinueInfo.continueDay;
  let cirArr = [];
  for(let i = 1,len=detailList.length;i<=len;i++){
    cirArr.push({
      i,
      rotate:180+(360*i/len),
      active: (i <= (curLevel - 1) || (i == curLevel && detailList[i-1].isReach && nextGetContinueInfo.nextGetContinueDay <= 0)) ? true:false //当前档的上一档高亮 || 最后一档满足条件
    })
  }
  this.setData({
    cirArr
