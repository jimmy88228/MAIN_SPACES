// pages/micro_mall/employee_center/distribution_brokerage/brokerage_content/identity_fill/identity_fill.js
...
function jump() {
  ...
  if (ops.fromRoute == 'brokerage') {
    wx.redirectTo({
      url: `/pages/micro_mall/employee_center/distribution_brokerage/distribution_take_out/distribution_take_out?balance=${ops.balance || 0}`,
    })
  } else if (ops.fromRoute == 'staff') {
    let result = this.result || {};
    let params = {
      func_type: "STORE_STAFF",
...
function setVerify() {
  ...
  app.StorageH.set(STORAGE_USER_INFOS_KEY, storage);
}
