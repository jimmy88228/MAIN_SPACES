    ...
    loadData.call(this).then(res=>{ 
      checkShow.call(this).then(res => {
        this.nextTick().then(()=>{
          ...
          salesStoreInit.call(this,res[2]);
        })
      });
    });
  },
...
function loadData() {
  return app.CL_DistrApi.staffDstbInfo({
    params: {
    ...
    return Promise.reject(res.msg||"数据异常");
  }).catch(e=>{
    app.SMH.showToast({title:e})
...
function checkShow() {
  ...
  return p.then(res => {
    ...
    let store_sales_val = [dt.store_yesterday_sales, dt.store_today_sales, dt.store_week_sales,dt.store_week_sales];
    let store_sales_tabs = [{nm:"昨日销售"},{nm:"今日销售"},{nm:"周销售"},{nm:"月销售"}];
    this.storeSalesOnhide = ['0.00','0.00','0.00','0.00'];
    ...
    let {is_show_income=1,is_sales_volume=1} = data_2;
    this.setData({
      commission_mod_show,
      commission,
      sales_tabs,
    ...
    return Promise.resolve([broker_val,sales_val,store_sales_val]);
  })
}
...
function salesStoreInit(val){ 
  if(!val || val.length<=0)return;
  !this.init_T_S_S && (this.init_T_S_S = 1400) || (this.init_T_S_S != 400 && (this.init_T_S_S = 400));
  this.sales_s = this.sales_s || this.selectComponent('#store_sales') || '';
  if (this.sales_s) {
    this.label_s_s = setTimeout(() => {
      this.sales_s.init(val);
      clearTimeout(this.label_s_s);
    }, this.init_T_S_S)
  }
}
