...
Page(app.BP({
  ...
  onShareAppMessage(e) {
    let title = String(this.activityInfo.shareTitle).trim() || "秒杀助力分享";
    ...
    if (e.target && e.target.dataset && e.target.dataset.route == 'index'){
      ...
      imageUrl = String(this.activityInfo.shareImg).trim() || '';
  ...
  shipSelect(e) {
    ...
    if (detail.index == 1) { // 自提
      let storeId = StoreH.storeId || 0;
      this.setData({
        selectShipInfo: { selectIndex: 1, id: storeId}
      })
    } else {
      this.setData({ 
    ...
    // let dataset = e.currentTarget.dataset || {};
    if (selectIndex != detail.index) {
      // getSumaryGoodsProductInfo.call(this, this.options);
      // getSku.call(this);
      getSku.call(this).then(setProductInfo.bind(this))
    }
  },
...
function loadData(update=false) {
  ...
  extra.diy = false;
  return app.RunApi.go('CL_SecKillApi', 'getGoodsDetail', params, extra).then(res => {
    if (res.code == 1) {
      let data = res.data || {};
      ...
      showPage.call(this, [p1, p2, p3,p4,p5,p6]);
  ...
  // .then(() => { //店员或分销收益
  //   const {activityId, goodsId} = this.options;
  //   this.commission = this.commission || this.selectComponent("#commission");
  //   return this.commission.getCommission("SKILL", goodsId, activityId, true);
  // })
...
function getSku(){
  ...
  return app.RunApi.go('CL_SecKillApi', 'getProducts', params, this.extra).then(res => {
    if (res.code == '1') {
...
function checkSkuSelect(){
  ...
  } else if(goodsExtend.selfGet == 2 && selectShipInfo.selectIndex != 2){
    name = "请选择配送方式" 
...
function checkPay(){
  ...
  return app.RunApi.go('CL_SecKillApi', 'checkPay', params, this.extra).then(res=>{
    if(res.code=='1'){
      return Promise.resolve(res);
    }
    return Promise.reject(res);
...
function checkCondition(_url ='checkCondition'){
  ...
  let check = false;
  return app.RunApi.go('CL_SecKillApi', url, params, this.extra).then(res => {
    if (res.code == '1') {
      this.setData({
        [`${url}`]:true
      })
...
function share() {
  ...
  return app.RunApi.go('post','CL_SecKillApi', 'addActivityLaunch', params, this.extra)
} 
