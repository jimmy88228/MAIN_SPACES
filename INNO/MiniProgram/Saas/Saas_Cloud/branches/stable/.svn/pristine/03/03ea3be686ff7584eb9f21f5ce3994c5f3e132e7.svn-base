...
function getSignInfo() {
  ...
  return app.CL_UserApi.getAppSign({
    params: {
...
function get_SignOrderActivityInfo() {
  ...
  return app.CL_UserApi.get_SignOrderActivityInfo({
    params: {
    ...
    if (e.code == 1) { 
      ...
      if(info){
        ...
        wx.nextTick(()=>{
          ...
          this.signContinuousId && this.signContinuousId.init(this.data.signOrderActivityInfo);
        })
      }
      return Promise.resolve(e);
    }
    e.msg && e.msg != "未找到相关签到活动信息" && app.SMH.showToast({
      "title": e.msg
    })
  }).finally(()=>{
    this.setData({
...
// 云店签到
function cloudCreateAppSign() {
  if (!this.loading) {
    this.loading = true;
    return app.CL_UserApi.createAppSign({
      data: {
        userToken: app.LM.userToken,
        brandCode: app.Conf.BRAND_CODE
      },
...
function getSignRecord(beginDate = initDate.call(this).firstDay, endDate = initDate.call(this).lastDay) {
  ...
  return app.CL_UserApi.getSignRecord({
    data: {
      userToken: app.LM.userKey,
      brandCode: app.Conf.BRAND_CODE,
      beginDate,
...
function getOrderActInfo(info){
  ...
  for(let i = 0,len=list.length;i<len;i++){
    ...
    if(item.isReach){
      ...
      (info.nextGetContinueInfo.nextGetContinueDay === undefined && item.continueDay) && (info.nextGetContinueInfo.nextGetContinueDay = item.continueDay)
      break;
    }
  }
  return info
...
function checkIfNeedAutoExpandCalendar() {
  ...
  signData.activity_id && signData.is_fold == 0 && (is_fold = 0);
  this.calendarId = this.calendarId || this.selectComponent('#calendarId');
