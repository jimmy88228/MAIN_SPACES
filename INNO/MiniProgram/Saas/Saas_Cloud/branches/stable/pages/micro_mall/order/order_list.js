...
const orderTypeObj = {
    "order_exchange":"refund_goods_tab",
    "order_swop":"swop_tab",
}
const Return_Type = {
    "refund_goods_tab":1,
...
Page(app.BP({
    ...
    setReqData: function() {
        ...
        switch (orderType) {
            ...
            case 'order_money':
                this.orderType = 4;
                orderTypeTit = "退换货";
                this.next_act = "order_money_info";
                break; 
            case 'order_swop':
                this.orderType = 4;
                orderTypeTit = "退换货";
    ...
    onTabClick: function(e) {
        ...
        console.log('clickTabId',this.data.clickTabId)
    ...
    onPageJump: function(e) {
        ...
        if(this.data.clickTabId == "store_tab"){
            ...
            if (this.next_act == "order_exchange_info") { 
                wx.navigateTo({
                    url: this.next_act + '?order_sn=' + order_sn + '&return_id=' + id + '&type=' + this.data.clickTabId
                })
            } else { //其他类型订单
                var link_order_sn = "";
                if (parent_order_sn) {
  ...
  onTap(e){
    ...
    if (type =="oneMoreOrder"){
      ...
      wx.navigateTo({
        url: `/pages/micro_mall/order/order_info?first_time_topay=${2}&order_id=${order_id}`
      })
...
function loadDataList(times=1,type) { 
    ...
    return app.CL_BuyApi[api.reqUrl]({
        params: {...params},
        other: {
            isShowLoad: true
        }
        ...
        if (e.code == "1") {
            ...
            for (let item in list) {
                let order_info = list[item]||{};
                let rec_time_format = order_info.recTime || "";
...
function setTabShow() {
    ...
    if((this.options.orderType == 'order_exchange_all' || this.options.orderType == 'order_exchange' || this.options.orderType == 'order_swop'|| this.options.orderType == 'order_money')){
        ...
        return Promise.all(arr).then(res=>{
            ...
            let clickTabId = "refund_goods_tab",isCanSwop = false,isCanMoney = false;
            res.forEach(item=>{
                if(item.Value == "1"){
            ...
            this.options.orderType && (this.options.orderType == 'order_swop') && isCanSwop && (clickTabId = 'swop_tab');
            this.options.orderType && (this.options.orderType == 'order_exchange') && (clickTabId = 'refund_goods_tab');
...
function initFn(){
    ...
    if (order_type == "order_exchange" || order_type == "order_exchange_all") {
        checkOrderText.call(this);
...
function getParams(times) {
    ...
    if(this.orderType != 4){
        ...
        params.returnType = Return_Type[id];
        api.reqUrl = "getReturnOrderList";
    }else if(this.orderType == 4  && id == 'refund_money_tab'){
        // api.reqUrl = "getOrderRefundList"
        api.reqUrl = "getRefundOrderList"
    } 
