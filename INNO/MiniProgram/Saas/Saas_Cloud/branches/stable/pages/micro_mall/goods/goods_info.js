...
Page(app.BP({
  ...
  onLoad: function (options) {
    ...
    let g_sale = bInfo.icon_url + "micro_mall/goods/g_sale.png";
  ...
  onShow() {
    ...
    if(this.onHided && this.countDown && this.data.showCountDown){
      setCountDown.call(this,this.data.goods_info||{});
    }
    if(this.needOnShow){
  ...
  assembleOptions(){ 
    this.checkLoginAsync()
      .then(()=>{
        if (this.options.q && this.options.scancode_time) {
      ...
      .catch(e => {
        console.log("验证登录状态失败，原因:", e);
        onShowEvent.call(this, this.options)
      })
  },
  handle_promote() {
    if(!this.checkPromote) return;
    this.promoteList.show();
  },
  onHide() {
    this.onHided = true;
    unListen.call(this);
  ...
  //   let store_id = this.data.storeId || 0;
  ...
  showStylePop: function (e) {
    console.log('showStylePop',e,checkIsCanPop.call(this))
    if(!checkIsCanPop.call(this))return
    let dataset = e.currentTarget && e.currentTarget.dataset || {};
    let {buy_type, product_id = 0, product_sn = 0} = e.detail || {};
    this.buy_type = buy_type;
    let click_type = dataset.click_type || "";
    getSumaryGoodsProductInfo.call(this, this.options, click_type).then(
      // 如果带有product_id或product_sn进来，则增加一步操作: 自动选择对应的规格
      (product_id || product_sn) && autoChooseProduct.bind(this, product_id, product_sn)
    );
  ...
  onTap(e){
    ...
    if (type == "attr"){
      ...
      let ruleFolderOpen = !this.data.ruleFolderOpen;
      this.setData({ ruleFolderOpen })
    }
  },
  longTap(e){
  ...
  _noFn(){},
  onSevenFish(){ 
    let {goods_info:data={},select_goods={}} = this.data||{};
    let productInfo = select_goods.productInfo||{}; 
    let isSelect = !!(productInfo.product_id);
    let color_item = this.data.color_img[select_goods.select_color_id || this.data.defaultColor]||{};
    let product = {
      cardType : 0,
      title: data.goods_name||"",
      desc:  getDesc(data,productInfo,isSelect),
      goodsId: isSelect ? productInfo.product_sn||"" : data.goods_sn||"",
      goodsCId:data.cat_id||0,
      goodsCName:data.cat_name||"",
      note: '售价：' + (isSelect ? `¥${productInfo.sale_price||0}`:`¥${data.min_price||0}`),
      picture:color_item[0] && color_item[0].img_url || data.goods_img || "",
      isShow: 1,
      sendByUser: 1,
      actionText:"发送商品",
      extraParam:{ 
        param1: isSelect ? `${productInfo.sale_price||0}`:`¥${data.min_price||0}`,
        param2: isSelect ? `${productInfo.warehouse_product_number||0}`:`¥${data.warehouse_goods_number||0}`,
        url:`/pages/micro_mall/goods/goods_info?goods_id=${data.goods_id}&color_id=${this.options.color_id||0}&product_id=${productInfo.product_id||0}`
      }
    };
    FISH.jump({product});
  },
...
function getDesc(data,productInfo,isSelect){
  let result = `吊牌价：¥${isSelect?productInfo.market_price:data.min_market_price||'--'}    库存：${isSelect?productInfo.warehouse_number||0:data.warehouse_goods_number||0}`;
  return result;
}

function onShowEvent(ops) {
  let goods_id = ops.goods_id;
  checkHasAddFav.call(this,ops).then(res=>{
    this.setData({
      isCollect:res&&res.data||0
    })
  });
  checkSetAdsPop.call(this)
  return getSumaryGoodsDetailData.call(this, ops).then(e => {  
    checkAutoChoose.call(this,ops);
    this.commission = this.commission || this.selectComponent("#commission");
    this.commission.getCommission("NORMAL", goods_id, ops.issued_id, true);
  }).finally(()=>{
    let goods_info = this.data.goods_info || {};
    ...
    Utils.delay(time).then(()=>{
      ...
      getGoodsDimensionPlan.call(this,goods_id);
    })
    Utils.delay(time + 500).then(()=>{
      loadComments.call(this);
      addGoodsVisitLog.call(this, ops);
      showPriceExplain.call(this);
      if (!this.checkPromote && goods_info.sale_type !== 99) { // 赠品不用获取促销活动
        getGoodsPromotionInfo.call(this,goods_id);
        getGoodsLabelInfo.call(this,goods_id);
        getGoodsNextPromotionInfo.call(this,goods_id);
      }
    })
    Utils.delay(time + 1000).then(()=>{
      param_Attr_List.call(this);
      // getVideoList.call(this);
      // is_show_store_share_info.call(this).catch(is_show_share_info.bind(this))
      is_show_store_share_info.call(this);
    ...
    //
    if (!this.customPageLoad){
      checkCustomPage.call(this).then(resData=>{
        let that = this;
        this.setData({
          showCustomPage : true
        })
        ...
        wx.nextTick(()=>{
          that.customPage = that.customPage || that.selectComponent("#microPage");
          that.customPage.getPageData({page_id: resData.page_id});
        })
      })
    }
  });
...
function getSumaryGoodsDetailData(options = {}) {
  return app.CL_GoodsApi.getSumaryGoodsDetailData({
    params: {
    ...
    if (e.code == "1") {
      ...
      //加载倒计时
      setCountDown.call(this,goods_info);
  ...
  .catch(err => {
    ...
    checkIfNeedBackToHome.call(this) // 报错后，检查是否需要跳转回首页
    return Promise.reject();
  })
  .finally(()=>{
...
function getGoodsPromotionInfo(goods_id){
  ...
  this.promoteList.getGoodsPromotion(goods_id,'goods_info');
}
//获取规格
function getSumaryGoodsProductInfo(options = {}, click_type) {
  ...
  if (this.data.storeId && selectShipInfo.selectIndex == 1){
    reqUrl = "getSumaryGoodsProductInfoByShippingStore";
    params.storeId = this.data.storeId || 0;//this.data.storeId;
  }else{
  ...
  return app.CL_GoodsApi[reqUrl]({
    ...
    if (e.code == "1") {
      ...
      // 增加了仓库库存，暂时快速处理
      if (reqUrl = "getSumaryGoodsProductInfo") { // 快递配送，规格总数量 = 门店库存 + 仓库库存
        let inventory_model_config = data.goodsExtend && data.goodsExtend.inventory_model_config; // 2： 仅取仓库库存
      ...
      for (let i = 0; i < ProductList.length; i++) {
        ...
        let color_item = {
          ...
          spec_sort: ProductList[i].spec_sort || 0,
      ...
      try {
        color_arr = color_arr.sort((a,b) => {
          let s1 = Number(a.spec_sort) || 0;
          let s2 = Number(b.spec_sort) || 0;
          return s2 - s1
        })
      } catch (error) {}
      this.setData({
...
function getCartNum() {
  ...
  return app.CL_GoodsApi.getCartStoageCount({
    data: {
      userToken: app.LM.userKey,
      brandCode: app.Conf.RBAND_CODE
    }
  }).then(e => {
    if (e.code == 1) {
      if (e.code == "1") {
...
function createShoppingCart() {
  ...
  else if ((goods_info.self_get == 0 || goods_info.self_get == 1) && selectShipInfo.selectIndex == 1 && !this.data.storeId || (goods_info.self_get == 1 && !selectShipInfo.selectIndex)){
    tip = "请选择自提店铺"
  }
  if(tip){
    app.SMH.showToast({
  ...
  return app.CL_GoodsApi.createBuyCarInsert({
    data: {
      "goods_id": goods_info.goods_id,
      ...
      "shippingStoreId": selectShipInfo.selectIndex == 1 ? this.data.storeId : 0,
      "clientSessionId": app.LgMg.channel && app.LgMg.channel.clientSessionId,
      "visitLogId": app.LgMg.pageLog && app.LgMg.pageLog.logId,
      "clickId": this.buy_type == 'now' ? '' : (app.PH.paramsJson('gdt_vid') || ''),
    },
    other: {
      isShowLoad: true
    }
    ...
    if (e.code == 1) {
      let goodsInfo = {...goods_info,sale_price:productInfo.sale_price,product_id:productInfo.product_id,product_sn:productInfo.product_sn};
      this.buy_type != 'now' && app.WeReportHelper.setGoodsPurchase({goodsInfo,action_type:0,action_num:select_goods.select_goods_count,is_goods_page:1})
      let data = e.data;
...
function getGoodsServices(options = {}) {
  return app.CL_GoodsApi.getGoodsServices({
    params: {
      goodsId: options.goods_id,
    }
  }).then(e => {
...
function addGoodsVisitLog(options = {}) {
  if (!app.LM.isLogin) return;
  return app.CL_GoodsApi.addGoodsVisitLog({
    data: {
      goodsId: options.goods_id,
    },
    other: {
...
function is_show_store_share_info() {
  // 店员code换店员ID
  ...
  if (params.options && params.options.query) {
    ...
    if (query.storeStaffCode) {
      return app.UserApi.getStoreStaffByCode({
        params:{
          "brandCode": app.Conf.BRAND_CODE,
        ...
        if(e.code == 1){
          this.setData({storeStaffInfo: e.data||{}})
          return Promise.resolve(e.data);
        }
        return Promise.reject(e)
      }).catch(err => {
        console.log("获取分享店员信息失败：", err)
...
function checkCustomPage(){
  ...
  return app.RunApi.go('CL_GoodsApi','getGoodsRecommendTemplate',params,extra).then(res=>{
    if (res && res.code =='1' && res.data && res.data.page_id){
...
function param_Attr_List(){
  return app.CL_GoodsApi.get_Goods_Param_Attr_List({
    params:{
      goodsId: this.options.goods_id || 0,
    },
    other: {
...
function setCountDown(goods_info={}){
...
function checkHasAddFav(options){
  return app.CL_GoodsApi.checkHasAddFav({
    data: {
      goodsId: options.goods_id || 0,
    },
  })
}
function getGoodsDimensionPlan(goodsId){
  return app.CL_GoodsApi.get_Goods_DimensionPlan({
    params:{
      goodsId,
      brandCode:app.Conf.BRAND_CODE
    }
    ...
    console.log('res',res);
    let dimensionPlanInfo = res.data||{};
    dimensionPlanInfo.isSpecShowSizeImg = !!((dimensionPlanInfo.Sspec_Show_Size_img == 1) && (dimensionPlanInfo.Size_Img1 || dimensionPlanInfo.Size_Img2));
    this.setData({dimensionPlanInfo});
  })
}
function checkIfNeedBackToHome(){ // 检查是否需要跳转回首页
  if (this.options && this.options.fromPage === "MY_COUPON"){ // 从优惠券列表页面跳转过来的，如果报错了，要帮用户跳转回首页
...
function checkSetAdsPop(){
  try {
    if (this.options.gdt_vid && !this.firstPop) { // 从朋友圈推广进入 且 落地页为商品详情页
      this.firstPop = true
      this.setData({
        adsPop: {
          isIndex:5,
  ...
  } catch (error) {console.log("获取弹窗广告报错", error)}
}

function getGoodsLabelInfo(goods_id){
  return getPromotionLabels(goods_id).then(promotionLabels => {
    this.setData({promotionLabels: promotionLabels[goods_id]})
  })
}

function getGoodsNextPromotionInfo(goods_id){
  return app.CL_GoodsApi.getGoodsNextPromotionInfo({
    params: {
      goodsId: goods_id,
      productId: 0
    }
    ...
    .then(res => {
      if (res.code == 1 && res.data && res.data.ruleId) return Promise.resolve(res.data);
      else return Promise.reject("GetGoodsNextPromotionInfo 无促销活动信息")
    })
    .then(data => {
      this.promoteAssistance = this.promoteAssistance || this.selectComponent("#promoteAssistance");
      this.promoteAssistance && this.promoteAssistance.loadData({...data, goods_id})
    })
    .catch(err => {
      console.log("getGoodsNextPromotionInfo err: ", err)
    })
}


function checkIsCanPop(){
  let goods_info = this.data.goods_info||{};
  if(goods_info.not_allow_buy_type==1 && goods_info.sale_type == 99){
    return false;
  }
  return true;
}
...
function checkAutoChoose(ops) {
  // 如果页面传参有product_id 或者 product_sn  且第一次
  let {saleStatus,goods_info} = this.data;
  console.log(!this.firstTimeAutoChoose && ((ops.product_id && ops.product_id != 0) || ops.product_sn),'进来1')
  if (!this.firstTimeAutoChoose && ((ops.product_id && ops.product_id != 0) || ops.product_sn)) {
    this.firstTimeAutoChoose = true;
    // 已到销售时间 且 不是赠品 则自动选择商品
    saleStatus && goods_info.sale_type != 99 && this.showStylePop({ detail: { buy_type: ops.buy_type || "", product_id: ops.product_id, product_sn: ops.product_sn }});
  }
}
/**
 * 自动选择指定商品(传入任意一个)
 * @param {String} product_id 商品product_id
 * @param {String} product_sn 商品条码
 */
 function autoChooseProduct(product_id, product_sn) {
  //  console.log('进来 autoChooseProduct',product_id, product_sn,this.data.color_size,)
  let argType = product_id ? "product_id" : "product_sn",
    arg = product_id || product_sn,
    color_size = this.data.color_size || {};
  A: for(let i of Object.keys(color_size)) {
    let size = color_size[i];
    for(let j of Object.keys(size)) {
      let product = size[j];
      // console.log('进来 for',product)
      if (product[argType] == arg) {
        // console.log('进来 product',arg)
        if (product.product_number > 0) {
          let {color_name: select_color, color_id: select_color_id, size_name: select_size, size_id: select_size_id} = product;
          this.toggleFnc(0, true, { type:"toggle",setFalse:true });
          let select_goods = this.data.select_goods;
          this.setData({
            swiper_current: 0,
          ...
          console.log('进来 select_goods',this.data.select_goods)
          getSpecNumMap.call(this);
        } 
        break A
      }
...
function recordTime(type) {
  if(type == 'set'){
    this.recordTimeSet = new Date().getTime();
    return this.recordTimeSet
  }else{
    let recordTimeRead = new Date().getTime();
    return (recordTimeRead - this.recordTimeSet)/1000;
  }
}

function setReport() {
  let productInfo = this.data.select_goods&&this.data.select_goods.productInfo||{};
  let goodsInfo = this.data.goods_info||{};
  goodsInfo.stayTime = Math.ceil(recordTime.call(this,'read')||0);
  this.validGoods && app.WeReportHelper.setGoodsExpose({goodsInfo,productInfo,isSelect:!!productInfo.product_id});
  this.validGoods && app.WeReportHelper.setGoodsBrowse({goodsInfo,productInfo,isSelect:!!productInfo.product_id});
}