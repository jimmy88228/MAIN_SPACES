...
Component(app.BTAB({
  ...
  methods: {
    ...
    initShippingInfo(selectInfo) { //init
      selectInfo = selectInfo || {};
      let selectShipInfo = this.data.selectShipInfo || {};
      let storeInfo = StoreH.storeInfo || {};
      if(selectInfo.shippingType == 1 && selectInfo.storeId){
        selectShipInfo.id = storeInfo.storeId;
        selectShipInfo.id = storeInfo.storeName;
      }
      selectShipInfo.selectIndex = selectInfo.shippingType || 2;
      this.setData({
    ...
    onTap(e){
      ...
      if (type =="shippingChange"){
        ...
        if(index == 1){
          let storeInfo = StoreH.storeInfo || {};
          selectShipInfo.id = storeInfo.storeId
          selectShipInfo.name = storeInfo.storeName
          selectShipInfo.selectIndex = index;
        } else {
          selectShipInfo.id = 0;
          selectShipInfo.name = '';
        }
        this.triggerEvent('shippingChange',{index, update, store_id: selectShipInfo.id});
      } 
