...
const app = getApp();
let couponPopProcudeFinishedOnce = false;
Component(app.BTAB({
  ...
  methods: {
    ...
    initData({type = "register", afterDismiss, afterHideFn}) {
      ...
      switch (type){
        case "register":
          console.log("getCouponsPop组件 initData")
          return registerShowCouponDetails() //value:1 显示优惠券列表 
            .then(value => value == '1' ? getCouponListAfterRegister.call(this) : Promise.resolve(false))
            .then((needToDisplay) => { //展示列表
              return new Promise(resolve => {
                if (!needToDisplay) return resolve(false)
                app.SMH.hideToast();
                app.SMH.hideLoading();
                wx.nextTick(() => {
                  console.log("getCouponsPopshow")
                  this.show()
                  console.log("randomUnit", this.data.randomUnit);
                  resolve(true)
                })
              })
            })
...
function registerShowCouponDetails(){
  ...
  return app.sysTemConfig("register_show_coupon_details")
    .then(e => {
      if (couponPopProcudeFinishedOnce) return Promise.resolve(0)
      else {
        couponPopProcudeFinishedOnce = true;
