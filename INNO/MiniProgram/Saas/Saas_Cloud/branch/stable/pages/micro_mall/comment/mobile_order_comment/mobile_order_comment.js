...
function getOnlineOrderCommentDetail(options = {}) {
    let _params= options.type == 'store'
    ?{orderSn:options.order_sn, brandCode: app.Conf.BRAND_CODE}
    :{orderId: options.order_id};
    let requestF = options.type == 'store'? app.GoodsApi.getOfflineOrderCommentDetail : app.CL_GoodsApi.getOnlineOrderCommentDetail;
    return requestF({
        params: {
            ..._params,
        },
