...
function loadData(type = 0, dateType) {
    if (this.hasMore[type] && !this.isLoading[type] && this.reflash[type]) {
        ...
        let api={api:"DistrApi",str:"cashOutList"};
        if (this.data.pageType == 'commission') {
            api.api = [`CL_DistrApi`]
            api.str = [`staffCommOrderList`]
        }else if (this.data.pageType == 'share') {
            api.api = [`CL_DistrApi`]
            api.str = [`getStaffDstbUserReport`]
        }
        ...
        selectLoadData.call(this, api, type, typeParam, endDate,dateType).then(res => {
            if (res.code == 1) {
                const data = res.data || {};
                let dataList = [];
                if (this.data.pageType == 'commission') {
...
function selectLoadData(api, type, typeParam, endDate,dateType) {
    let obj = {};
    obj = {
    ...
    if (api.str && this.data.pageType == 'commission') { //分销订单
    ...
    } else if (api.str && this.data.pageType == 'record') { //提现记录
        obj.status = typeParam;
    } else if (api.str && this.data.pageType == 'share') { //提现记录
        obj = {
            userToken:  app.LM.userKey || "",
            brandCode: app.Conf.BRAND_CODE,
