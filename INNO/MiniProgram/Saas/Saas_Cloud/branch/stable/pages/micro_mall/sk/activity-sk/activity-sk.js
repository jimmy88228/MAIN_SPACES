...
import storeHandle from '../../../../common/helper/handle/storeHandle.js';
...
Page(app.BP({
  ...
  onShareAppMessage(e){
    let title = String(this.activityInfo.shareTitle || this.activityInfo.activityName || "助力分享").trim();
    let groupId = this.options.groupId || 0;
    let imageUrl = String(this.activityInfo.shareImg || '').trim();
    let target = 0;
    let shareType = ""
    if (e && e.from == 'button'){
...
function initParams(ops){
  return this._checkUserLogin().finally(()=>{
    iniTabbar.call(this);
    loadAcGroup.call(this).then(res => {
      console.log('loadAcGroup then',res)
...
function getGoodsList(isReachBottom){
  ...
  let loadConf = isReachBottom ? {} : {
    init: true, 
    layout: app.StringUtl.numTransEn(this.activityInfo.layoutType || 1),
