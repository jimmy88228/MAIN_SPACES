...
Component(app.BTAB({
  ...
  data: {
    ...
    changeShipInfo: {},
    promotionInfo: {},
    navH: parseFloat(app.SIH.navPlace) || 0,
    extraH:parseFloat(app.SIH.navPlace) + 48 || 0,
  ...
  methods: {
    ...
    singleSelectConfirm(e) {
      ...
      let {fromType, select, index, recId} = dataset;
      let fromPrm = !!(fromType == 'promotion');
    ...
    cancelSign(e){
      ...
      storage.storeId = app.StoreH.storeId;
...
function countPromotion(rec_str = '', total_count) {
  ...
  return app.RunApi.go('post', 'CL_GoodsApi', 'countPromotionInfoByJieSuan', params,{diy:true}).then(res => {
    if (res.code == '1') {
      ...
      this.promoteAssistance && this.promoteAssistance.loadData(data.nextPromotionInfo || {})
      this.setData({
        total_count
      })
    }
...
function checkSignReward(){
  ...
  return app.RunApi.go('CL_UserApi','get_UserSignOrderActivityReward',params).then(res=>{
    let data = res.data||{};
    let list = data.list||[];
    let curSignActInfo = list[0] || {};
    try {
      list.length && (curSignActInfo = list.sort((a, b) => {
    ...
    let curStoreId = app.StoreH.storeId;
    let storage = app.StorageH.get('curSignActInfo',{});
    if(curSignActInfo.id && (storage.id != curSignActInfo.id || storage.storeId != curStoreId)){
      this.setData({curSignActInfo,cancelSignBool:false})
    }
    storage.storeId = curStoreId;
