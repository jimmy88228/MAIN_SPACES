...
function getLotteryResult() {
    ...
    if (this.properties.isCheckWeChat != 3) p = get_UserIsGroupChatWeChat(globalOptions.activityId || 0);
    // if (this.properties.isCheckWeChat != 3) p = get_UserIsGroupChatWeChat(this.properties.isCheckWeChat);
    return p.then(() => {
      return app.CL_LotteryApi.getLotteryResult({
          data: {
...
function get_UserIsGroupChatWeChat(lotteryId){
  return app.CL_LotteryApi.get_UserIsGroupChatWeChat({
    data: {
      lotteryId
    },
    other: {
