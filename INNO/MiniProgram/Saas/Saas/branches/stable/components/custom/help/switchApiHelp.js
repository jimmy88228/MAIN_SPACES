...
function SwitchApi({bindType,data,index,extend,sr_t,Limit_S}) {
  ...
  let initMsg = {goApi:true,data:[],hide:false,apiParams:{api, url, params, extra, extend}};
  let detail = {};
  let p = new Promise((rs, rj) => {
    switch (bindType) { //1~12 初始化参数
      ...
      case 13:
        ...
        break;
      case 15:
        let index = extend.index || 0;
        detail = (data.detail_list && data.detail_list[index]) || {};
        try {
          if(data.moduleType == 4){ //导航广告 -- 广告
            let s_ad = [ detail ];
            if (s_ad.length > 0) {
              mapData(s_ad, data.moduleId);
            }
            initMsg.data = s_ad;
          } else if(data.moduleType == 5){ //导航广告 -- 商品
            if(detail.bind_type == 1){  // 单品
              api = 'GoodsApi';
              url = 'getALLGoodsListByGoodsIds';
              params = initParams({bindType,index,sr_t,data: data});
              if (!params || !params.goodIds) {
                initMsg.hide = true;
              };
            } else {   // 列表
          ...
          if(!extend.index) extend.index = index;
          if(!extend.acId) extend.acId = detail.activity_id;
        } catch (e) {
          console.log("模块错误", e)
          .hide = true;
        }
      ...
      case 4:
        try {
          ...
          if (carousel_ad.length > 0) {
            mapData(carousel_ad,data.moduleId);
          }
      ...
      case 7:
        try {
          ...
          if(!listItem)throw Error('detail_list异常')
          if (listItem.activity_show_type == 'auto') {
            ...
            throw Error('activity_show_type异常');
          }
    ...
    if(bindType == 3 || bindType == 4 || bindType == 13 || bindType == 16 || initMsg.hide || (bindType == 15 && data.moduleType == 4)){
      initMsg.goApi = false;
      initMsg.apiParams.extend = extend;
    ...
    initMsg.apiParams = {api,url,params,extra, extend};
    return rs(initMsg);  
    ...
    let p = new Promise((rs, rj) => {
      return RunApi.go(api, url, params, extra).then(res => { //对应模块换数据   //广告和轮播不用另外调接口，在case已经整理完毕
        if (bindType == 1) { //单品
          ...
          if(url == 'getALLGoodsListByGoodsIds'){
            tempData = res.data && res.data.goodsList || [];
          } else if(url == 'getSearchGoodsListBySkip'){
            tempData = res.data && res.data.goods_list || [];
          }
        }
...
function initParams({index = 0, bindType = 0, data = {}, extend = {},sr_t,Limit_S}) { //传参初始化  SwitchApi 函数用到
  ...
  if (bindType == 7) {
  ...
  }else if (bindType == 8 || bindType == 10 || bindType == 11 || bindType == 12 || bindType == 14) {
    ...
    let detail = (data.detail_list && data.detail_list[index]) || {};
    if(detail.bind_type == 1){//data.bind_type 1 单个数据中单品标识  2 单个数据中商品列表标识
      let goodsIds = [];
      let itemData = (data.moduleItem && data.moduleItem.itemJson[detail.activity_id]) || []
    ...
    } else if(detail.bind_type == 2){
      params = {
        functype: detail.func_type || 'CA',
        catId: detail.related_id || 0,
        strAttrId: '',
        strAttrValue: '',
        colorCatId: 0,
        startPrice: -1,
...
function mapData(data = [], moduleId) { //热点处理
  data.forEach((item, index) => {
    let extend_content = item.extend_content || "";
    let func_type = item.func_type || "";
    if(!item.module_id) item.module_id = moduleId; //广告需要加上moduleId
    if(extend_content.indexOf("%5B") != -1){
      extend_content = decodeURIComponent(extend_content) || "";
    }
    item.customData = {
      func_type: func_type,
    ...
    try{
      if (extend_content && func_type == "RD") {
        ...
        let map_data = extend_content && JSON.parse(extend_content) || {};
