...
Page(app.BP({
    ...
    onLoad: function(options) {
        ...
        let countKey = options.type == 'store' ? 'goods_comment_offline_word_count' : 'goods_comment_online_word_count'
        app.sysTemConfig(countKey).then(data => {
            let val = parseInt(data.Value); 
            this.setData({
                minTextNum: val >= 0 ? val : MIN_TEXT_NUM ,
                maxTextNum: val >= 0 ? Math.max(val,MAX_TEXT_NUM) : MAX_TEXT_NUM
            })
        })
    },
    // onShow() {
    ...
    handleContentInput: function(e) {
        ...
        this.setData({
            ...
            currentTextNum: num < this.data.maxTextNum ? num : this.data.maxTextNum,
        })
    },
    // 发布
    handleCommitData: function() { 
        let imgList = [].concat(this.data.imgList);
        let minTextNum = this.data.minTextNum;
        if (minTextNum > 0 && this.data.commentContent.length < minTextNum) {
            app.SMH.showToast({
                title: `字数不少于${minTextNum}个`
            });
            return;
        }
        if(this.lock)return;
    ...
    commitData: function() {
        ...
        let reqUrl = isStoreOrder ? "commentOfflineOrderGoods" : "commentOnlineOrderGoods"
...
function getGoodsDefaultImage(options = {}) {
    return app.GoodsApi.getGoodsDefaultImage({
        params: {
...
function getOrderCommentDetail(type="") {
    let options = this.options||{};
    let isStoreOrder = this.data.isStoreOrder;
    let req = isStoreOrder ? "getOfflineOrderCommentDetail" : "getOnlineOrderCommentDetail";
    let reqData = {};
    if(isStoreOrder == 1){
        reqData = {
...
function shippingCommitFnc(){
    ...
    let isStoreOrder = this.data.isStoreOrder;
    // 店铺、微商城评论
    let req = isStoreOrder ? "commentOfflineOrder" : "commentOnlineOrder"
    let data = {
        "brandCode": app.Conf.BRAND_CODE
    } , otherData = {}
    if(isStoreOrder){
