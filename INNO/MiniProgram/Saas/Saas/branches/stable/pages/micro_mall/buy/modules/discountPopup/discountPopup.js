...
Component(app.BTAB({
  ...
  observers:{
    temChooseDiscountGoods:function(data){
      ...
      // computedBuyLimit.call(this, idGoodsChoose, this.data.ruleList, this.data.orderBuyLimit, selectAllCount);
...
function checkExchange(dataset){
  ...
  let chooseNum = parseInt(idGoodsChoose[dataset.goodsId] && idGoodsChoose[dataset.goodsId].number) || 0;
  if(!dataset.isFit){
    warn = '还没达到该换购门槛';
  } else if(selectAllCount == orderBuyLimit && orderBuyLimit > 0){ //针对整单
    warn = '已超过换购件数，最大换购数量为' + orderBuyLimit + '件';
  } else if(!(dataset.mixNum > 0)){// 不可选商品
    if(chooseNum > 0){ // 已选择商品
      warn = '该商品已超出最大换购数(' + (dataset.mixNum + chooseNum) + ')'
    } else {
      warn = '该商品暂无换购数量';
    }
  }
...
function computedBuyLimit(selectData, ruleList, orderBuyLimit = 0, selectAllCount = 0){
  ...
  for(let i = 0; i < ruleList.length; i++){
    ...
    for(let j = 0; j < goodsList.length; j++){
      ...
      // let userHadBuy = selectData[goods.goodsId] && selectData[goods.goodsId].number || 0;
      // let mixNum = Math.min(orderBuyLimit, goods.orderBuyLimit, (goods.userBuyLimit - userHadBuy)); 
