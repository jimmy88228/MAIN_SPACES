...
function loadData(update=false) {
  ...
  extra.diy = false;
  getGoodsDimensionPlan.call(this,this.options.goodsId);
...
function getSku(){
  ...
  return app.RunApi.go('SecKillApi', 'getProducts', params, this.extra).then(res => {
    if (res.code == '1') {
      ...
      select_goods.goodsExtend = goodsExtend;
      select_goods.goodsTotalInfo = goodsTotalInfo;
      ...
      productList.forEach(item=>{
        if (!colorObj[item.colorId]){
          colorObj[item.colorId] = {};
          colorArr.push({ colorId: item.colorId, colorName: item.colorName });
          if (goodsExtend.attrCount == 1){
            colorObj[item.colorId].sizeList || (colorObj[item.colorId].sizeList = {})
            colorObj[item.colorId].sizeList[0] = item;
          }else{
            colorObj[item.colorId].colorId = item.colorId;
            colorObj[item.colorId].colorName = item.colorName; 
          }
        }
        if (!sizeObj[item.sizeId]){
          sizeObj[item.sizeId] = {};
          sizeArr.push({ sizeId: item.sizeId, sizeName: item.sizeName });
          if (goodsExtend.attrCount == 1){
            sizeObj[item.sizeId].colorList || (sizeObj[item.sizeId].colorList = {});
            sizeObj[item.sizeId].colorList[0] = item;
          }else{
            sizeObj[item.sizeId].sizeId = item.sizeId;
            sizeObj[item.sizeId].sizeName = item.sizeName; 
          }
        }
        ...
        if (goodsExtend.attrCount == 2) {
          colorObj[item.colorId].sizeList || (colorObj[item.colorId].sizeList = {});
          colorObj[item.colorId].sizeList[item.sizeId] = item;
          sizeObj[item.sizeId].colorList || (sizeObj[item.sizeId].colorList = {});
          sizeObj[item.sizeId].colorList[item.colorId] = item;
        }
      });
...
function setSelectInfo(type='',data=0){
  ...
  if(type == 'color'){
    ...
    select_goods.select_color_id = data;
    select_goods.select_color = colorObj[data].colorName || '';  
    let size_id = select_goods.select_size_id; 
    this.setData({
      swiper_current:0
    })
    if (size_id && colorObj[data].sizeList){
      if ((!colorObj[data].sizeList[size_id])){ //点颜色时 已选中的尺寸不存在于点击的颜色 清除操作
        // console.log('清除尺码 ', colorObj[data].sizeList[size_id]);
    ...
    if(data == select_goods.select_size_id){
      select_goods.select_size_id = 0;
      select_goods.select_size = "";
      select_goods.productInfo = {};
      return
    ...
    select_goods.select_size = color_id && colorObj[color_id].sizeList && colorObj[color_id].sizeList[size_id] && colorObj[color_id].sizeList[size_id].sizeName || ''; 
  }  
  // console.log('select_goods', select_goods);
}
...
function setProductInfo(){
  let select_goods = this.select_goods || {};
  let goodsExtend = select_goods.goodsExtend || {};
  let colorObj = this.data.colorObj || {};
  let color_id = select_goods.select_color_id || 0;
  let size_id = select_goods.select_size_id || 0;
  // if (goodsExtend.attrCount==1){
  //   if (select_goods.select_color_id){
  //     let color_id = select_goods.select_color_id;
  //     let size_id = 0; 
  //     select_goods.productInfo = colorObj[color_id] && colorObj[color_id].sizeList && colorObj[color_id].sizeList[size_id] || {};
  ...
  //   } 
  // }
  if ((goodsExtend.attrCount==1 && color_id) || (goodsExtend.attrCount==2 && color_id && size_id)){
    select_goods.productInfo = colorObj[color_id] && colorObj[color_id].sizeList && colorObj[color_id].sizeList[size_id] || {};
    console.log('选满');
  }else{
    console.log('未选满');
