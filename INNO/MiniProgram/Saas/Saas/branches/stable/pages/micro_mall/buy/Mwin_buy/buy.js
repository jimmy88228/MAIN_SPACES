...
Page(app.BP({
    ...
    useCashTicket: function() {
        ...
        if (cashCouponUseLimit && useCashCoupon.length && useCashCoupon.length == cashCouponUseLimit) return this.setData({ 'cashCoupon_data.error': `该订单最多可以使用${cashCouponUseLimit}张现金券`});    
...
function listen(){
  ...
  if (this.options.scene && !this.options.related_key){
    ...
    //   if (paramsJson.related_key) {
    //     this.options.related_key = paramsJson.related_key;
...
function listenLogin(){
  this.listenLoginStatuId = app.EB.listen("LoginStateChange", () => {
    if (app.LM.isLogin && this.options.related_key) {
      onShowEvent.call(this, this.options.related_key);
    }
  });
}
