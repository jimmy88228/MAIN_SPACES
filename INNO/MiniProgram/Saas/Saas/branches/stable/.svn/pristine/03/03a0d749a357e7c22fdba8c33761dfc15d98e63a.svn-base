...
function getAreaByCode(code) {
  ...
  return {
    code: code,
    province: getAreaData().province_list[pCode] && getAreaData().province_list[pCode].region_name || '',
    city: getAreaData().city_list[cCode] && getAreaData().city_list[cCode].region_name || '',
    area: getAreaData().area_list[code] && getAreaData().area_list[code].region_name || '',
  };
}
...
function getTargetParentAreaListByCode(target, code) {
  ...
  result.unshift({
    code,
    name: getAreaData().area_list[code] && getAreaData().area_list[code].region_name || '',
  });
  if (['city', 'province'].includes(target)) {
    code = code.slice(0, 4) + '00';
    result.unshift({
      code,
      name: getAreaData().city_list[code] && getAreaData().city_list[code].region_name || '',
    });
  }
  if (target === 'province') {
    ...
    result.unshift({
      code,
      name: getAreaData().province_list[code] && getAreaData().province_list[code].region_name || '',
    });
  }
  return result;
}
...
function getAreaByAddress({province, city, area}) {
  const {province_list, city_list, area_list} = getAreaData();
  const result = {
    code: '',
    province: '',
    city: '',
  ...
  for (let _code in province_list) {
    let _province = province_list[_code] && province_list[_code].region_name || "";
    if (_province.indexOf(province) === 0) {
      ...
      for (let _code_city in city_list) {
        if (_code_city.indexOf(_code) === 0) {
          let _city = city_list[_code_city] && city_list[_code_city].region_name || "";
            ...
            if (area) {
              ...
              for (let _code_area in area_list) {
                if (_code_area.indexOf(_code_city) === 0) {
                  let _area = area_list[_code_area] &&  area_list[_code_area].region_name;
