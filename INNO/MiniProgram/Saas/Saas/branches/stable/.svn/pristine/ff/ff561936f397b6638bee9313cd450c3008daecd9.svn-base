...
function updateBtnContainer(){
  ...
  let container = [
    ...
    },
    {
      ...
      "dataType":"shipping",
      "name":"查看物流",
      "status": order_info.shipping_status != 0 && order_info.shipping_status != 3 && this.data.show_invoice_btn && (order_info.show_invoice_no && order_info.show_invoice_no.length>0) && this.isShowLogistics == 1
    },
  ];
  let unfoldNum = 0;
  let unfoldBtnArr = []; //展开
...
function asyncGetConfig(){
  ...
  let p7 = app.sysTemConfig("is_show_logistics").then(e=>{
    this.isShowLogistics = e.Value||0
  })
  arr.push(p7);
