...
Component(app.BTAB({
  ...
  methods: {
    ...
    getAllQuery(callBack){
      this.throttle(1000,'refreshKey',()=>{
        this.mcGetQuery(`.cItem${this.data.tabIndex||0}`,'all').then(res=>{
          ...
          // console.log('getAllQuery',this.data.tabIndex,this.isInitData,JSON.parse(JSON.stringify(this.nodeInfo),this.data._pageModelList));
          if(!this.isInitData){ 
            this.initData();
          }else{
    ...
    initData(){
      ...
      // console.log('initData',arr,sclInfo)
      arr.every((item,i)=>{
        if(this.properties.autoShow || (((item.top+sclInfo.scrollTop) <= this.screenH) && !this.nodeLoad[i])){
          ...
          console.log('初始化',this.properties.autoShow,i,JSON.parse(JSON.stringify(arr))) 
          wx.nextTick(()=>{
