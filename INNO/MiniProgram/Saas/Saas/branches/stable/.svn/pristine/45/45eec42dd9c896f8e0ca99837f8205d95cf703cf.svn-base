...
Page(app.BP({
    ...
    jump(e) {
        ...
        if (type == 'comments') {
            ...
            url = url + `?goods_id=${goodsId || 0}&related_id=${this.options.id}&source=GRASS`
        }
        wx.navigateTo({
            url: url,
        })
    ...
    chooseShareType(data) {
        ...
        let opKind = app.OpKind[PAGE_TYPE] || app.OpKind.GRASS_DETAIL;
        let shareType = app.ShareType[PAGE_TYPE] || app.ShareType.GRASS_DETAIL;
        let allData = {
          info:{
            imgUrl: infoData.imgdomain && infoData.pubImages && (infoData.imgdomain + (infoData.pubImages[0] && infoData.pubImages[0].img_path || "")) || "",
            articleInfo: infoData,
            opKind:opKind,
          },
          scene: {
            "shareType": shareType,
      ...
      setClipBoardF(e){
        ...
        return text ? WxApi.setClipboardData({data: text}) : Promise.reject("复制到剪切板失败")
      }
}))
//
...
function shareLogEvent(){
    let infoData = this.data.infoData || {};
    let pubInfo = infoData.pubInfo || {};
    return app.GrassApi.Op_SharePublish_Shares({
        data: {
