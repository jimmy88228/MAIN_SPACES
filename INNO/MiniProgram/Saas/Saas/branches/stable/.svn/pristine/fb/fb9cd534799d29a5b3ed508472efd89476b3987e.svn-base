// components/custom/module_ branches/advertise.js
...
Component(app.BTAB({ 
        ...
        for (let i = 0; i < n.length; i++) {
          ...
          if (iItem.func_type == "VIDEO" && iItem.link_url) {
            ...
            if (!this.videoObj["video_" + i]) {
              ...
              if (iItem.img_path) { // 计算视频高度，根据视频海报图适应
                WxApi.getImageInfo({src:  this.properties.img_url + iItem.img_path})
                  .then(res => {
                    let {width, height} = res;
                    let widthScale = 750 / width;
                    let fixedHeight = height * widthScale;
                    this.setData({[`videoSizeObj.video_${i}`]: {height: fixedHeight}})
                  })
              }
            }
            ...
            try {
              for(let j = 0; j < iItem.map_data.length; j++){
                ...
                map_item.customData = {
                  ...
                  applet_path: map_item.url
                }
                if(map_item.func_id == 'CHANNELSLIVE'){
                  // map_item.link_url = map_item.url;
                  // ChannelsLiveH.getChannelsInfo(map_item);
                }
              }
            } catch (error) {
              console.log(error,iItem);
            }            
          } else if(iItem.func_type == 'CHANNELSLIVE'){
            ChannelsLiveH.getChannelsInfo(iItem);
          }
        }
        this.setData({
          m_data: n,
  ...
  methods: { 
    goLink: function(e) {
      ...
      // console.log(dataset,{...(dataset.customData||{})},dataset.customData);
      ...
      if (func_type == "COUPON") { //优惠券
        ...
        let temp = "", data = {};
        if (dataset.stringJump || dataset.extend_contentd){
          temp = JSON.parse(dataset.stringJump || dataset.extend_contentd || {});
          data.appId = temp.appId;
          data.applet_path = temp.applet_path;
        } else if (dataset.applet_path && dataset.extent_id){
          data.appId = dataset.extent_id;
          data.applet_path = dataset.applet_path;
        } else{
          return;
        }
