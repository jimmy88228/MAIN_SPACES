...
class template {
  ...
  initNewGoodsTemplate(allData={}){
    ...
    if(opKind == 'COLLOCATE_GOODS'){ //搭配商品的特定处理
      if (goodsInfo.discountPrice && parseFloat(goodsInfo.discountPrice) > 0){
        line_colt += 1;
        let price = this.initData('text', padding, 0);
        price.text = '最多可省' + goodsInfo.discountPrice;
        price.position = 'relative';
        price.extraH = line_colt < 2 ? baseH*3 : baseH;
        price.color = Config.style.font_color || '#000';
        arr.push(price);
      }
       
      if (goodsInfo.toDate){
        line_colt+=1;
        let toDate = this.initData('text', padding, 0);
        toDate.size = 24;
        toDate.extraH = line_colt < 2 ? baseH*3 : baseH;
        toDate.color = '#7f7f7f';
        toDate.text = '活动至' + goodsInfo.toDate;
        toDate.position= 'relative';
        arr.push(toDate);
      }
      if (goodsInfo.packageDecription){
        line_colt+=1;
        let decrip = this.initData('text', padding, 0,280);
        decrip.size = 24;
        decrip.color = '#7f7f7f';
        decrip.text = goodsInfo.packageDecription;
        decrip.ellipsis = 2;
        decrip.position = 'relative';
        decrip.extraH = line_colt < 2 ? baseH*3 : baseH;
        arr.push(decrip);
      }
    ...
    if (goodsInfo.price) {
      ...
      } else if (opKind === "POINT_GOODS" && goodsInfo.min_integral > 0) { // 对于积分礼品，加上 xxx积分
        extra = " + 积分" + goodsInfo.min_integral;
  ...
  initArticleTemplate(allData = {}){
    let draw = allData.draw || {};
    // let baseInfo = draw.baseInfo || {};
    let arr = draw.drawArr = draw.drawArr || [];
    let info = allData.info || {};
    let canvasW = this.canvasConf.width;
    let articleInfo = info.articleInfo || {};
    ...
    if (info.imgUrl) {
      let bg = this.initData('image', 0, baseTopLine, canvasW, canvasW);
      bg.url = info.imgUrl || '';
      bg.mode = 'aspectFit2';//fitORfill -> 当高>宽==正常的等高等比例模式(aspectFit)，当宽<高==等宽裁剪模式(aspectFill)
      bg.h = canvasH * (P_Conf_Bool? 0.55 : 0.75);
      bg.w = canvasW; 
      arr.push(bg);
    }
    baseTopLine += P_Conf_Bool ? (canvasW + padding - (canvasH * 0.12)) : (canvasW + padding + (canvasH * 0.095));
    // 文字 文章标题
    if (articleInfo.pubInfo && articleInfo.pubInfo.pub_title){
      // baseTopLine += canvasH * 0.045 - ((articleInfo.pubInfo.pub_title.length > 18 ? 1 : 0) * 10);
      this.checkName = articleInfo.pubInfo.pub_title;
      line += 1;
      line_colt += 1;
      // let title = this.initData('text', padding + 200, baseTopLine, baseStrW);
      let title = this.initData('text', padding, baseTopLine, baseStrW);
      title.ellipsis = P_Conf_Bool?1:2;
      // title.align = "center"
      title.w = canvasW * 0.655
      // title.ellipsis = 4;
      title.text = articleInfo.pubInfo.pub_title;
      title.size = 22;
      title.bold = true;
      title.checkLine = P_Conf_Bool ? true : false;
      arr.push(title);
      baseTopLine += title.text.length >= 18 ? title.size + (0.03 * canvasH) : title.size;
    }

    baseTopLine += canvasH * 0.015
    // // 文字 文章内容
    if (articleInfo.pubInfo && articleInfo.pubInfo.pub_content) {
      let textArr = articleInfo.pubInfo.pub_content.split('\n');
      let textLine = textArr.length
      for (let i = 0; i < textLine ; i++) {
        let textStr = ""
        if (i == 3) textStr = "..."
        else textStr = textArr[i];
        line += 1;
        const content = this.initData('text', padding, baseTopLine);
        // content.position = 'relative';
        // content.extraH = getExtraH.call(this, line, 15, { type:'sn'});
        // content.ellipsis = 1;
        content.color = '#B2B2B2';
        content.w = canvasW * 0.6;
        content.size = 20;
        // content.text = articleInfo.pubInfo.pub_content;
        content.text = textStr.substr(0, 18) + (textStr.length > 18 ? '...' : '')
        arr.push(content);
        baseTopLine += content.size + canvasH * 0.01;
        if (i == 3) break;
      }
    }
    return Promise.resolve(arr || []);
