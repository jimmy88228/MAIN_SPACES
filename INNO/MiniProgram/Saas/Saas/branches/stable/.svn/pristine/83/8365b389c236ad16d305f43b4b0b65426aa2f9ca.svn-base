...
Page(app.BP({
  data: {
    ...
    optionList: [{ name: '全部门店', canUse: -1 }, { name: '允许使用', canUse: 1 }, { name: '不可使用', canUse: 0 }],
    storePhoneShow: false, // 是否显示店铺电话
  },
  page: 0,
  hasMore: true,
  all_store_data: {},
  ...
  onLoad: function (options) {
    ...
    let _data = {type:options.type||""};
     // 储值页面跳转进来附近店铺
    if (options.type === 'prepaid') {
      _data.defaultOption =  this.data.optionList[0]
    }
...
function getStoreData(searchName, isInit) {
  ...
  switch (ops.type) {
    case "selectByGoods": //商品详情进入店铺列表
      ...
      reqParams.type = fromType2ParamType[ops.fromType] || 0
