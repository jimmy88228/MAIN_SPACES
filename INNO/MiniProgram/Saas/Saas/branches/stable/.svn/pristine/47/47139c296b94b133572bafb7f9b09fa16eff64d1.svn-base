...
function toMiniProgram(appId,path,envVersion){
  ...
  if (Conf.BRAND_CODE === "MRJ" && appId === "wx45a10947e1c36f49") { // 品牌: 美人计 要求跳转到该appId带上特定参数
  ...
  return LM.checkIfStore()
    .then((storeStaffInfo = {}) => {
      ...
      if (staff_type == 1) { // 用户是店长
        let appendedUrl = `salesmanNo=${salesmanNo}&salesmanMobile=${salesmanMobile}&agentNo=${agentNo}`;
