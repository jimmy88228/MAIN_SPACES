...
import { modifyGoodsSelfGet } from './module/videoLiveChannelControl/helper.js';
let app = getApp();
// let numArr = [1, 2, 3, 5];
...
Page(app.BP({
  data: {
    ...
    fixOpacity:0,
    isHideFixBox:false,
    showCustomModule: "", // 是否显示自定义模块 "" -> 不显示；"embed" -> 基础组件；"page" -> 自定义页
    ruleFolderOpen: true, // 是否展开尺码
    packageDisabledNormalBuy: 0, // 是否开启"搭配套餐不允许普通购买"
  ...
  onLoad: function (options) {
    ...
    let arrow = bInfo.icon_url + "micro_mall/coupon/arrow.png";
  ...
  onShow() {
    app.LM.loginAsync(true).finally(()=>{
      ...
      // 清空已选择的自提店铺
      let selectShipInfo = this.data.selectShipInfo || {selectIndex: 2}
      if (!app.StorageH.get("select_store")) {
        selectShipInfo = {
          selectIndex: this.data.selectShipInfo && this.data.selectShipInfo.selectIndex || 2
        }
      }
      this.setData({
        isFromVideoAccount: videoAccountType[paramsJson.scene],
        selectShipInfo
      })
    })
  },
  ...
  onReady: function () {
    ...
    AllStaffApply.checkAllStaffApply(this,{goods_id:this.options.goods_id||0,goodsType:"NORMAL"});
    // let ops = this.options||{};
    // // 如果页面传参有product_id 或者 product_sn,且已到销售时间 则自动选择商品
    // (ops.product_id || ops.product_sn) && this.data.saleStatus && sale_type && this.showStylePop({ detail: { buy_type: ops.buy_type || "", product_id: ops.product_id, product_sn: ops.product_sn }})
    // 检测全民分销员
    // app.LM.loginAsync(false).finally(()=>{
    //   checkStaffApply().then(msg=>{
    //     this.staffApply = this.staffApply || this.selectComponent("#staffApply");
    //     this.staffApply._show(msg);
    //   })
    // }) 
  },
  preView(e){
  ...
  onPageScroll(e){
    ...
    if (this.data.showCustomModule === 'page') {
  ...
  onReachBottom() {
    ...
    if (this.data.showCustomModule === 'page') {
      this.pageTab = this.pageTab || this.selectComponent(`#${this.data.microType}`);
      console.log('this.pageTab',this.pageTab,this.data.microType,this.data.showCustomModule)
  ...
  videoJump(){
    setTimeout(()=>{
      anchorFixed.call(this,'#videoList','view');
    },300)
  },
  commentEvent(e){
...
function onShowEvent(ops) {
  ...
  return getSumaryGoodsDetailData.call(this, ops).then(e => {
    ...
    //
    if (!this.customModuleLoad){
      checkCustom.call(this).then(resData=>{
        let showCustomModule = "",
          customModuleComponentId,
          customModuleData;
        if (resData.PageId) { // 要加载的 是自定义页
          showCustomModule = 'page';
          customModuleComponentId = `#${this.data.microType}`;
          customModuleData = {
            page_id: resData.PageId || ""
          }
        } else { // 要加载的是 基础组件
          showCustomModule = 'embed';
          customModuleComponentId = '#embedId';
          customModuleData = resData;
        }
        this.setData({showCustomModule}, () => {
          setTimeout(()=>{
            this.customModule = this.customModule || this.selectComponent(customModuleComponentId);
            this.customModule.getPageData(customModuleData);
          },200)
        })
      })
    }
...
function getSumaryGoodsDetailData(options = {}) {
  return app.GoodsApi.getSumaryGoodsDetailData({
    ...
    if (e.code == "1") {
      ...
      modifyGoodsSelfGet(goods_info); // 视频号处理
      ...
      goods_info.isShowSizeImg = !!((goods_info.spec_show_size_img == 1) && (goods_info.size_img1 || goods_info.size_img2));
  ...
  return app.GoodsApi[reqUrl]({
    ...
    if (e.code == "1") {
      ...
      for (let i = 0; i < ProductList.length; i++) {
        ...
        // 第一次调用此方法 且 当自动选择自提门店，且货存没有时；帮用户切换回快递配送
        if (!this.FirstTimeGetInfoFinish && params.storeId && Object.keys(color_size).every(key => color_size[key].total_num == 0)) {
          this.FirstTimeGetInfoFinish = true
          return this.shipSelect({ currentTarget: {dataset: {index: 2} }})
        }
...
function getSpecNumMap(type="") {
  ...
  if ((select_goods.select_color_id && select_goods.select_size_id) || (select_goods.select_color_id && goodsExtend.attr_count == 1)) {
    //选完
    console.log('选完',goodsExtend.attr_count,select_goods);
    ...
    console.log('未选完',goodsExtend.attr_count,select_goods);
    let canSelectSpec = []; 
    ...
    if (select_goods.select_color_id) { //选颜色就刷新size_arr
      this.setData({
        size_arr: size_arr
      })
    } else if (select_goods.select_size_id) { //选尺码就刷新color_arr
      this.setData({
        color_arr: color_arr
      })
    }
...
/**
 * 检查是否需要自动选择商品
 * @param {Object} ops 页面的参数
 */
function checkAutoChoose(ops) {
  // 如果页面传参有product_id 或者 product_sn  且第一次
  if (!this.firstTimeAutoChoose && ops.product_id || ops.product_sn) {
    this.firstTimeAutoChoose = true;
    // 已到销售时间 且 不是赠品 则自动选择商品
    this.data.saleStatus && this.data.goods_info.sale_type != 99 && this.showStylePop({ detail: { buy_type: ops.buy_type || "", product_id: ops.product_id, product_sn: ops.product_sn }});
  }
}
/**
 * 自动选择指定商品(传入任意一个)
...
function createShoppingCart(isCheckStore=false) {
  ...
  return app.BuyApi.createBuyCarInsert({
    data: {
      ...
      "related_id": this.options.source && Number(this.options.related_id) || 0
    },
    other: {
      isShowLoad: true
    }
    ...
    if (e.code == 1) {
      ...
      this.pageDialog.setSingleBtn("确认");
  ...
  return app.GoodsApi.get_GoodsPackageList({
    ...
    if (res.code == 1) {
      ...
      if (dataList.length >= 1) {
        ...
        goodsPackageDisabledNormalBuy().then(value => {
          this.setData({
            packageDisabledNormalBuy: value || 0,
            saleStatus: value == 1 ? false : true
          })
        })
      }
      this.setData({
...
function goodsPackageDisabledNormalBuy(){ // 是否开启"搭配套餐不允许普通购买"
  return app.sysTemConfig("goods_package_disabled_normal_buy")
    .then(e => {
      return Promise.resolve(e.Value);
    })
    .catch(err => {
      console.log("获取搭配套餐不允许普通购买配置失败, 原因", err);
...
function showPriceExplain() {
  return app.GoodsApi.getGoodsDetailBottomImg({
    params: {
      brandCode: app.Conf.BRAND_CODE,
      goodsId: this.options.goods_id
    }
  }).then(res => {
    console.log("商品价格解释图: ", res);
    let priceExplain = res && res.data && res.data.imgUrl || "";
    this.setData({priceExplain});
  })
}
...
function checkStaffApply(){
  // if(app.globalData.staffApplyWd)return Promise.reject();
  return app.LM.checkIfStaffDstbEvent(true).then(res => {
    if(res && (!res.isStaffDstbData && res.enableWholeDstb)){ 
      return {applyDstbDesc:res.applyDstbDesc ||"",applyDstbHint:res.applyDstbHint ||""}
    }else{
      return Promise.reject();
    }
  })
...
function checkIsCanPop(){
  let goods_info = this.data.goods_info||{};
  if(goods_info.not_allow_buy_type==1){
    return false;
  }
  return true;
}
...
function getGoodsDimensionPlan(goodsId){
  return app.GoodsApi.get_Goods_DimensionPlan({
    params:{
      goodsId,
      brandCode:app.Conf.BRAND_CODE
    }
