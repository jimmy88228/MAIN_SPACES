...
Component(app.BTAB({
  ...
  properties: {
    ...
    isHideAssist:{ // 隐藏 助力活动组件
      type:Boolean,
      value:false
    },
    isHideAssistancePop: { // 隐藏 助力提示弹窗组件
      type: Boolean,
      value: false
    },
    isHideAdsPop: { // 隐藏 广告弹窗组件
      type: Boolean,
      value: false
    },
    isHideContact: { // 隐藏 客服组件
      type: Boolean,
      value: false
    },
    isHideShare:{ // 隐藏 分享组件
    ...
    isHidePageHome: { // 隐藏 回到首页组件
      type: Boolean,
      value: false
    },
    isHideAllFixedBtn: { // 隐藏 回到顶部组件（名称未改）
      type:Boolean,
      value:false
    },
  ...
  return app[apiPack.api][apiPack.url]({
    ...
    if (e.code == 1) {
      ...
      if (!showNav) { //不是tab页面
        ...
        showType = this.data.showType || setting.type||"top"
        isTabPage = true;
      }
      ...
      this.cur_name = "custom" + this.currentId;
      this[this.cur_name] = this[this.cur_name] || this.selectComponent("#" + this.cur_name); 
      shareMsgSaving.call(this,shareInfo)
      this.setAdsPop();
      this.setData({
        navList,
        pageStyleConfig,
...
function shareMsgSaving(data) {
  ...
  if (data.wxShareTitle && data.wxShareImage) {
    shareConf = {
      cfg_title: data.wxShareTitle,
      cfg_pic: data.wxShareImage,
