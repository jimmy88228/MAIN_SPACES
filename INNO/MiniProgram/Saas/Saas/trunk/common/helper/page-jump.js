...
import { ActionRoute, ActionName, LogMap } from "../manager/log-map.js"
import WxGH from "./handle/wxGroupHandle.js";
import ChannelsLiveH from "./handle/channelsLiveHandle.js";
import WxSub from "./handle/wxSubscribe"
import WxApi from "./wx-api-helper.js";
import StringUtl from "../support/utils/string-util"
...
export const FuncType = {
  ...
  contactStaffUrl: { // 联系店员
    type: "contactStaffUrl",
    action: "contactStaff"
  },
  goodsBrandUrl: { //品牌
    type: "goodsBrandUrl",
...
function PageJump(params) {
  ...
  // 针对新后台小程序跳转
  code = code == 'appletJump' ? code.toUpperCase() : code;
  switch (code) {
    // 新微页面
    case FuncType.qmenu.type:
      if (obj.sn == 'shopHome') {
        ...
        url = '/pages/micro_mall/point/point_goods_list/point_goods_list'
      } else if (obj.sn == '11') {
    ...
    case FuncType.couponUrl.type:
      ...
      postUserReceiveBonus.call(this,{
        related_id: obj.id,
        page_id: obj.page_id,
        cb:couponCB,
      ...
      break;
    case FuncType.contactStaffUrl.type:
      // 联系店员
      // if (obj.id) { // 指定店员
      //   actionParams = '&plugid=' + obj.id;

      // }
      page.outsideContainer = page.outsideContainer || page.selectComponent("#outsideContainer");
      let contactStaffGuide = page.outsideContainer && page.outsideContainer.getCompnent("contactStaffGuide");
      contactStaffGuide && contactStaffGuide.initData();
      break;
    case FuncType.goodsBrandUrl.type:
      //秒杀分组模块
      actionParams = '&brand_ids=' + obj.id
      url = '/pages/micro_mall/category/category?func_type=SE' + actionParams
      break;
    ...
    case FuncType.VC.type:
      if (obj.goods_id) {
        action = "action1"
        actionParams = 'related_id=' + obj.related_id + '&goods_id=' + obj.goods_id;
        url = '/pages/micro_mall/goods/goods_info?goods_id=' + obj.goods_id + '&color_id=' + (obj.color_id || "") + '&issued_id=' + (obj.issue_id || "")
      } else {
        action = "action2"
        actionParams = 'related_id=' + obj.related_id;
        url = '/pages/micro_mall/category/category?func_type=' + obj.func_type + '&related_id=' + obj.related_id
    ...
    case FuncType.GOOD.type:
      ...
      url = '/pages/micro_mall/goods/goods_info?goods_id=' + (obj.related_id || obj.goods_id || 0) + '&color_id=' + (obj.color_id || "") + '&issued_id=' + (obj.issue_id || "")
      break;
    ...
    case FuncType.APPLETJUMP.type:
      let appId = obj.appId || obj.name || 0; //新旧兼容
      let path = obj.applet_path || obj.sn || ""; //新旧兼容
...
function addActionLogHandle(obj, page, action, actionParams,type){
  let name = (obj.name && ActionName[obj.name]) || obj.ActionName || "";
  ...
  try {params.link_url && (params.link_url = decodeURIComponent(params.link_url))} catch (error) {} // 将链接decode
    ...
    .then((wxGroupType) => {
      UserApi.postUserReceiveBonus({
        data: {
          ...
          wxGroupType, //  0是一般情况， 1是让这个接口不用再校验是否在群里了，因为getUserIsGroupChatWeChat已经验证过
        },
        other: {
          isShowLoad: true
        }
...
function toMiniProgram(appId,path,envVersion){
  console.log("小程序跳转: 方法参数", arguments)
  let p = new Promise(resolve => resolve(path))
  if ((Conf.BRAND_CODE === "MRJ"|| Conf.BRAND_CODE === "MEIR") && appId === "wx45a10947e1c36f49") { // 品牌: 美人计 要求跳转到该appId带上特定参数
    p = handleNavToSpecMP(path)
  }   

  return p
    .then(targetPath => {
      return WxApi.navigateToMiniProgram({appId, path: targetPath, envVersion: envVersion || "release"})
    })
    .catch(err => {
      console.log("navigateToMiniProgram: fail, 原因: ", err);
      SMH.showToast({title: err.message || (err.errMsg && err.errMsg.indexOf('cancel') == -1 && "跳转失败" || err)})
    })
}

function handleNavToSpecMP(path){
  return LM.checkIfStore()
    .then((storeStaffInfo = {}) => {
      let {staff_type, staff_code: salesmanNo, store_code: agentNo, staff_mobile: salesmanMobile} = storeStaffInfo || LM.storeInfo || {};
      if (staff_type == 1) { // 用户是店长
        let appendedUrl = `salesmanNo=${salesmanNo}&salesmanMobile=${salesmanMobile}&agentNo=${agentNo}&brand=${Conf.BRAND_CODE === "MRJ"?1:2}`;
        path += ((path.indexOf('?') >= 0 ? '&' : '?') + appendedUrl)
        console.log("跳转特定appId的路径: ", path)
        return Promise.resolve(path)
      }
      return Promise.reject(new Error("当前用户暂无访问权限"))
    })
}
...
let couponCB = (relatedId,curStatus)=>{
  if(curStatus != 'register'){
    ...
    WxSub.subscribeGlobal({...BonusInfo,relatedId});
  }else{
