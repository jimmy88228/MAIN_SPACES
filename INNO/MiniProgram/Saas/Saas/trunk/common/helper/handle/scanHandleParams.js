...
class ScanParamsManager {
  ...
  getParams(keys = []){
    return new Promise((rs, rj) => {
      ...
      //console.log('SceneParamsChange listen')
      this.sceneParamsChangeId = EB.listen("SceneParamsChange", () => {
        //console.log('SceneParamsChange listen call')
        let paramsJson = PH.paramsJson();
        if (keys instanceof Array) {
          for (let i = 0; i < keys.length; i++) {
            let key = keys[i];
  ...
  scanActionAnalyse(result,pageDialog,type="goods"){
    if(typeof(result) == "string"){
      ...
      if(type == 'goods'){
        if(result.goods_id){
          wx.navigateTo({
            url: `/pages/micro_mall/goods/goods_info?goods_id=${result.goods_id||0}&color_id=${result.color_id||0}&product_id=${result.product_id||0}`,
          })
        }
      }
    }
...
function getGoodsScanInfo(result,pageDialog){
  return GoodsApi.getGoodsScanInfo({
    ...
    if(res.code == 1){
      ...
      if(data){
        wx.navigateTo({
          url: `/pages/micro_mall/goods/goods_info?goods_id=${data.goodsId||0}&color_id=${data.colorId||0}&product_id=${data.productId||0}`,
        })
      }
      return Promise.resolve(res);
    }
    if(pageDialog){
      let msg = res && res.msg && res.msg || "";
      let title = (msg.indexOf('不存在') != -1 || !msg) ? "条码无效或无法识别，请重新扫码" : msg
      pageDialog.setTitle(title);
