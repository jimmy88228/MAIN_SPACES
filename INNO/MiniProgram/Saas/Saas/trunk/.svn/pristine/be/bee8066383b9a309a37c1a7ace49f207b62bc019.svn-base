...
class ParseArea {
  ...
  static init() {
    for (const code in getAreaData().province_list) {
      const province = getAreaData().province_list[code] && getAreaData().province_list[code].region_name || "";
    ...
    for (const code in getAreaData().city_list) {
      const city = getAreaData().city_list[code] && getAreaData().city_list[code].region_name || "";
      if (city.length > 2) {
        ParseArea.CityShort[code] = CityKeys.reduce((v, key) => v.replace(key, ''), city);
      }
    }
    for (const code in getAreaData().area_list) {
      let area = getAreaData().area_list[code] && getAreaData().area_list[code].region_name || "";
  ...
  static parseByProvince(addressBase) {
    const province_list = getAreaData().province_list;
    const results = [];
    const result = {
      province: '',
      city: '',
    ...
    for (const code in province_list) {
      const province = province_list[code] && province_list[code].region_name || "";
  ...
  static parseByCity(addressBase) {
    const city_list = getAreaData().city_list;
    const results = [];
    const result = {
      province: '',
      city: '',
    ...
    for (const code in city_list) {
      const city = city_list[code] && city_list[code].region_name || "";
  ...
  static parseByArea(addressBase) {
    const area_list = getAreaData().area_list;
    const results = [];
    const result = {
      province: '',
      city: '',
    ...
    for (const code in area_list) {
      const area = area_list[code] && area_list[code].region_name || "";
