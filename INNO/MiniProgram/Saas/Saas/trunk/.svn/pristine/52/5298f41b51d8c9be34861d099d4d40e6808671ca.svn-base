...
Page(app.BP({
  ...
  onLoad: function (options) {
    ...
    this.setData({
      ...
      is_minishop: app.PH && app.PH.isfromSceneChannel("VIDEOLIVE") ? true : false,
    })
  ...
  onUnload() {
    ...
    this.data.shipping && this.data.shipping.way_id == 2 && app.StorageH.remove("select_store");
  ...
  onReady(){
    ...
    app.sysTemConfig("is_erp_point").then(data => {
      this.setData({
        is_erp_point:data.Value||0
      })
    })
    // 现金券限制使用数量
    app.sysTemConfig("cashout_coupon_use_limit").then(data => {
      this.cashCouponUseLimit = data && data.Value || 0
    })
    onShowFnc.call(this);
  },
  /**
  ...
  getCheckOut: function (type) {
    ...
    this.reqData.staffId = this.curStaffId = store_data.staff_id || this.curStaffId || 0;
  ...
  assignData: function (return_data, type) {
    ...
    let isUsePromote = this.data.isUsePromote || 0, isUseSecPromote = this.data.isUseSecPromote || 0;
    ...
    if (this.reqData.isUseSecPromote == 1 && sec_promote.length > 0) {
      isUseSecPromote = 1;
    } else {
      isUseSecPromote = 0;
    ...
    // 储值 默认勾选
    if (return_data.stored_value_entity) {
      let canuse_storedvalue = (return_data.stored_value_entity && return_data.stored_value_entity.canuse_storedvalue) || 0;
      if(canuse_storedvalue && return_data.stored_value_entity.sel_store_value){
        this.setData({
          use_prepaid: 1
        })
    ...
    // IOS上，选择积分，再勾选储值，可抵用WXML无法再次渲染，先清空，在赋值
    this.setData({
      prepaid_data: {}
    })
    this.setData({
    ...
    //存在用户操作
    console.log("检测是否存在用户操作", hasOtherOperate.call(this));
    if (hasOtherOperate.call(this)){
      ...
      // else if(return_data.stored_value_entity.sel_store_value){
      //   console.log("进入");
      //   this.setData({
      //     use_prepaid: 1
      //   })
  ...
  useCashTicket: function () {
    ...
    const cashCouponUseLimit = this.cashCouponUseLimit; // 现金券限制数量，0为无限制
    if (cashCouponUseLimit && useCashCoupon.length && useCashCoupon.length == cashCouponUseLimit) return this.setData({ 
      'cashCoupon_data.error': `只能用${cashCouponUseLimit}张，若需要使用其他现金券，请删除后再添加使用`,
      cashCoupon_input: "",
    })
    ...
    return app.UserApi.getBalanceCheckCoupon({
      ...
      if (e.code == "1") {
        ...
        let usecashCouponsMoney = 0;
        for (let j = 0; j < useCashCoupon.length; j++) {
          //总价
          usecashCouponsMoney += parseFloat(useCashCoupon[j].couponMoney);
          if (useCashCouponIds) {
        ...
        pay_data.usecashCouponsMoney = usecashCouponsMoney;
  ...
  delUseCashCoupon: function (e) {
    ...
    for (let i = 0; i < useCashCoupon.length; i++) {
      if (useCashCoupon[i].couponId == coupon_id) {
        ...
        usecashCouponsMoney += parseFloat(useCashCoupon[i].couponMoney);
        if (useCashCouponIds) {
  ...
  order_createOrder: function () {
    ...
    if(app.Conf.LiveType == 'channels'){
      params.is_minishop = app.PH && app.PH.isfromSceneChannel("VIDEOLIVE") ? 1 : 0;
    }

    this.params = params;
    let tip = "",callBack=null;
    let valetInfo = this.data.valetInfo || {},
      isValetOrder = !!valetInfo.userToken, isUseCashPay = params.pay_mode.pay_code === "cashpay";
    if ((params.shipping.way_id == 2 && (!params.address_data.address_id || params.address_data.address_id == 0)) && (!isValetOrder || isUseCashPay) ){ // 校验快递配送的收货地址，（代客订单除了现金支付都不用校验)
        tip = "请填写收货地址信息";
  ...
  getvaletinfo(e) {
    ...
    if (detail.userToken) {
      ...
      storeData.contact = detail.realName || "自提会员";
      storeData.mob_phone = detail.mobilePhone || "";
      storeData.select_store = select_store;
      app.StorageH.set("store_data", storeData);
    } else {
  ...
  checkValetUse(){
    ...
    if (!valetInfo.mobilePhone) {
      app.SMH.showToast({title: "该用户暂未绑定手机号"})
      return 
    }
    let valetOrderInfo = this.data.valetOrderInfo || {};
    ...
    if (valetOrderInfo.needSms && valetOrderInfo.isSmsChecked == 0){
      this.ValetModule && this.ValetModule.valetPhoneSms(valetInfo, valetOrderInfo.nextSendSeconds,ValetSmsType['asset']);
    }
  },
  checkValetOrder(e){
    console.log("检测完毕", e);
  ...
  selectPay(e){
    ...
    if(paymentList[index].pay_code == 'cashpay'){
      ...
      if(valetOrderInfo.canUseCashPay == 1 && valetOrderInfo.isUseCashPayChecked == 1){
        ...
        let valetInfo = this.valetInfo || {};
        let staffMobilePhone = app.LM.userInfo && app.LM.userInfo.mobilePhone || "";
        this.ValetModule && this.ValetModule.valetPhonePaySms({...valetInfo, staffMobilePhone}, valetOrderInfo.useCashPayNexSmsSeconds,ValetSmsType['pay']);
      }
    }else{
      this.reqData.paymentId = paymentList[index] && paymentList[index].pay_id || 0;
    }
...
function goPay(params, identity, isForGetGiftInventory) {
  ...
  if(app.Conf.LiveType == 'channels'){
    obj.is_minishop = params.is_minishop || 0
  }
  if (identity) {
  ...
  try {
    if(app.PH.paramsJson('gdt_vid')){
      obj.clickId = app.PH.paramsJson('gdt_vid');
    }
  } catch (error) {}
  ...
  return app.BuyApi.addNewOrderInfo({
    ...
    if (e.code == "1") {
      ...
      if (data.OrderId && data.OrderId != "0") {
        ...
        // 订阅消息
        wxSubscribe.subscribeGlobal({...this.subConfig, relatedId: data.OrderId}).finally(res=>{
          ...
          if (identity && this.normalGoods) {
            ...
            this.pageDialog.setSingleBtn({
              ...
              tap: function () {
                ...
                wx.redirectTo({
                  url: `../order/order_info?first_time_topay=${(this.valetInfo && this.valetInfo.userToken) ? 0 : 1}&order_id=${data.OrderId}`,
                })
              }
            })
            this.clickDisable = false;
  ...
  let successBackFn = function () {
    WxApi.getLocation({
      ...
      if (!that.loc_checked && !that.jumpAlready) {
        ...
        let shipping = that.data.shipping||{};
        let store_id = shipping.store_id;
        that.loc_checked = true;
        that.showModalMsg = !!!store_id;
        let reqParams = {
          latitude: -1,
          longitude: -1,
        }
