...
Page(app.BP({
  ...
  assembleOptions(){
    ...
    let cartUniqueKey = this.options.cartUniqueKey;
    console.log("PDA options", this.options);
    if (scene) {
      app.SHP.getParams(["cartUniqueKey"]).then((params) => {
        this.options = {
          ...this.options,
          ...params
        }
        this.reqData.sessionKey = params.cartUniqueKey;
        this.setData({
          options: this.options
        })
        this.getCheckOut();
      })
    } else if(cartUniqueKey){
      this.reqData.sessionKey = cartUniqueKey;
      this.getCheckOut();
    }
  },
  ...
  beforeUseCachTicket() {
    this.useCashTicket();
    // let hasOther = hasOtherOperate.call(this, "cashCoupon", false);
    // if (!hasOther) {
    //   this.useCashTicket();
    //   return;
    // }
    ...
    // this.pageDialog.setTitle("温馨提示");
    // this.pageDialog.setTouchCancel(false);
    // this.pageDialog.setCentent("使用现金券会清空之前的勾选操作，是否继续？");
...
function goPay(params, identity, isForGetGiftInventory) {
  ...
  let obj = {
    ...
    "sessionKey": this.reqData.sessionKey
  }
        ...
        if (identity && this.normalGoods) {
          ...
          this.pageDialog.setSingleBtn({
            ...
            tap: function () {
              ...
              wx.redirectTo({
                url: '/pages/micro_mall/order/order_info?buyType=PDA&first_time_topay=1&order_id=' + data.OrderId,
              })
            }
          })
