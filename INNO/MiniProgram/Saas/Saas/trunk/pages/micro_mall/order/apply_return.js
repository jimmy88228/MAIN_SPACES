...
Page(app.BP({
    ...
    confirmReturn: function () {
        ...
        return app.BuyApi[url]({
            ...
            if (e.code == "1") {
                // 退货后 调起订阅消息授权
                formData.return_type == 0 && wxSubscribe.subscribeGlobal({...this.subConfig, relatedId: e.data || 0});
                wx.nextTick(() => {
                  wx.showModal({
                    title: "提示",
...
function getReturnApplyDetail(options = {}) {
    return app.BuyApi.getALLOrderEntity({
        params: {
            ...
            brandCode: app.Conf.BRAND_CODE,
            userToken:app.LM.userToken,
            ...getOrderInfoParams({order_id: options.order_id})
        }, other: { isShowLoad: true }
    }).then(e => {
        if (e.code == "1") {
            let data = e.data;
      ...
      app.SMH.showToast({title: err && err.msg || "获取订单信息失败"});
