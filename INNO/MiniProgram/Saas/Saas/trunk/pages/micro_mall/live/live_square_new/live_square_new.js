...
Page(app.BP({
    ...
    if (typeof e.currentTarget.dataset.id != 'undefined'){
      ...
      if(roomId){
        ...
        if(e.currentTarget.dataset.status){
          ...
          wx.navigateTo({
            url: '/pages/micro_mall/live/live_detail/live_detail?id=' + roomId
          })
        }
      }
    }
  ...
  onShareAppMessage: function () {
    return {
      title: '直播广场',
      imageUrl: 'http://mmbiz.qpic.cn/mmbiz_jpg/JiaQtYiay7icxO6XPVHHX5RYJ9kcvQhtgsX557AE5s2UnwKXCpaNSb4djLAb4ZNVPwaVl2ozkZRL3gDdgzrwiaRPyg/0'
    }
  },
  //订阅下一场直播
  subscribeNextLive(){
...
function getLiveRoomList(){
  ...
  return app.RunApi.go('LiveApi', 'getLiveRoomList', params, extra).then(res => {
    if (res.code == '1') {
      ...
      let nextRoomInfo = this.data.nextRoomInfo;
      let livingRoom = this.data.livingRoom
      let isLiving = this.data.isLiving
      for(let i = 0; i < data.length; i++){
        ...
        //即将直播的房间
        if(roomId == this.data.nextRoomId){
           nextRoomInfo = data[i]
        }
        //正在直播的房间信息
        if(data[i].liveStatus == 101){
          livingRoom[roomId] = data[i]
          if(isLiving == false){
            isLiving = true
          }
        }
