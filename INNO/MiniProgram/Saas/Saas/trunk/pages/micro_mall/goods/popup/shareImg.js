...
function adaptiveDrawCanvas(canvasId = '', res = [], multiple = 1, callback) {
  ...
  res && res.forEach((item, index) => {
    ...
    //图片
      ...
      }else if (item.mode == 'aspectFit2') {
        try{
          let {width: modifiedImageWidth, height: modifiedImageHeight} = aspectFitImg(downMsg.width,downMsg.height,item.w,item.h);
          let absTranslateX = parseFloat(item.w - modifiedImageWidth === 0 ? item.x : (Math.abs(item.w - modifiedImageWidth) / 2) + item.x).toFixed(3);
          let absTranslateY = parseFloat(item.h - modifiedImageHeight === 0 ? item.y : (Math.abs(item.h - modifiedImageHeight) / 2) + item.y).toFixed(3);
          downMsg.path && ctx.drawImage(downMsg.path, absTranslateX * baseW, absTranslateY * baseW , modifiedImageWidth * baseW, modifiedImageHeight * baseW);
        }catch(err){
          console.log("aspectFit2计算报错,原因:", err);
...
function codeInitDraw(ctx, baseW, res, baseInfo, canvasId) {
  ...
  if (!baseInfo.codeDiy && info.gen_barcode_style !=2 && (info.opKind != "STAFF_ACTIVITY" || info.opKind == "STAFF_ACTIVITY" && (this.saveActive == 0 || info.gen_barcode_style == 1))) {
    ...
    let tipText = draw && draw.codeType == 'QR' ? '扫码注册会员' : '长按识别get同款'
...
function staffCodeHandle(){//根据配置读取staff_code展示的内容
  ...
  let dstbStaffName = app.LM.staffInfo.dstbStaffName || "";
  ...
  if (staffCode) {
    if(Identity_Conf && Identity_Conf.cfg_pic == 'none'){
      ...
      staffCode = dstbStaffName || "";
    } else if (Identity_Conf && Identity_Conf.cfg_pic == 'code'){
      staffCode = staffCode
    } else if (Identity_Conf && Identity_Conf.cfg_pic == 'nick_name') {
      staffCode = this.data.realName;
    }
...
function checkInfo(data = {}) {
  ...
  if (this.staffInfo.staffCode && this.staffInfo.staffCode != data.scene.staffCode) {
    data.scene.staffCode = this.staffInfo.staffCode;
  }
  if (this.storeInfo.staff_code && this.storeInfo.staff_code != data.scene.staff_code) {
...
function initTemplate(allData = {}) {
  ...
  else if (draw && draw.template == 'grass') { // 种草文章
    return checkShareConf.call(this, "grass").then(check_res => {
      allData.info = allData.info || {};
      allData.info.ShareConf = this.ShareConf || {};
      return DrawTemplate.initArticleTemplate(allData).then(res => {
        return Promise.resolve(res || []);
      })
    })
  }
