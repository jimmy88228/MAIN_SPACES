...
Page(app.BP({
  ...
  confirmBuy() {
    ...
    setLoading.call(this, 3000);
    let activity_info = this.data.activity_info;
    let activityId = activity_info.activityId || 0;
    let params = {
      activityId,
      staffId: app.PH && (typeof app.PH.paramsJson === "function") && app.PH.paramsJson("store_staff_id") || 0, // 店员staff_id
    }
    if (!this.order_id) {
...
function goPay(order_id = '') {
  return PayH.UnifiedorderByOrderId({type:"buyb",order_id})
  .then(res => {
    this.buyPopAnimate();
    this.order_id = 0;
