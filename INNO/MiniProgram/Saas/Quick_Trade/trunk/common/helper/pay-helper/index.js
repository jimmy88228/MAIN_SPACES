...
import {
  PayType
} from "../../manager/log-manager/index";
import Conf from "../../../config/index";
import WxApi from "../../utils/wxapi/index";
import DateUtil from '../../utils/date/index'
import retainSessionH from '../retain-session-handler/index';
class payManager {
  static getInstance() {
  ...
  UnifiedorderByOrderId(type, orderId, isNeedUser = true, checkMod = false, curOrderInfo = {}) {
    ...
    return getPaymentInfoHandle.call(this, {
      type,
      orderId,
      isNeedUser
    }).then(e => {
...
const Fnc = {
  ...
  requestOrderPayment(e, curOrderInfo) {
    ...
    console.log('requestOrderPayment params', {
      ...params
    });
    // return WxApi.requestOrderPayment({...params})
    return WxApi.requestOrderPayment({
      ...params
    }).then((res) => {
      console.log("支付回调", res);
