      ...
      <page-top isSave @save="saveLabelEvent">
          <Form class="add-label-page" :model="formData" :rules="ruleValidate" ref="lableForm">
            ...
            <FormItem label="自动标签" prop="is_auto_label" size="large" :label-width="100" v-if="formData.is_auto_label == 1">
                <dataSelect defaultTime="" :customDate="execTimeRange" @sT="(date)=>getDate(date,'exec_start_time')" @eT="(date)=>getDate(date,'exec_stop_time')"></dataSelect>
                <div class="flex m-bottom-25 fs-0" style="width:100%;" >
                    <FormItem prop="exec_start_time" class="margin-0" style="width:180px;height:1px"><Input v-show="false" v-model="formData.exec_start_time" /></FormItem>
                    <span class="hold_stay" style="width:34px;"></span>
                    <FormItem prop="exec_stop_time" class="margin-0" style="width:180px;height:1px"><Input v-show="false" v-model="formData.exec_stop_time" /></FormItem>
                </div>
            </FormItem>
            <FormItem label="标签规则" :label-width="100">
              ...
              <div class="operate-module">
                  <div class="flex operate-area f-just-between">
					...
					<!-- <Table :data="temData" :columns="temColumns" :span-method="handleSpan"></Table> -->
...
export default {
    ...
    return {
        ...
        ruleValidate: {
            ...
            c_sex: [{required: true, message: '请选择性别', trigger: 'change'}],
						exec_start_time: [{required: true, message: '请选择自动标签开始时间', trigger: 'change' }],
						exec_stop_time: [{required: true, message: '请选择自动标签结束时间', trigger: 'change' }],
  ...
  methods: {
			...
			// 	let productL = row.product.length;
			// 	console.log("rowIndex:", rowIndex, "columnIndex:", columnIndex, "productL:", productL);
			// 	if(typeof(this.spanKey[row.goods_id]) == "undefined") {
			// 		this.spanKey[row.goods_id] = rowIndex;
			// 	}
			// 	if(columnIndex < 2){
			// 		console.log("spanKey", this.spanKey, "----",rowIndex);
			// 		if(this.spanKey[row.goods_id] == rowIndex){
			// 			return [productL, 1];
			// 		} else {
			// 			return [0, 0]
			// 		}
      ...
      getDate(date, type){
				console.log("sdsd", date);
					this.$set(this.formData, type, date);
      },
