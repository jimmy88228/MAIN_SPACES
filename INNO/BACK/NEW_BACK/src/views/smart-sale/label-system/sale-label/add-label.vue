      ...
      <page-top isSave @save="saveLabelEvent">
          <Form class="add-label-page" :model="formData" :rules="ruleValidate" ref="lableForm">
            ...
            <FormItem label="自动标签" prop="is_auto_label" size="large" :label-width="100" v-if="formData.is_auto_label == 1">
                <dataSelect :customDate="execTimeRange" defaultTime="" @sT="(date)=>getDate(date,'exec_start_time')" @eT="(date)=>getDate(date,'exec_stop_time')"></dataSelect>
...
export default {
  ...
  methods: {
      ...
      getLabelInfo(){
        ...
        return util.ajax.post(util.apiUrl.SaleLabelInfo, {
            ...
            if(res.code) {
               ...
               return this.getFormDataKey(formData);        
            }
        }).finally(()=>{
            this.showSpan = false
        })
      ...
      addLabelEvent(params){
        this.showSpan = true;
        return util.ajax.post(util.apiUrl.SaleLabelAdd,params).then(e =>{
            let res = e.data || {};
            if(res.code && res.data) {
