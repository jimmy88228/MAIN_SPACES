<template>
	<div>
			...
			<div class="flex f-just-between m-bottom-15">
				...
				<div class="space-nowrap flex">
					<div v-if="type == 'manual'">
						...
						<Poptip confirm placement="bottom" title="确定删除选中的会员吗?" @on-ok="removeUser">
							<Button type="error">删除选中的会员</Button>&nbsp;&nbsp;
						</Poptip>
					</div>
					<Input class="" style="width:250px" v-model="formValidate.searchq" placeholder="会员号、手机号、逗号隔开可以搜索多个"
						clearable search enter-button @on-search="search" @on-clear="search"
		...
		<!--任务提示-->
		<Modal v-model="taskModel" @on-ok="sendMarketing">
