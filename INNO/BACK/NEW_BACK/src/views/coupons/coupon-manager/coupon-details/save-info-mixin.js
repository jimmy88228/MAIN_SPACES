					...
					if (basicValid && extendValid) {
						...
						return this.$ajax.post(this.$route.params.id ? (this.isEditStaus ? this.$api.couponsEdit : this.$api.couponsCopy) :
								this.$api.couponsAdd, {
                            		...
                            		invalid_time_show_code: this.formItem.validityTime.invalid_time_show_code,//券冻结期间前端不隐藏优惠券号
									...
									wxgroup_select: this.extendFormItem.wxgroupSelect.length === 0 ? '' : this.extendFormItem.wxgroupSelect.map(
										item => item.id).join(),
                                    qwgroup_select: this.extendFormItem.qwgroupSelect.length === 0 ? '' : this.extendFormItem.qwgroupSelect.map(
										item => item.id).join(),
                            		setting_value: this.extendFormItem.settingValue,
                            		...
                            		platform_type_id: this.extendFormItem.platformTypeId,
