<template>
  <div class="goods-size-explain-cat">
    <Card>
      <Row>
        <Col span="12">
        ...
        <Col span="12" class="btn-group">
          ...
          <Button type="primary" icon="md-add" @click="create" v-if="canCreate.add">添加尺码图分类</Button>
...
<script>
import SearchForm from './search-form-size-explain-cat';
import Mixin from './mixin-size-explain-cat.js';
...
export default {
  ...
  methods: {
    onLoadData (page, data) {
      ...
      return this.$ajax.post(this.$api.goodsSizeExplainCat, params)
      .then(response => {
    ...
    onDelItem (row) {
      return this.$ajax.post(this.$api.goodsSizeExplainCatDel, {
        id: row.id
      });
    },
    handleRecord () {
