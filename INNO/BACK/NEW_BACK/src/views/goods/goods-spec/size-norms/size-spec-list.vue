...
export default {
  ...
  methods: {
    ...
    onLoadData (page, data) {
      return this.$ajax.post(this.$api.goodsSizeValueList, {
        sizecat_id: this.id,
    ...
    onDelItem (row) {
      return this.$ajax.post(this.$api.goodsSizeValueRemove, {
        size_id: row.size_id
      });
    },
    onAdd () {
    ...
    handleSynchronization (arr) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.spinShow = true;
          return this.$ajax.post(this.$api.sizeSysnchroAttr, {
            size_ids: arr
          })
          .then((response) => {
