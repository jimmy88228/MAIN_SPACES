		...
		<template>
			<div class="goods-cat-inner">
				<Row type="flex">
					...
					<Col style="width:420px;text-align: right;">
						<Button type="info" :loading="exportLoading"  @click="confirmExport">导出分类</Button>
...
<script>
	...
	export default {
		...
		methods: {
			onLoadData(page) {
				this.showSpin = true;
				return this.$ajax.post(this.$api.goodsCatList, {
						page,
						isInit: 1
					})
					.then(response => {
			...
			searchPage() {
				this.showSpin = true;
				return this.$ajax.post(this.$api.goodsCatList, this.formSearch)
					.then((response) => {
						this.showSpin = false;
			...
			// 复制链接
			copyUrl(data) {
				var link = "pages/micro_mall/category/category?func_type=CA&related_id="+data.id+"&strWhere=";
			...
			onDelItem(row) {
				this.showSpin = true;
				return this.$ajax.post(this.$api.goodsCatRemove, {
						cat_id: row.id
					})
					.then((response) => {
						this.showSpin = false;
