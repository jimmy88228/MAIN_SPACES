<template>
  <div class="spec_item">
    <Poptip placement="top-start" v-model="showItem" transfer width="450" popper-class="spec_item_pop">
        <div class="attr_wrapper">
          <Input placeholder="请选择规格" class="spec_input" v-model="formatItem.spec_name" @input="handleChange">
        ...
        <div class="gallery" v-if="attrType === 'color'" @click.stop>
          <div class="gallery_wrapper">
            <div class="gallery_inner" v-if="formatItem.goods_gallery">
              <SlickList v-model="formatItem.goods_gallery" :useDragHandle="true" @input="onPageSortInput($event)" axis="x" class="gallery_list" v-viewer>
                <SlickItem v-for="(item, imgIndex) in formatItem.goods_gallery" :index="imgIndex" :key="imgIndex" class="goods_img_wrapper">
                  <img :src="item.img_url" class="img"/>
                  <Icon type="ios-close-circle-outline" class="close" @click="delImg(imgIndex)" title="删除" />
                  <Icon v-handle class="handle-img" type="md-apps" size="18" title="拖动排序"/>
                </SlickItem>
              </SlickList>
            </div>
        ...
        <div class="sort spec_content_sort" slot="content">
          <div class="sort_content">
            <label>分类名&nbsp;</label>
            <Select v-model="formatItem.cat_id" @on-change="attrIds = [];" style="width: 120px;margin-right: 10px;">
              <Option :value="0">全部</Option>
              <Option v-for="item in originList" :value="item.groupId" :key="item.groupId">{{ item.value }}</Option>
            </Select>
            <label>属性值&nbsp;</label>
						<!-- multiple -->
            <Select v-model="attrIds" class="attr-select" multiple transfer filterable style="width: 200px;">
              <Option :value="0">全部</Option>
...
export default {
  ...
  computed: {
    ...
    originList () {
			console.log("originTree", this.originTree);
  ...
  methods: {
   ...
   //    const regex = /sign/gmi;
			// console.log("attrIds", this.attrIds);
			// console.log("formatItem", this.formatItem);
      ...
      this.showItem = false;
			let addItems = [];
			let formatItem = JSON.parse(JSON.stringify(this.formatItem))
      this.attrList.forEach(item => {
				if(attrIds.indexOf(item.id) != -1){
					let items = {
						cat_id: formatItem.cat_id,
						id: item.id,
						spec_name: this.curItem.id == item.id ? this.curItem.spec_name : item.spec_name,
						spec_code: item.spec_code,
						value: formatItem.value
					};
					if(this.attrType === 'color'){
						items.goods_gallery = item.goods_gallery;
						item.spec_sort = item.spec_sort;
					}
					addItems.push(items)
				}
        ...
        // }
			...
			if(addItems.length > 0){
				this.$emit('selectedChange', addItems, this.curIndex);
			}
    },
    delItem () {
  ...
  watch: {
    curItem: {
      handler (newVal) {
				...
				console.log("formatItem newVal变化", newVal);
        if (newVal) {
          this.formatItem = {...newVal};
					if(newVal.id){
						this.attrIds = [newVal.id];
					}
        }
      },
      immediate: true,
