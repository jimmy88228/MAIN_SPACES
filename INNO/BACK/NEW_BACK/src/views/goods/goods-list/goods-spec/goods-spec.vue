      ...
      <Col :lg="20" :xl="20" :xxl="22">
        <div class="spec_table">
          <div class="spec_table_item">
            <div class="color_header spec_header space-nowrap">
              <span>规格1：</span>
            ...
            <Row class="color_content">
              <!-- <Col class="title space-nowrap">规格值：</Col> -->
              <Col class="spec_table_cont"><!--:lg="20" :xl="20" :xxl="22"-->
                <div class="sort_list">
                  <SlickList v-model="colorList" :useDragHandle="true" @input="onPageSort($event)" axis="y" class="gallery">
                    <SlickItem v-for="(item, index) in colorList" :index="index" :key="item.spec_code" class="goods_img_wrapper">
                      <spec-item
                        :key="item.id"
                        :cur-item="item"
                        :cur-index="index"
          ...
          <div class="spec_table_item spec_size_item">
            <template v-if="allowShow && toggle">
              <div class="size_header spec_header space-nowrap">
                <span>规格2：</span>
                ...
                <Col class="spec_table_cont"><!--:lg="20" :xl="20" :xxl="22"-->
                  <div class="sort_list size_sort_list">
                    <spec-item
						...
						<Icon type="md-close-circle" class="close" size="24" v-if="allowShow && toggle && !isMulti" @click="delSize"/>
          </div>
          <Tooltip :content="toolText">
            <Button @click="createNew" :disabled="!allowShow || (allowShow && toggle)">+双规格</Button>
          </Tooltip>
        </div>
      </Col>
    </Row>
...
export default {
  ...
  methods: {
    ...
    handleColorChange (item, index) {
			console.log("colorList", JSON.parse(JSON.stringify(this.colorList)));
