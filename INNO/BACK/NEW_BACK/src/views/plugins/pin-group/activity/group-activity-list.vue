...
export default {
  ...
  methods: {
    ...
    confirmExportActivity(data){
      let params = Object.assign({}, data, this.condition);
      return this.$ajax.post(this.$api.GroupActivityExport,params).then((response) => {
    ...
    handleImportBatchActivity (row) {
      //批量修改活动
      this.uploadUrl = this.$api.GroupActivityBatchUpload;
      this.$refs.batchImport.openModal(this.canCreate, this.uploadUrl, this.$api.GroupActivityBatchDownload);
    },
    confirmExportActivityGoods(data){
      let params = Object.assign({}, data, this.condition);
      return this.$ajax.post(this.$api.GroupActivityGoodsExport,params).then((response) => {
          var res = response.data;
          if (res.code) {
            var jobId = res.data;
            this.jobIdCol.push(jobId);
    ...
    handleImportActivityGoodsByProductSn (row) {
      this.uploadUrl = this.$api.GroupActivityGoodsUploadByProductSn;
      console.log(this.uploadUrl)
      this.$refs.batchImport.openModal(this.canCreate, this.uploadUrl, this.$api.GroupActivityGoodsDownloadByProductSn);
    },
    onImportSuccess () {
      this.loadData();
    },
