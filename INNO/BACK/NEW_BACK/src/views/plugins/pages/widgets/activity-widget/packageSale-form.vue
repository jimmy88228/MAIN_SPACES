...
<template>
	<div class="package-sale-form">
		<titleBar>搭配套餐组件 设置</titleBar>
		...
		<!--统一活动头-->
		<activityHeaderForm :currIndex="currIndex" headerTitle="搭配套餐"></activityHeaderForm>
		
		<!--商品的选择器-->
		<packageGoodsSelect ref="goods-select" @on-ok="onGoodsSelectOk"></packageGoodsSelect>
	</div>
</template>
				...
				for( var i in obj ){
					if( arrGoodsIds.indexOf(obj[i].id) == -1 ){
						this.formItem.goodsList.push({
							id: obj[i].id,
							goods_id: 0,
							img_url_thumb: obj[i].package_bg_image,
							name: obj[i].package_name,
							sale_price: obj[i].max_sale_price,
							market_price: obj[i].max_sale_price,
						});
					}
				}
			},
