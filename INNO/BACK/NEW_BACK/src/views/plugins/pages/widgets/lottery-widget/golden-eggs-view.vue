	...
	<div class="golden-eggs-view">
		...
		<!-- <template v-if="prizeNum"> -->
			...
			<div class="prize-cont">
				<div class="prize-item" v-for="item in prizeNum" :class="{'middle-item': item == 2}">
					<template v-if="item == 2">
						<img :src="info.activePrize.img" class=""/>
					</template>
					<template v-else>
						<img :src="info.defaultPrize.img" class=""/>
					</template>
				</div>
				<div class="prize-active hammer_wrapper_action">
					<img :src="info.lotteryBtn.img" class=""/>
				</div>
			</div>
		<!-- </template> -->
	...
	export default {
		...
		methods: {
			init() {
				...
				if(!this.info.defaultPrize){
					this.$set(this.info, 'defaultPrize', {
						img: "",
					});
				}
				if(!this.info.activePrize){
					this.$set(this.info, 'activePrize', {
						img: "",
					});
				}
				if(!this.info.lotteryBtn){
					this.$set(this.info, 'lotteryBtn', {
						img: "",
					});
				}
			},
