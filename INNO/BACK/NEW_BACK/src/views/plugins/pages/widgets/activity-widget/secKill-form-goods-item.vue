					 ...
					 v-bind="dragOptions" @start="dragStart" @end="dragEnd" @change="dragChange">
						...
						<div v-for="(item,index) in formItem.goodsList" :name="index" :key="index" class="goods-item-list">
							...
							<Row :gutter="10">
								...
								<div>
									<span v-if="item.view_market_price != item.view_price" 
									style="text-decoration: line-through;">￥{{item.view_market_price}}</span>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 选择商品
			onSelectGoods() {
				this.$refs['goods-select'].openModal([], 'multi', 0 , this.formItem.activity_id );
			},
			// 商品选择的，回调
			onGoodsSelectOk(obj) {
				var arrGoodsIds = [];
