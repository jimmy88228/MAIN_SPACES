	...
	<div class="sec-kill-form">
		...
		<Form ref="formValidate" :model="formItem" :rules="ruleValidate" label-position="top">
			...
			<div class="goods-list-box">
				...
				<div>当前秒杀分组：{{formItem.group_name}}</div>
		...
		methods: {
			init() {
				...
				if ( typeof(this.formItem.activityGroup) == 'undefined' ) {
					...
					this.$set( this.formItem, 'group_id', 0 ); // 这里特殊一些，需要group_id
					this.$set( this.formItem, 'group_name', "" );
				}
			},
			handleSeckillSelect(){
				this.$selectContent({
					mode: 'seckill',
					type: 'checkbox',
					groupId: this.formItem.group_id,
					groupName: this.formItem.group_name,
					data: [],
					getList: (data) => {
						let currActivityId = [];
						...
						data.forEach((item)=>{
							if( this.formItem.group_id == null || this.formItem.group_id == 0 ){
								this.$set(this.formItem, 'group_id', item.group_id );
								this.$set(this.formItem, 'group_name', item.group_name);
							}
			...
			removeGroup(index) {
				this.$Modal.confirm({
					...
					onOk: () => {
						...
						if( this.formItem.activityGroup.length == 0 ){
							this.formItem.group_id = 0;
							this.formItem.group_name = "";
						}
					},
				});
			},
