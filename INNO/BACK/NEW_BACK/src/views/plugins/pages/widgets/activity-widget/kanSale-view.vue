...
<template>
	<div class="act-sale-view kan-sale-view">
		<!--统一活动头-->
				...
				<Row type="flex" style="flex-wrap:wrap;" :gutter="info.gutter">
					...
					:span="info.layout == 'one' ? 24 : (info.layout == 'two' ? 12 : 8)">
						...
						<div class="goods-item" :style="{'margin-bottom': info.gutter + 'px'}">
							<Tag :color="styleColor" class="tag">砍价</Tag>
							...
							<Row type="flex">
								...
								<Col :style="{width: info.layout == 'one' ? '190px' : ( info.layout == 'three' ? '120px' : '110px') }">
									...
									class="btn" :style="{marginLeft: info.layout=='three' ? '8px' : '0'}">
										<div class="tname" :class="info.content_switch.show_time == false ? 'single' :'' ">砍价活动</div>
		...
		<div v-else class="empty">
			<Icon type="ios-cut-outline" size="60" color="#2d8cf0"></Icon>
			<div>请点击编辑 砍价活动</div>
		</div>
	</div>
</template>
		...
		methods: {
			...
			initGoodsList(){
				if( this.info.activity_model == 'auto' ){
					...
					return this.$ajax.post(this.$api.bargainActivityList,{
						status: -1,
						page: 1,
						pageSize: show_num
					}).then((response)=>{
						...
						if(res.code){
							...
							for(let i = 0; i < items.length; i++){
								goodsList.push({
									...
									img_url_thumb: items[i].activity_img,
									name: items[i].activity_name,
									sale_price: items[i].max_market_price,
