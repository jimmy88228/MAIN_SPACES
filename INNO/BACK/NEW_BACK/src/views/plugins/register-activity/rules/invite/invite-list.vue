<template>
  <div class="register-list">
    <Card>
      <div class="note">
        <span>邀请返利：邀请人成功邀请别人注册后获得的返利。</span>
      </div>
      <div class="btn-group">
        <Button type="primary" icon="md-add" @click="openModal">添加规则</Button>
      </div>
      <Table :loading="tableLoading" :height="tableHeight" :columns="tableColumns" :data="tableData" ref="myTable">
        ...
        <template slot-scope="{ row, index }" slot="handle">
          ...
          <span v-show="row.handle.remove" @click="delItem(row, '删除提示', '确定删除该邀请返利吗？')"><a>删除</a></span>
        </template>
      </Table>
    ...
    <invite-form ref="registerForm" @on-success="onLoadData"/>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    onLoadData (page, data) {
      ...
      return this.$ajax.post(this.$api.inviteRebateList, params)
      .then(response => {
    ...
    onDelItem (row) {
      return this.$ajax.post(this.$api.inviteRebateRemove, {
        id: row.id
      });
    }
  }
