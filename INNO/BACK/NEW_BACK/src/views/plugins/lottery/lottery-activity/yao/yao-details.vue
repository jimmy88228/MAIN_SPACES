<template>
  <div class="yao-details">
    <ActivityTemplate type="yao" :id="id" @allow="allowBack" @getPrizeList="handlePrize">
      <template #content>
          ...
          <Table :columns="columns" :data="tableData.data" ref="myTable">
            ...
            <template slot-scope="{ row, index }" slot="content">
              <FormItem
                ...
                <InputNumber :min="0" v-model="row.prizeIntegral" v-show="row.prizesType === 1" @on-change="val => handleSetData('prizeIntegral', index, val)"></InputNumber>
              </FormItem>
              <FormItem
                ...
                <label>%</label>
						...
						<template slot-scope="{ row, index }" slot="everyDayNumber">
						  <FormItem
						    ...
						    <InputNumber :min="0" v-model="row.everyDayNumber" @on-change="val => handleSetData('everyDayNumber', index, val)"></InputNumber>
						  </FormItem>
						</template>
  ...
  methods: {
    addItem () {
      ...
      let newItem = {
        ...
        joinPoolNumber: 0,
        sort: 0,
        pic: require('@rs/images/point.png')
      };
    ...
    setImage (index, type) {
      let defaultImg = '';
      // 匹配到默认图的优先使用/或没有选择
      let isDefault = /(point|bonus|goods|thanks).png/.test(this.tableData.data[index].pic);
      if (isDefault || this.tableData.data[index].pic === '') {
        switch (type) {
          case 1:
            defaultImg = require('@rs/images/point.png');
            break;
          case 2:
            defaultImg = require('@rs/images/bonus.png');
            break;
          case 3:
            defaultImg = require('@rs/images/goods.png');
            break;
          default:
            defaultImg = require('@rs/images/thanks.png');
            break;
        }
    ...
    handlePrize (prize, thanks, imgCol) {
      this.imgCol = imgCol;
      if (prize.length === 0 || Object.keys(thanks).length === 0) return;
