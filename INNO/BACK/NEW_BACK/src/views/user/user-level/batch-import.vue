...
<template>
    <div>
        ...
        <p v-if="file" class="file-name">Upload file: {{file.name}}</p>
        <Button type="info" v-if="file != null" class="upload-btn" :loading="uploadLoading" @click="upload">确定上传</Button>
        <Divider />
        <div class="download-wrapper">
            <span>如需模板，请下载~</span>
            <Button type="text" class="download-btn" :loading="downloadLoading" @click="download">下载模板</Button>
        </div>
...
export default {
  ...
  methods: {
    // 打开模态框
    paseParamModal (rank_id, uploadUrl, downloadUrl) {
