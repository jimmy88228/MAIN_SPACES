	...
	<div class="user-level-list">
		<Card>
			<div class="btn_group_level">
				...
				<Button type="info" icon="md-add" @click="showUserLevelImport" v-if="canCreate.upload">批量修改会员等级</Button>
			</div>
		...
		<userLevelImport ref="userLevelImport"></userLevelImport>
	</div>
</template>
<script>
	...
	import userLevelImport from './user-level-import';
	export default {
		data() {
			return {
				canCreate: {}
		...
		components:{ userLevelImport,BatchImport },
		...
		methods: {
            ...
            showUserLevelImport(){
			    this.$refs["userLevelImport"].showModal();
                this.$refs["userLevelImport"].getLerverData();
			}
		},
		mounted() {
			this.loadData();
