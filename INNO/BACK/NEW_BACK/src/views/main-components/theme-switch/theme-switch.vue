...
export default {
  ...
  methods: {
    setTheme (themeFile) {
      ...
      if (window.localStorage.theme) {
        const themeList = JSON.parse(window.localStorage.theme);
        let index = 0;
        ...
        window.localStorage.theme = JSON.stringify(themeList);
      } else {
  ...
  created () {
    ...
    if (window.localStorage.theme) {
      const hasThisUser = JSON.parse(window.localStorage.theme).some(item => {
