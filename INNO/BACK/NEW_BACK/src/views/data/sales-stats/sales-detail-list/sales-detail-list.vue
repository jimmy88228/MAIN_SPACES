	...
	<Card class="visit-page">
		<div class="flex f-just-between f-align-center m-bottom-10">
			<div class="flex f-align-center">
				日期&nbsp;&nbsp;
				<date-select ref="dateSelect" defaultTime="month" dateType="report" class="inline-b space-nowrap"
					@sT="handleStart" @eT="handleEnd" />
				&nbsp;&nbsp;
				<Input class="" style="width:250px" v-model="formSearch.searchq" placeholder="请输入品牌名" clearable search
					enter-button @on-search="loadData()" @on-clear="loadData()"
		...
		<div v-show="pageTotal" class="list_page">
			<Page :total="pageTotal" :page-size="pageSize" :current="currentPage" :page-size-opts="pageSizeOpts"
				@on-change="e => changePage(e)" @on-page-size-change="ps => handlePageSize(ps)" show-elevator show-total
				show-sizer></Page>
		</div>
		<!--异步处理导出excel组件-->
		<div class="col">
	...
	export default {
		...
		methods: {
			onLoadData(page, data) {
				...
				return util.ajax.post(util.apiUrl.salesDetailList, params).then(e => {
					let res = e.data || {};
					if (res.code) {
						let data = res.data || {};
						...
						for (let i = 0; i < limitBrand.length; i++) {
							for (let j = 0; j < data.list.length; j++) {
								if (limitBrand[i].brandId == data.list[j].brand_id && limitBrand[i].brandName ==
									data.list[j].brand_name) {
			...
			handleExport() {
				this.$Modal.confirm({
					...
					onOk: () => {
						...
						return this.$ajax.post(this.$api.SalesDetailListExport, {
							...formSearch,
							page: 1,
							pageSize: 20
						}).then((response) => {
