...
<script>
import mixin from './coupon-stats-mixin.js';
import notice from '@/views/my-components/mq-notice/mq-notice';
import salesForm from './coupon-stats-form';
...
export default {
	...
	data () {
		return {
			formSearch:{
					...
					end_time: "", //结束时间 默认 7天
					report_start_time: "",  //统计日期 开始时间 默认 
					report_end_time: "", //统计日期 结束时间 默认 
					searchq: "",   //搜索参数
					sort_by: "", //排序字段  默认发放会员数
					sort_order: ""
			},
			showSpin: true,
			brandId:0,
	...
	methods: {
		...
		onLoadData(page, exteData){
			this.showSpin = true;
			return this.$ajax.post(this.$api.couponsStatsList, {
				...this.formSearch,
				...exteData,
		...
		handleExport () {
			this.$Modal.confirm({
				...
				onOk: () => {
					...
					return this.$ajax.post(this.$api.couponsStatsExport,{
						...formSearch,
						brand_id: this.brandId || 0,
					}).then((response) => {
