    ...
    <Form ref="form" :label-width="100" label-colon>
      <Row>
        <Col span="12" class="order_details_item">
          ...
          <FormItem label="原订单号"><p><a @click="handleGoOrderInfo(orderMessage.order_id)">{{orderMessage.order_sn ? orderMessage.order_sn : '--'}}</a></p></FormItem>
          <!-- <FormItem label="物流单号"><p>{{orderMessage.ex_return_no || '--'}}</p></FormItem> -->
          <FormItem label="申请时间"><p>{{orderMessage.create_time}}</p></FormItem>
          <FormItem label="退单类型"><p>{{orderMessage.refund_type_str}}</p></FormItem>
        </Col>
        <Col span="12" class="order_details_item">
          <span class="title title_slot">退货信息</span>
          <FormItem label="退款状态"><p>{{orderMessage.refund_status_str}},{{orderMessage.confirm_status_str}}</p></FormItem>
          <FormItem label="原支付方式"><p>{{orderMessage.pay_name}}</p></FormItem>
          <FormItem label="原支付单号"><p>{{orderMessage.payment_sn || '--'}}</p></FormItem>
