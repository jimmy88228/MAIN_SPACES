	...
	export default {
		...
		methods: {
			...
			init() {
				...
				// ajax 请求获取初始化数据
				this.$ajax.post( this.$api.cloudGoodsPageList, {
						isInit: 1,
			...
			initTable(res) {
				...
				this.columns[(this.columns.length - 1)].render = (h, params) => {
					...
					if (params.row.handle.edit) {
						...
						// 设置默认首页
						if( params.row.is_index == 0 && params.row.is_super_admin ){
							buttons.push(
								h('span', {
										attrs: {
											title: '设置为全云店默认主页'
									...
									[h('a', {
										on: {
											click: () => {
												this.setDefaultPage(params.row);
											}
										}
									}, '设为全云店默认主页')]
								)
			...
			// 设置关联店铺
			setPageStore(row){
				this.$router.push('/cloud-shop/cloud-page-store-list?page_id='+row.page_id);
			},
			// 设为主页
			setDefaultPage(row){
				this.$Modal.confirm({
					title: '操作提示',
					content: '确定设置为全云店默认主页吗？',
					okText: '确定',
					cancelText: '取消',
					onOk: () => {
						// ajax 提交
						this.$ajax.post(this.$api.cloudGoodsPageSetDefaultPage, {
							id: row.id
						})
						.then((response) => {
							var res = response.data;
			...
			removePage(index, row) {
				this.$Modal.confirm({
					...
					onOk: () => {
						// ajax 提交删除页面
						this.$ajax.post(this.$api.cloudGoodsPageRemove, {
								id: row.id
							})
							.then((response) => {
								var res = response.data;
			...
			updateStatusAction(index, row, status) {
				this.tableLoading = true;
				// ajax 保存数据
				this.$ajax.post(this.$api.cloudGoodsPageStatus, {
						page_id: row.page_id,
						status: status,
					})
					.then((response) => {
