<template>
  <PageTopBase class="hot-goods-form" isSave @save="confirm">
    <Form ref="formValidate" :model="formItem" :rules="ruleValidate" :label-width="140">
      ...
      <FormItem label="选择商品" prop="goodsData">
        <goods-select :data="formItem.goodsData" type="radio" @del-tag="handleTag">
...
export default {
  ...
  methods: {
    loadData () {
      this.spinShow = true;
      return this.$ajax.post(this.$api.CloudDistributionActivityGoodInfo, {
        id: this.id
      })
      .then(response => {
    ...
    confirm() {
      this.$refs.formValidate.validate((valid) => {
        if (valid) {
          ...
          this.$ajax.post((this.id === 0 ? this.$api.CloudDistributionActivityGoodAdd : this.$api.CloudDistributionActivityGoodEdit), {
            ...this.formItem,
            id: this.id,
