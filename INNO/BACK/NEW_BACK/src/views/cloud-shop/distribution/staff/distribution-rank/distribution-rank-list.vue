      ...
      <Row class="m-bottom-15">
        ...
        <Col span="4" class="btn-group">
          <div class="flex" style="justify-content:flex-end;">
            ...
            <p class="m-top-5" ><Icon style="font-size:26px;"  type="ios-help-circle-outline" @click="showPic" /></p>
          </div>
        </Col>
      </Row>
...
export default {
  ...
  methods: {
    onLoadData (page, data) {
      ...
      return this.$ajax.post(this.$api.CloudDistributionRankList, params)
      .then(response => {
    ...
    onDelItem (row) {
      return this.$ajax.post(this.$api.CloudDistributionRankRemove, {
        id: row.id
      });
    },
    handleSet (row) {
      return this.$ajax.post(this.$api.CloudDistributionRankSet, {
        id: row.id
      }).then( _ => {
