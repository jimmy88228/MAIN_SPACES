...
<script>
	...
	import Template from '@/views/my-components/list-component/template/template.vue';
	...
	export default {
		...
		methods: {
			...
			handleGoods(type) {
				...
				this.$Modal.confirm({
					...
					onOk: () => {
						...
						switch (type) {
							case 'batchBind':
								url = this.$api.CloudDistributionStaffBind;
								break;
							case 'openStaff':
								url = this.$api.CloudDistributionStaffOpen;
								break;
							case 'closeStaff':
								url = this.$api.CloudDistributionStaffClose;
								break;
							default:
								break;
						}
			...
			onLoadData(page, data) {
				...
				return this.$ajax.post(this.$api.CloudDistributionStaffList, params)
					.then(response => {
			...
			onDelItem(row) {
				return this.$ajax.post(this.$api.CloudGoodsBrandRemove, {
			...
			handleImport() {
				...
				this.$refs.batchImport.openModal(this.canCreate.import ? {
					...
					upload: false,
					download: false
				}, this.$api.CloudDistributionStaffImportDistribution, this.$api.CloudDistributionStaffDownDistribution);
			},
			handleImport2() {
				this.$refs.batchImport2.openModal({
					upload: true,
					download: true
				}, this.$api.CloudDistributionStaffRelieve, this.$api.CloudDistributionStaffRelieveDown)
			},
			handleImport3() {
				this.$refs.batchImport.openModal({
					upload: true,
					download: true
				}, this.$api.CloudDistributionStaffDeduction, this.$api.CloudDistributionStaffDeductionDown)
			},
			handleImport4() {
				this.$refs.batchImport.openModal({
					upload: true,
					download: true
				}, this.$api.CloudDistributionStaffUpdateData, this.$api.CloudDistributionStaffUpdateDataDown)
			},
			onImportSuccess() {
				this.loadData();
			},
			...
			handleExport() {
				this.$Modal.confirm({
					...
					cancelText: '取消',
					onOk: () => {
						return this.$ajax.post(this.$api.CloudDistributionStaffListExport, {
							...this.condition
						}).then((response) => {
							var res = response.data;
							if (res.code) {
			...
			clearFan(row){
				this.$Modal.confirm({
                    title: '分销员解除粉丝',
						...
						cancelText: '取消',
                        onOk: () => {
							return this.$ajax.post(this.$api.CloudDistributionStaffClearFan, {staff_id:row.dstb_staff_id}).then((response) => {
								var res = response.data;
								if (res.code) {
