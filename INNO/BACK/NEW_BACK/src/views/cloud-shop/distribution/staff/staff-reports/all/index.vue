...
export default {
  ...
  methods: {
    onLoadData (page, data) {
      ...
      return this.$ajax.post(this.$api.CloudStaffReportsList, params)
      .then(response => {
    ...
    handleExport () {
			this.$Modal.confirm({
				...
				cancelText: '取消',
				onOk: () => {
					return this.$ajax.post(this.$api.CloudStaffReportsListExport, {
            ...this.condition
          }).then((response) => {
    ...
    handleView (row) {
      this.onSetState({
        start_time: this.condition.start_time,  //开始时间
        end_time: this.condition.end_time,   //结束时间
        searchq: row.dstb_staff_code,  //模糊搜索
      })
    }
  },
  mounted () {
