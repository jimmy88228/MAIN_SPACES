	...
	export default {
		...
		methods: {
			onLoadData(page, data){
				...
				return this.$ajax.post(this.$api.CloudDistributionActivityAwardList, {
			...
			setState(row){
				...
				return this.$ajax.post(this.$api.CloudDistributionActivityAwardStatus, {
					id: row.id,
					enable: is_enabled
				}).then((response)=>{
					let res = response.data || {};
			...
			deleteMessage(id, index) {
				this.$Modal.confirm({
					...
					onOk: () => {
						this.tableLoading = true;
						this.$ajax.post(this.$api.CloudDistributionActivityAwardRemove, {
							id: id,
						}).then((response)=>{
							let res = response.data || {};
							if(res.code){
