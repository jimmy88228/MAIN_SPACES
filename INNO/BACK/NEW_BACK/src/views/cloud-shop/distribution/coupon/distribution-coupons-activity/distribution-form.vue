...
export default {
  ...
  methods: {
    loadData () {
      this.spinShow = true;
      return this.$ajax.post(cloudDistributionCouponsActivityInfo, {
        id: this.id
      })
      .then(response => {
    ...
    confirm () {
      this.$refs.formValidate.validate((valid) => {
        if (valid) {
          ...
          this.$ajax.post((this.formItem.id === 0 ? this.$api.CloudDistributionCouponsActivityAdd : this.$api.CloudDistributionCouponsActivityEdit), {
            ...this.formItem,
