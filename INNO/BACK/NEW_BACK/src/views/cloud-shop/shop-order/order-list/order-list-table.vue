	...
	<div class="order-list-table">
		<Table :columns="tableColums" :data="tableData" :span-method="handleSpan" disabled-hover>
			<template slot-scope="{ row }" slot="orderInfo">
				<div class="header_padding" style="margin:0px;padding:0px;">
					<div class="item g_item">
						<template v-if="row.order_status == 4">
			...
			<!--	<div v-if="status != 1000">
				...
				<div v-if="row.order_status == 4 && status != 1000">
					<router-link tag="a" :to="{name: 'shop-order-info', params: {sn: row.order_id}}" target="_blank" v-if="row.handle.edit">查看</router-link>
				</div>
			</template>
		</Table>
	...
	export default {
		...
		methods: {
			...
			loadData() {
				...
				return this.$ajax.post(this.$api.ShopOrderList, {
			...
			handleFlag(bool, orderId) {
				...
				return this.$ajax.post(this.$api.ShopRedFlagOrder, {
					order_id: orderId,
					flag: bool ? 1 : 0
				}).then(response => {
					const res = response.data;
