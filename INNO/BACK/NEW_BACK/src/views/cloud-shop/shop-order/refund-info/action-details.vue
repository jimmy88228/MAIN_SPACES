<template>
  <div class="action_details">
    ...
    <div class="note">
      ...
      <Button v-if="cancel_rollback" type="primary" @click="handle('cancel_rollback')">反审核</Button>
    </div>
...
export default {
  ...
  computed: {
    ...
    cancel_rollback() {
      return this.orderButton.cancel_rollback;
    }
  },
  methods: {
    handle(type) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.spinShow = true;
          return this.$ajax.post(this.$api.ShopOrderRefundUpdate, {
            id: this.sn,
