      ...
      <Form ref="formDynamic" :model="formDynamic">
        <Table :columns="columns" :data="formDynamic.pageData" ref="myTable">
          ...
          <template slot-scope="{ row, index }" slot="handle">
            ...
            <Divider type="vertical" v-if="!idDisabled"/>
            <span><a @click="handleEditStock(row, index)">设置库存</a></span>
          </template>
        </Table>
      </Form>
      <div class="footer">
  ...
  methods: {
    ...
    handleEditStock (row, index) {
      this.rowData = row;
			...
			let activityStoreStock = this.activityStoreStock || {};
			let activity_store = activityStoreStock.activity_store || [];
			let store_goods = activityStoreStock.store_goods || [];
			let this_goods_store = {}, goods_stores = []
			for(let i = 0; i < store_goods.length; i++){
				let _goods_id = store_goods[i].goods_id;
				if(_goods_id == goods_id){
					this_goods_store[store_goods[i].store_id] = store_goods[i].inventory || 0
				}
			}
			for(let i = 0; i < activity_store.length; i++){
				goods_stores.push({
					...activity_store[i],
					inventory: this_goods_store[activity_store[i].store_id] || 0
				})
			}
			this.$UIModule({
				mode: "store-stock",
      ...
      // this.$refs.normsFormStock.setData(index).show();
    },
    handleDel (index) {
