    ...
    <Card v-show="showList">
      ...
      @on-selection-change="getSelectAct" @on-select-all="handleSelectAll">
        ...
        <template slot-scope="{ row }" slot="handle">
          ...
          <span v-show="row.handle.remove" @click="delItem(row, '删除提示', '确定删除该活动吗？')"><a>移除</a></span>
          <Divider type="vertical"/>
          <span v-show="row.handle.edit" @click="handleGoodsEdit(row)"><a>绑定商品</a></span>
          <Divider type="vertical"/>
          <span v-show="row.handle.edit" @click="bindingStore(row)">
            <a>关联店铺</a>
          </span>
...
export default {
  ...
  methods: {
    ...
    onLoadData (page, data) {
      ...
      return this.$ajax.post(this.$api.cloudSeckillActivityList, params)
      .then(response => {
    ...
    onDelItem (row) {
      return this.$ajax.post(this.$api.cloudSeckillActivityRemove, {
        id: row.id
      });
    },
    createActivity () {
    ...
    handleBatchStatus(val) {
				...
				this.spinShow = true;
				return this.$ajax.post(this.$api.cloudSeckillActivityEditStatus, {
      ...
      handleCopy (row) {
        ...
        return this.$ajax.post(this.$api.cloudSeckillActivityCopy, {
          id: row.id
        }).then(() => {
      ...
      confirmExport(row){
				return this.$ajax.post(this.$api.cloudSeckillActivityGoodsExport,{
					activityId: row.id
				}).then((response) => {
