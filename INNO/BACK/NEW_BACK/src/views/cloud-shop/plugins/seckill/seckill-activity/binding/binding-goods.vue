      ...
      <div>
        <Row>
          ...
          <Col span="12" class="btn-group">
            <Button type="info" icon="md-add" @click="createActivity">添加商品</Button>
          </Col>
        </Row>
...
export default {
  ...
  methods: {
    onLoadData (page, data) {
      ...
      return this.$ajax.post(this.$api.cloudBindingGoodsList, params)
      .then(response => {
    ...
    addGood (data) {
      return this.$ajax.post(this.$api.cloudBindingGoodsAdd, {
        id: this.id,
    ...
    onDelItem (row) {
      return this.$ajax.post(this.$api.cloudBindingGoodsRemove, {
        id: this.id,
    ...
    handleBatch () {
      this.$Modal.confirm({
        ...
        cancelText: '取消',
        onOk: () => {
          return this.$ajax.post(this.$api.cloudBindingGoodsBatchRemove, {
            id: this.id,
