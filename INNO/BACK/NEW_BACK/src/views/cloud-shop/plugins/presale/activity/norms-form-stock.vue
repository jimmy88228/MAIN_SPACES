...
export default {
  ...
  methods: {
    ...
    confirm () {
      this.$refs.formDynamic.validate(valid => {
        if (valid) {
          ...
          this.$emit('edit-norms', this.formDynamic.pageData, this.curIndex);
          this.hide();
        } else {
          this.showLoading();
        }
    ...
    // 打开模态框
    setData (index, is_first) {
      this.resetStatus(); //需重置，此时打开都是同个modal
      this.curIndex = index;
      if (is_first == 1) return this;
      this.$nextTick(() => {
        console.log(this.rowData, '打印：', this.rowData,)
          this.tableLoading = true;
          return this.$ajax.post(this.$api.cloudPresaleStoreGoodsInfo, {
