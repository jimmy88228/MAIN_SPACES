...
export default {
  ...
  methods: {
    onLoadData (page, data) {
      ...
      return this.$ajax.post(this.$api.cloudPopupAdvertRemindList, params)
      .then(response => {
    ...
    onDelItem (row) {
      return this.$ajax.post(this.$api.cloudPopupAdvertRemindRemove, {
        id: row.id
      });
    },
    createActivity() {
    ...
    editStatus(row) {
      this.spinShow = true;
      return this.$ajax.post(this.$api.cloudPopupAdvertRemindStatus, {
        id: row.id
      }).then(response => {
        const res = response.data;
        if (res.code) {
