		...
		<Row type="flex" style="margin-bottom:10px;">
			<Col style="flex:1 1 0%;">
				<Tooltip content="返回" placement="bottom-start">
					...
					<span style="margin-left:30px">活动推广关联店铺列表</span>
				</Tooltip>
			</Col>
...
export default {
	...
	methods: {
		...
		init() {
			...
			this.$ajax.post(this.$api.cloudPopupAdvertStoreList, {
				isInit: 1,
		...
		removeItem( index, row ){
			this.$Modal.confirm({
				...
				onOk: () => {
					// ajax提交数据
					this.tableLoading = true;
					this.$ajax.post(this.$api.cloudPopupAdvertStoreRemove, {
		...
		onStoreOk( items ){
			...
			// ajax提交数据
			this.tableLoading = true;
			this.$ajax.post(this.$api.cloudPopupAdvertStoreAdd, {
				store_ids: storeIds,
				activity_id: this.currPageId,
			})
			.then(response => {
