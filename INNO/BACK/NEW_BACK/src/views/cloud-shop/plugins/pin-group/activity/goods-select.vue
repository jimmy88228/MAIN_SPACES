  ...
  <div class="goods_select">
    <Table :columns="goodsTypeColumns" :data="goodsTypetableData" ref="goodsType" class="table">
      ...
      <template slot="handle">
        ...
        <!--<span @click="handleDel"><a :class="delClass">设置店铺库存</a></span>-->
...
export default {
  ...
  data () {
    ...
    const checkSalePrice = (rule, value, callback) => {
      ...
      const reg = /^sale_price(?<index>\d+)$/;
      const matchObj = reg.exec(field);
      const index = matchObj.groups.index;
      if (this.goodsSpectableData[index].market_price && this.goodsSpectableData[index].market_price < value) {
    ...
    return {
      ...
      tableRules: {
        ...
        sale_price: [
          ...
          {validator: checkSalePrice, type: 'number'}
        ],
        market_price: [
    ...
    data (nV) {
      ...
      this.goodsSpectableData = nV[0].get_products.map(item => {
        return Object.assign({}, item, {
          ...
          sale_price: item.sale_price == undefined ? 0  : Number(item.sale_price),
          active_number: Number(item.active_number) ? Number(item.active_number) : item.product_number == undefined ? 0 : Number(item.product_number), //初始化活动库存等于库存，编辑的时候取active_number
