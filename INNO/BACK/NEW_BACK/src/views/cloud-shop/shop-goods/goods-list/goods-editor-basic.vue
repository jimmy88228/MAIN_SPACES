	...
	export default {
		...
		data() {
			...
			return {
				...
				auth: {
					is_show_goods_distribution_commission:false,
					is_show_self_buy_commission:false
				}
			}
		},
		methods: {
				...
				this.$refs.formValidate.validate((valid) => {
					...
					if (valid) {
						...
						this.$ajax.post((this.formItem.goods_id == 0 ? this.$api.ShopGoodsAdd : this.$api.ShopGoodsEdit), {
							...
							staff_dstb_goods_commate: this.formItem.staff_dstb_goods_commate,
							staff_dstb_goods_commate_arr: this.formItem.staff_dstb_goods_commate_arr,
            				is_new_distribution: this.formItem.is_new_distribution
						}).then((response) => {
							this.spinShow = false;
