...
export default {
  ...
  methods: {
    ...
    openModal (row, showFooter) {
      ...
      this.formItem.id = typeof (row.colorcat_id) !== 'undefined' ? Number(row.colorcat_id) : 0;
      if (this.formItem.id == 0) {
        ...
        // 编辑时候的初始化数据
        this.formItem.name = row.colorcat_name;
        this.formItem.sortOrder = Number(row.colorcat_sort);
      }
    },
    // 模态框确认事件
    modalOk () {
      this.$refs.formValidate.validate((valid) => {
        if (valid && this.sortVaild) {
          // ajax 保存数据
          util.ajax.post((this.formItem.id == 0 ? util.apiUrl.ShopGoodsColorAdd : util.apiUrl.ShopGoodsColorEdit), {
            colorcat_id: this.formItem.id,
            colorcat_name: this.formItem.name,
            colorcat_sort: this.formItem.sortOrder
            })
		    		.then((response) => {
