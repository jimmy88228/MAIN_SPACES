        ...
        <div class="spec_table">
          <div class="spec_table_item">
            <div class="color_header">
              <span>规格名：</span>
              <span><Input type="text" v-model="colorUnitEdit" class="basic_input" @on-change="handleEditColor" placeholder="颜色" :disabled="disabled"/></span>
            ...
            <Row class="color_content">
              ...
              <Col :lg="20" :xl="20" :xxl="22">
                <div class="sort_list">
                  <span v-for="(item, index) in colorList" :key="index" class="goods_img_wrapper">
                      <spec-item
                        :key="item.id"
                        :cur-item="item"
                        :cur-index="index"
                        ...
                        @editGallery="handleGallery"></spec-item>
                    </span>
                    <Button type="text" @click="addColorItem" class="add" :disabled="saleType == 99 && colorList.length === 1 || disabled">
                      <Icon type="md-add" />
                      <span>添加规格值</span>
                    </Button>
                </div>
          ...
          <div class="spec_table_item spec_size_item">
            <template v-if="allowShow && toggle">
              <div class="size_header">
                <span>规格名：</span>
                <span><Input type="text" v-model="sizeUnitEdit" class="basic_input" @on-change="handleEditSize" placeholder="尺码" :disabled="disabled"/></span>
              ...
              <Row class="color_content">
                ...
                <Col :lg="20" :xl="20" :xxl="22">
                  <div class="sort_list size_sort_list">
                    ...
                    <Button type="text" @click="addSizeItem" class="add" :disabled="saleType == 99 && sizeList.length === 1 || disabled">
                      <Icon type="md-add" />
                      <span>添加规格值</span>
                    </Button>
                  </div>
          ...
          <Tooltip :content="toolText">
            <Button @click="createNew" :disabled="!allowShow || (allowShow && toggle) || disabled">添加规格项目</Button>
          </Tooltip>
        </div>
      </Col>
    </Row>
...
export default {
  ...
  methods: {
    ...
    loadData (page) {
      this.$ajax.all(
        [
          this.$ajax.post(this.$api.ShopColorTree),
          this.$ajax.post(this.$api.ShopSizeTree)
        ]
      ).then(
        ...
        if (newVal) {
          let data = [...newVal].map(item => {
            return {
              ...
              spec2_id: item.spec2_id,
              spec1_id: item.spec1_id,
              spec2_name: item.spec2_name,
              spec1_name: item.spec1_name,
