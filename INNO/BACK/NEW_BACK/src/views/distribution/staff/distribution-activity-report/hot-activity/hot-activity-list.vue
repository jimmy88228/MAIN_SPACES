    ...
    <Card>
      ...
      <Table :loading="tableLoading" :height="tableHeight" :columns="tableColumns" :data="tableData" ref="myTable">
        ...
        <template slot-scope="{ row, index }" slot="handle">
          ...
          <Divider type="vertical" v-show="row.handle.edit && row.handle.export"/>
          <span v-show="row.handle.export" @click="exportData(index, row)"><a>导出统计</a></span>
        </template>
      </Table>
...
export default {
  ...
  methods: {
    onLoadData (page, data) {
      ...
      return this.$ajax.post(this.$api.distributionActivityReport, params)
      .then(response => {
    ...
    handleExport () {
			this.$Modal.confirm({
				...
				cancelText: '取消',
				onOk: () => {
					return this.$ajax.post(this.$api.distributionActivityReportExport, {
            id: this.currentId,
            ...this.exportData
          }).then((response) => {
    ...
    confirmExport(data){
      let params = Object.assign({}, data, this.condition);
      return this.$ajax.post(this.$api.distributionActivityReportExport,params).then((response) => {
          var res = response.data;
          if (res.code) {
            var jobId = res.data;
            this.jobIdCol.push(jobId);
