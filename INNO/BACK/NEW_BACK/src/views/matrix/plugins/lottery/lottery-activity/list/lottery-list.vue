...
import 	{ lotteryType, typeCode } from '@/views/matrix/plugins/lottery/typeMap.js';
export default {
  components: {
    SearchForm,
  },
  ...
  methods: {
    onLoadData (page, data) {
      return this.$ajax.post(this.$api.MatrixLotteryActivityList, {
				...this.condition,
				...data,
			})
      .then(response => {
    ...
    createActivity () {
			this.$selectModule({
				...
				getList: (data) => {
					console.log(data);
					let type_code = data[0].type_code || "";
					if(!type_code) {
