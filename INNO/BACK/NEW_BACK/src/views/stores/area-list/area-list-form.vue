        ...
        <FormItem label="上级区域" class="spec-item">
          <Cascader
            ...
            @on-change="selectSort">
              <Input v-model="currentName" readonly/>
              <!-- {{currentName}} -->
            </Cascader>
...
export default {
  ...
  methods: {
    ...
    handleSortList (context, ids) {
      const format = context.map(item => {
        let disabled = false;
        ids = ids || [];
        if(ids.indexOf(item.id + "") != -1){
          disabled = true;
          this.currentName = ids.indexOf(item.id) == 0 ? item.title : this.currentName ? this.currentName + '/' + item.title : item.title;
        }
        return {
          disabled: disabled,
          value: item.id,
          ...
          children: item.children.length ? this.handleSortList(item.children, ids) : []
        }
      });
      return format;
    },
    selectSort (value, selectedData) {
      ...
      //   return false;
      // }
      this.currentName = selectedData.map(o => o.label).join('/');
      this.isSortValid = true;
      this.validTip = false;
