	...
	<div class="store-list">
		...
		<div v-show="pageTotal" class="list_page list_page_fixed">
			...
			<div class="btn_group"  v-else>
				<Checkbox v-model="isCheckAll" @on-change="handleCheck">当页全选</Checkbox>
				<ButtonGroup>
					<Button @click="startStoreMore">批量启用店铺</Button>
				</ButtonGroup>
			</div>
	...
	export default {
		...
		methods: {
				...
				return this.$ajax.post(this.$api.storeList, params)
					.then(response => {
						...
						if (res.code) {
							...
							let applet = res.data && res.data.applet;
							for(let i = 0; i < applet.length; i++){
								applet[i].applet_name = applet[i].cfg_remark;
							}
							this.applet = applet;
			...
			startStoreMore() {
				...
				this.removeStore(this.selectStore.map(item => item.id), 'start');
			},
			...
			handleAppletQrcode( row ) {
				this.$UIModule({
					...
					props: {
						title: "选择小程序",
						codeTitle: "店铺小程序二维码",
						codeTip: '<p>店铺名称：' + row.name +'</p><p>店铺code：' + row.code +'</p>'
					},
					options: {
				...
				// const optionItems = [];
				// const _this = this;
				// if(Number(_this.applet.length)==1){
				// 	this.appId =_this.applet[0].appid;
				// }else{
				// 	this.appId = 0;
				// }
