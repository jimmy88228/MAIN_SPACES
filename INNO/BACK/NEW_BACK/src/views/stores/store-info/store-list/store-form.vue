	...
	<div class="store_form">
		<PageTopBase isSave @save="confirm">
			<Form ref="formValidate" :model="formItem" :rules="ruleValidate" :label-width="140" label-colon>
				...
				<FormItem label="是否可用储值">
					<i-switch size="large" v-model="formItem.is_allow_use_stored_value" true-value="1" false-value="0">
						<span slot="open">开启</span>
						<span slot="close">关闭</span>
					</i-switch>
					<span>备注：控制店铺消费时，是否可使用储值抵扣下单</span>
				</FormItem>
		...
		methods: {
					...
					.then(response => {
						...
						if (res.code) {
							...
							if (Number(this.id)) {
								...
								this.formItem.is_allow_use_stored_value = storeMes.is_allow_use_stored_value;
			...
			confirm() {
				...
				this.$refs.formValidate.validate((valid) => {
					if (valid) {
						this.spinShow = true;
						this.$ajax.post(Number(this.id) ? this.$api.storeEdit : this.$api.storeAdd, {
                            	...
                            	is_allow_use_stored_value: this.formItem.is_allow_use_stored_value,
