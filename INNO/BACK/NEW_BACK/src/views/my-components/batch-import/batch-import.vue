	...
	<div>
		<Modal class-name="upload-wrapper" v-model="modalShow" :title="modalTitle" :footer-hide="true"
			...
			<div class="upload-input" v-if="canCreate.goodsInputImport">
				<p class="m-bottom-5">如需手动输入,请按照要求输入商品货号</p>
				<Input type="textarea" v-model="uploadInputTxt" class="upload-textarea" placeholder="输入货号,多货号逗号隔开"/>
				<div class="text-c">
...
<script>
	...
	export default {
		...
		methods: {
			...
			uploadInput(){
				this.modalShow = false;
				this.$emit("on-success", this.uploadInputTxt);
			}
		}
	}
</script>
