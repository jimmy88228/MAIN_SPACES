...
<template>
	<div>
		<Modal v-model="showModal" :loading="modalLoading" :title="modalTitle" :width="880" :styles="{top:'20px'}" class="kansale-goods-select"
		 @on-ok="onOk">
						...
						<div class="select-box">
							...
							<div>
								<div class="goods-img-box" title="点击查看大图">
									<img :src="(typeof(item.activity_img) == 'undefined' ? item.activity_img : '')" :alt="item.activity_name"
	...
	export default {
		...
		methods: {
			...
			initData() {
				...
				// ajax 请求获取初始化数据
				util.ajax.post(util.apiUrl.bargainActivityList, {
						isInit: 2,
			...
			initTable(res) {
				...
				this.columns[(this.columns.length - 1)].render = (h, params) => {
					...
					const color = Number( row.is_enabled ) == 1 ? 'green' : 'red';
