    ...
    <div class="content">
      <Table :columns="columns" :data="pageData" ref="myTable">
        ...
        <template slot-scope="{ row, index }" slot="perOrder">
          <Input v-model="row.perOrder" type="number" style="width: 100px;" @on-change="e => handleSetData('perOrder', index, e.target.value)"/>
        </template>
        <template slot-scope="{ row, index }" slot="perUser">
          <Input v-model="row.perUser" type="number" style="width: 100px;" @on-change="e => handleSetData('perUser', index, e.target.value)"/>
        </template>
        <template slot-scope="{ row, index }" slot="sortOrder">
          <Input v-model="row.sortOrder" type="number" style="width: 100px;" @on-change="e => handleSetData('sortOrder', index, e.target.value)"/>
        </template>
        ...
        <div class="action">
          ...
          <template>
            ...
            <Poptip v-model="showBatchInput" placement="bottom-end">
              ...
              <Button type="primary" data-key="limit" @click.native="handleSetBatchType('limitNumber')">设置售卖数量</Button>
...
export default {
  ...
  computed: {
    ...
    checkAll: {
      get() {
        ...
        if (this.checkType === 1) {
          let ids = this.pageData.map(item => item.goods_id);
  ...
  methods: {
    ...
    handleSetBatchData (key, index, val) {
      this.pageData[index][key] = val;
      this.pageData[index].get_products.forEach(item => {
        item[key] = val;
      });
    },
    ...
    handleEditNorms (data, index) {
      this.pageData[index].get_products = data;
    },
    ...
    cloneGoodsData: {
      handler(nV) {
        ...
        let formatData = JSON.parse(JSON.stringify(nV)).filter(item => {
          ...
          item.perOrder = item.perOrder || 0;
          item.perUser = item.perUser || 0;
