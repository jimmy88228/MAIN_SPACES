  ...
  <div class="ad-module">
    <DPoptip title="编辑内容" placement="right" width="500" :eles="ele">
        <div class="content" :class="contentClass">
          <!--<span v-for="item in originData.lineType" :key="item" class="item item_fixed">
            <img src="@rs/images/default-img.jpg" class="img"></img>
            <span>{{titleMap[item]}}</span>
          </span>-->

          <span v-for="(item ,key) in adList" :key="item" class="item item_fixed">
            <img :src="item.pic" class="img"></img>
            <span>{{titleMap[key+1]}}</span>
          </span>
        </div>
        <div slot="content">
          <Form ref="FormData" :label-width="60">
            <FormItem label="列表样式">
              <RadioGroup v-model="originData.lineType" @on-change="handleChangeLineType">
                <Radio :label="1">一行一个</Radio>
                <Radio :label="2">一行两个</Radio>
                <Radio :label="3">一行三个</Radio>
                <div>
                  <Radio :label="4">一行四个</Radio>
                  <Radio :label="5">一行五个</Radio>
                </div>
              </RadioGroup>
            </FormItem>
            <div class="edit">
...
export default {
  ...
  methods: {
    ...
    handleChangeLineType(val) {
      let hasLen = this.originData.adList.length;
      if (val - hasLen > 0) {
        for (let i = 0; i < val - hasLen; i++) {
          let listData = AdModule.getAdList();
          this.originData.adList.push(listData);
        }
      } else {
        this.originData.adList = this.originData.adList.slice(0, val - hasLen);
      }
    },
