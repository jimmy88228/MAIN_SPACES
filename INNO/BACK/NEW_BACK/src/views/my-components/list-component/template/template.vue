    ...
    onLoadData (page, data) {
      ...
      .then(response => {
        ...
        if (res.code) {
          ...
          let apiPageSize = _data.pageSize || 0;

          _data.isShowPage = apiPageSize == '10000' ? false : true;
          //不符合table结构,拼装
    ...
    setListKey(items){
      for(let i = 0; i < items.length; i++){
				if(this.listKey){ //部分接口数据没有id，根据listKey 拼装 id
					if(items[i][this.listKey] &&  items[i][this.listKey] != items[i].id){
  ...
  watch: {
    ...
    status: {
      handler ({mode, type, extraAddtion,listKey, modeStyle, reqConfig}){
        ...
        this.mode = (reqConfig && reqConfig != 'labels') ? reqConfig : mode;
        this.type = type;
