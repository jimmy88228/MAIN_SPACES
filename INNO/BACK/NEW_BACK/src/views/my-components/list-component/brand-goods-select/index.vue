		...
		<list-component ref="listComponent" :status="currentStatus" :table-column="columns" :form-search="formSearch">
			...
			<template v-slot:search="{ searchPage, data }">
				<Form ref="formSearch" :model="formSearch" :label-width="90" inline class="goods_search">
					...
					<div style="color:red">备注：支持多货号搜索，货号之间需用英文逗号分隔开</div>
	...
	export default {
		...
		methods: {
			loadExtraData() {
				this.$ajax.all(
					[
						this.$ajax.post(this.$api.brandGoodsSyncCategory),
			...
			handleStart(date) {
				this.formSearch.startSaleTime = date;
			},
			handleEnd(date) {
				this.formSearch.endSaleTime = date;
			},
		},
		mounted() {
			// 获取分类数据
