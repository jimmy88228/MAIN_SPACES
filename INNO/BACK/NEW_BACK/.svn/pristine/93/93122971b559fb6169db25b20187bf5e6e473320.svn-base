...
export default {
  ...
  methods: {
    loadData () {
      this.spinShow = true;
      return this.$ajax.post(this.$api.distributionActivityGoodInfo, {
        id: this.id
      })
      .then(response => {
    ...
    confirm() {
      this.$refs.formValidate.validate((valid) => {
        if (valid) {
          ...
          this.$ajax.post((this.id === 0 ? this.$api.distributionActivityGoodAdd : this.$api.distributionActivityGoodEdit), {
            ...this.formItem,
            id: this.id,
