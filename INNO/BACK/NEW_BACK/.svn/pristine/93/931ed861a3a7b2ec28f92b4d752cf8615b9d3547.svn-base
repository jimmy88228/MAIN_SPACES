<template>
	<PageTopBase topTitle="云店促销规则" isSave @save="confirm">
		<div class="full-reduction">
			<Form ref="formValidate" :model="formItem" :rules="ruleValidate" :label-width="140" v-friendly-errors>
				...
				<FormItem label="是否允许使用优惠劵" prop="isAllowCoupon">
					<RadioGroup v-model="formItem.isAllowCoupon">
						<Radio label="1">是</Radio>
						<Radio label="0">否</Radio>
					</RadioGroup>
				</FormItem>
				<FormItem label="是否允许使用积分" prop="isAllowPoint">
						...
						<div class="rule_content">
							...
							<FormItem class="form_item_24" label="优惠内容" :label-width="80" :prop="'rules.'+index+'.selectDiscount'" :rules="{required: true, message: '优惠内容不能为空', trigger: 'change', type: 'array', min: 1}">
								<CheckboxGroup v-model="item.selectDiscount" class="check_vertical">
									...
									<Checkbox label="5" v-if="activityType != 4">订单金额优惠</Checkbox>
									<RadioGroup v-model="item.discountType" vertical v-show="activityType != 4 && item.selectDiscount.includes('5')"
									 @on-change="(typeValue) => {handleDiscountTypeChange(typeValue, index)}">
										...
										<FormItem v-show="activityType == 1" class="form_item_24" ref="priceRef" :prop="'rules.'+index+'.price'"
				...
				<FormItem label="参与活动商品" prop="isAllGoods">
					...
					<div class="goods_wrapper" v-if="formItem.isAllGoods == 0">
						<FormItem prop="goodsSelect">
							<div v-for="(item,index) in formItem.goodsItems" :key="index">
								<Tag  
								size="large" closable 
		...
		<expand-select ref="expandSelect" 
		:selected="formItem.goodsSelect" title="选择满减商品" 
		@get-goods-id="handleGoodsId" 
		@get-goods="onSelectGoods"></expand-select>
...
<script>
	...
	export default {
		...
		methods: {
			loadData() {
				this.spinShow = true;
				this.$ajax.post((this.activityType == 1 || this.activityType == 2) ? this.$api.cloudFullReductionGeneralInfo : this
					.$api.cloudFullReductionOverallInfo, {
						rule_id: this.id || 0
					}).then(response => {
					...
					if (res.code) {
						...
						if (data) {
							this.formItem = {
								...
								goodsItems: data.get_limit_goods,
			...
			createRule() {
				let newId = this.formItem.rules.length == 0 ? 0 : this.formItem.rules[this.formItem.rules.length - 1].id;
			...
			handleGoodsId(ids) {
				// let idCols = this.formItem.goodsSelect || [];
				// ids.map( item => {
				// 	if( item != null && (idCols.indexOf(item + "") == -1)){
				// 		this.formItem.goodsSelect.push( item );
				// 	}
				// });
			...
			// 选中商品的回调
			onSelectGoods( items ){
				// let idCols = this.formItem.goodsItems.map(item => parseInt(item.goods_id));
				// items.map( item => {
				// 	if( item.goods_id != null && !idCols.includes(parseInt(item.goods_id))){
				// 		this.formItem.goodsItems.push( item );
				// 	}
				// });
				console.log("change goodsItems", items);
				this.formItem.goodsItems = items;
			},
			onCloseGoods( index ){
				this.$Modal.confirm({
					title: '操作提示',
					...
					onOk: () => {
						...
						this.$delete(this.formItem.goodsItems, index);
					},
				});
			},
			confirm() {
				this.$refs.formValidate.validate(valid => {
					if (valid) {
						...
						if (this.id) {
							// 编辑
							api = (this.activityType == 1 || this.activityType == 2) ? this.$api.cloudFullReductionGeneralEdit : this.$api
								.cloudFullReductionOverallEdit;
						} else {
							// 添加
							switch (this.activityType) {
								case 1:
								case 3:
									api = this.$api.cloudFullReductionGeneralMoneyAdd;
									break;
								case 2:
								case 4:
									api = this.$api.cloudFullReductionGeneralPieceAdd;
									break;
								default:
									break;
							}
						...
						this.$ajax.post(api, {
								...
								goods_ids: this.formItem.goodsSelect.join(),
