		...
		methods: {
					...
					buttons.push(
						h('span', {
							...
							[h('a', {
								on: {
									click: () => {
										this.copyPage( params.index, params.row );
									}
								}
							}, '复制')]
						)
					...
					buttons.push(
						h('span', {
							...
							[h('a', {
								on: {
									click: () => {
										this.$util.copyTextToClipBoard( 'pages/micro_mall/custom_page/custom_page?page_id=' + params.row.id );
									}
								}
							}, '复制链接')]
						)
			...
			copyPage(index, row){
				this.$Modal.confirm({
					...
					onOk: () => {
						// ajax 提交复制页面
						this.$ajax.post(this.$api.goodsPageCopy, {
							id: row.id
						})
						.then((response) => {
							var res = response.data;
