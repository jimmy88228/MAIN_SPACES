...
<template>
	<div>
		<Modal v-model="showModal" :loading="modalLoading" :title="modalTitle" :width="970" :styles="{top:'20px'}" class="integral-goods-select"
		 @on-ok="onOk">
			...
			<Row :gutter="10">
				...
				<Table ref="select-table" :loading="tableLoading" :height="tableH" :columns="columns" :data="tableData" @on-select="onTableSelect"
...
<script>
	...
	export default {
		...
		computed: {
			// 获取内容框高度
			getContentHeight() {
				var tHeight = this.tableH = document.body.clientHeight - 320;
			...
			importGoods(){
				this.$UIModule({
					...
					options: {
						...
						uploadUrl: this.$api.integralGoodsIdImportById,
						downloadUrl: this.$api.integralGoodsIdTplById
					},
					success:(data)=>{
						console.log("成功", data);
						let uploadInputTxt = data || "";
						this.formSearch.searchq_type = 'id'; // 货号搜索
