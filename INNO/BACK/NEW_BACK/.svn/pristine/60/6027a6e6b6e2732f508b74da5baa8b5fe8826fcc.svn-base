...
<template>
	<div>
		<Modal v-model="modalShow" class="goods-page-template" :title="modalTitle" :loading="modalLoading" :footer-hide="modalFooterHide"
		 :width="800">
					...
					<Row>
						...
						<div class="cover-img">
							...
							<transition name="tran-button">
								<div class="inner-box">
									<div class="buttons">
										<Button type="success" @click="createPage(tplCode)">开始编辑</Button>
									</div>
								</div>
							</transition>
...
<script>
	...
	export default {
		...
		methods: {
			...
			init() {
				// ajax 获取网站模板列表
				this.$ajax.post(this.$api.cloudGoodsPageTemplateList, {
			...
			createPage(tplCode) {
				...
				// ajax 获取网站模板列表
				this.$ajax.post(this.$api.cloudGoodsPageAdd, {
