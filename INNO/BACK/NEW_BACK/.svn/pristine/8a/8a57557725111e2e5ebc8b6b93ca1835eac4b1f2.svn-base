...
<template>
	<div class="coin-manage">
		<Alert show-icon>
	        <span style="font-weight: bold;">虚拟币说明</span>
	        <template slot="desc">虚拟币可以作为奖励赠送给用户，用户可以互相赠送或购买商品来消耗虚拟币。虚拟币可以提现，可以充值。</template>
	    </Alert>

		<titleBar>虚拟币赠送策略</titleBar>
		...
		<!--表单组件-->
	    <coinForm ref="coin-form" @on-save="saveOk"></coinForm>
	</div>
</template>
...
export default {
  ...
  methods: {
    ...
    init () {
      ...
      // ajax 请求获取初始化数据
        	util.ajax.post(util.apiUrl.coinManage, {
        		isInit: 1
        	})
    		.then((response) => {
    ...
    editItem (index, row) {
        	// 打开编辑模态框
      this.$refs['coin-form'].openModal(row);
    },
    // 表单保存成功的回调
    saveOk (obj) {
        	// 直接重新加载一次数据
