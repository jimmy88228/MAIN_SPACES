...
const app = {
	state: {
		...
		hidePendingTip: false, // 首页展示订单提示
		appletList: [],
	},
	mutations: {
		setHidePendingTip(state, bool) {
			state.hiddenPendingTip = bool;
		},
		...
		setOpenedList(state) {
			state.pageOpenedList = window.localStorage.pageOpenedList ? JSON.parse(window.localStorage
		...
		// 设置客服人员的状态
		setWorkerStatus(state, val) {
			state.workerStatus = val;
		},
		// 客服已读动作触发
		...
		//
		setAppletList(state, val){
			state.appletList = val || [];
		}
	}
};
