...
<template>
	<div class="act-sale-view package-sale-view">
		<!--统一活动头-->
				...
				<Row type="flex" style="flex-wrap:wrap;" :gutter="info.gutter">
					...
					:span="info.layout == 'one' ? 24 : (info.layout == 'two' ? 12 : 8)">
						...
						<div class="goods-item" :style="{'margin-bottom': info.gutter + 'px'}">
							<Tag :color="styleColor" class="tag">套餐</Tag>
							<div class="img-box" :style="{
								'background-image' : 'url('+item.img_url_thumb+')',
								height: ( 375/(info.layout == 'one' ? 1 : (info.layout == 'two' ? 2 : 3)) - info.gutter ) +'px'
							}"></div>
							<div v-if="info.content_switch.show_time" class="act_time">距结束 00:00:00</div>
		...
		<div v-else class="empty">
			<Icon type="ios-cube-outline" size="60" color="#2d8cf0"></Icon>
			<div>请点击编辑 搭配套餐活动</div>
		</div>
	</div>
</template>
		...
		methods: {
			...
			initGoodsList(){
				if( this.info.activity_model == 'auto' ){
					...
					return this.$ajax.post(this.$api.goodsPackageList,{
						status: -1,
						page: 1,
						pageSize: show_num
					}).then((response)=>{
						...
						if(res.code){
							...
							for(let i = 0; i < items.length; i++){
								goodsList.push({
									id: items[i].id,
									goods_id: 0,
									img_url_thumb: items[i].package_bg_image,
									name: items[i].package_name,
									sale_price: items[i].max_sale_price,
									market_price: items[i].max_sale_price,
								})
							}
