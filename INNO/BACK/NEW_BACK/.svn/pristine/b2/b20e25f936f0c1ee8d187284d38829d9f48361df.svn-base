...
export default {
  ...
  watch: {
    orderGoods(newVal) {
      ...
      const send_numbers = this.tableData.map(item => item.goods_send_num).join(',');
      const rec_ids = this.tableData.map(item => item.id).join(',');
              ...
              h('Input', {
                ...
                on: {
                  'on-change' (e) {
                    ...
                    const rec_ids = that.tableData.map(item => item.id).join(',');
