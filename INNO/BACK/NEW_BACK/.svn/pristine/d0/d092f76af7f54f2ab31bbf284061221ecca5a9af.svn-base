...
export default {
  ...
  methods: {
    ...
    confirmPop(id, admin_remark) {
      ...
      this.spinShow = true;
      return this.$ajax.post(this.$api.refundRecycleBinRemark, {
        id,
        admin_remark
      }).then(response => {
            ...
            scopedSlots: {
              orderInfo: ({ row }) => {
                return <div class="info_wrap">
                  {
                    row.refund_info.map((item, index) => {
                      return (
                        <div class="goods_wrapper">
                          ...
                          <div class="refund">
                            <p class="detail">退款原因: <span>{this.rowData.refund_reason_str}</span></p>
                          </div>
                          {row.refund_info.length - 1 == index ? <span></span> : <Divider class="common_divider" />}
                        </div>
                      )
                    })
                  }
              ...
              handle: ({ row }) => {
                return <div class="handle_content">
                  {this.rowData.handle.edit ? <router-link tag="a" to={{ name: 'refund-recycle-info', params: { sn: this.rowData.id } }} target="_blank" class="edit_item">查看</router-link> : <span></span>}
                  {row.handle.remarks ?
