...
export default {
  ...
  methods: {
    ...
    confirmPop(return_id, admin_remark) {
      ...
      this.spinShow = true;
      return this.$ajax.post(this.$api.ShopReturnOrderRemark, {
        return_id,
        admin_remark
      }).then(response => {
        ...
        <div class="exchange-order-details">
          <Table columns={this.tableColums} data={this.tableData} show-header={this.isShowHeader}  {...{
            scopedSlots: {
                    ...
                    row.return_info.map((item, index) => {
                      return (
                        <div class="goods_wrapper">
                          ...
                          <div class="refund">
                            <p class="detail">换货原因: <span>{this.rowData.return_reason}</span></p>
                            <div class="detail">
                              <p>更换规格：<span>{item.exchange_arr}</span></p>
                            </div>
                          </div>
              ...
              returnTime: () => {
                return <div>
                  <p>{this.rowData.return_time.split(' ')[0]}</p>
                  <p>{this.rowData.return_time.split(' ')[1]}</p>
                </div>
              },
              user: () => {
                return <div>{this.rowData.user_name}</div>
              },
              updateTime: () => {
                return <div>
              ...
              handle: ({ row }) => {
                return <div class="handle_content">
                  {this.rowData.handle.edit ? <router-link tag="a" to={{ name: 'shop-exchange-info', params: { sn: this.rowData.return_id } }} target="_blank" class="edit_item">查看</router-link> : <span></span>}
                  {this.rowData.handle.newOrder ? <router-link tag="a" to={{ name: 'shop-order-info', params: { sn: this.rowData.new_order_id } }} target="_blank" class="edit_item">查看新订单</router-link> : <span></span>}
                  {row.handle.remarks ?
