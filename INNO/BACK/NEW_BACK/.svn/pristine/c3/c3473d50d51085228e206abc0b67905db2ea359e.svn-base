...
<script>
	 ...
	 * 站点页面编辑器，主框架
	 */
	import editorHeader from '@/views/plugins/pages/editor-header';
	import editorLeftMenu from '@/views/plugins/pages/editor-left-menu';
	import editorRightSide from '@/views/plugins/pages/editor-right-side';
	...
	export default {
		...
		methods: {
			...
			init() {
				this.$nextTick(() => {
					...
					if (this.pageType == 'userCenter') {
						this.data.appId = this.$util.cache.get('cloudshop_page_app_id');
			...
			initUserCenter() {
				...
				// ajax 请求获取数据
				this.$ajax.post(this.$api.cloudGoodsPageInfo, {
			...
			// 切换小程序 AppID
			onAppIdChange(){
				this.$util.cache.set('cloudshop_page_app_id', this.data.appId);
			...
			onPageSave() {
				...
				// ajax 提交数据
				this.$ajax.post(this.$api.cloudGoodsPageEdit, {
						goods_id: this.goodsId,
						page_id: this.pageId,
			...
			// 返回列表
			goBack() {
				if( this.$route.fullPath == '/cloud-shop/cloud-goods-cat-page' ){
