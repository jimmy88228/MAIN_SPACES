  ...
  <PageTopBase>
    ...
    <transition-group name="fade">
      ...
      <div v-show="!showBaisc" key="goods" class="group-activity-select-wrapper">
        <Button type="dashed" @click="handleSelect" class="basic_select" v-if="!id || this.formItem.goodsSelect== ''">选择商品</Button>
        <goodsSelect
          :id="id"
          :data="formItem.goodsSelect"
          :storeGoods="formItem.storeGoodsSelect"
          @get-spec-data="handleSpecData"
          @clear-goods-data="handleClear"
          @on-validate-table="handleValidateTable"
          v-show="formItem.goodsSelect.length"/>
...
export default {
  ...
  methods: {
    ...
    loadData () {
      this.spinShow = true;
      return this.$ajax.post(this.$api.cloudGroupActivityInfo, {
        activity_id: this.id
      })
      .then(response => {
        ...
        if (res.code) {
          ...
          if (data) {
            ...
            this.formItem = {
              ...
              storeGoodsSelect: data.activity_store_goods,//门店商品库存
              goodsSelect: data.get_collage_group_goods.length != 0 ? [
                {
    ...
    confirm () {
      ...
      this.$refs.formValidate.validate(valid => {
        if (valid && this.isValidTable) {
            console.log('表单提交：', this.formItem.storeGoodsSelect);
          this.spinShow = true;
          return this.$ajax.post(this.id ? this.$api.cloudGroupActivityEdit : this.$api.cloudGroupActivityAdd, {
            activity_id: this.id,
            ...
            'store_goods_stock': this.formItem.storeGoodsSelect
          })
          .then(response => {
            const res = response.data;
            if (res.code) {
