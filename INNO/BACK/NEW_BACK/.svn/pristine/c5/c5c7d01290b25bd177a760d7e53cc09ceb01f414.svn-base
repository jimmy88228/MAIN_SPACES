		...
		<Row type="flex" style="margin-bottom:10px;">
			<Col style="flex:1 1 0%;">
				<Tooltip content="返回" placement="bottom-start">
					...
					<span style="margin-left:30px">秒杀活动 - 关联店铺列表</span>
				</Tooltip>
			</Col>
...
export default {
	...
	methods: {
		// 初始化
		openModal( activityId ) {console.log('哈哈哈：');
		...
		init(){
			...
			this.$ajax.post(this.$api.cloudSeckillActivityStoreList, {
				isInit: 1,
		...
		removeItem( index, row ){
			this.$Modal.confirm({
				...
				onOk: () => {
					// ajax提交数据
					this.tableLoading = true;
					this.$ajax.post(this.$api.cloudSeckillActivityStoreRemove, {
		...
		onStoreOk( items ){
			...
			// ajax提交数据
			this.tableLoading = true;
			this.$ajax.post(this.$api.cloudSeckillActivityStoreAdd, {
				store_ids: storeIds,
				activity_id: this.currActivityId,
			})
			.then(response => {
