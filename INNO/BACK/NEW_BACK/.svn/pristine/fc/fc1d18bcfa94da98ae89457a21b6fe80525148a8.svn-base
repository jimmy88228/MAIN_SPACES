...
<template>
	<div class="pin-sale-form">
		<titleBar>拼团组件 设置</titleBar>
		...
		<Form ref="formValidate" :model="formItem" :rules="ruleValidate" label-position="top">
			...
			<div style="background-color: #eee;padding:5px 5px 0 5px;border-radius: 4px;">
				...
				<FormItem v-if="formItem.activity_model == 'manual' " label="">
					...
					<pin-select type="checkbox">
						<Button long @click="handleSelect">
		...
		<!--统一活动头-->
		<activityHeaderForm :currIndex="currIndex" headerTitle="拼团"></activityHeaderForm>
	</div>
</template>
				...
				for( var i in obj ){
					if( arrGoodsIds.indexOf(obj[i].id) == -1 ){
						this.formItem.goodsList.push({
							id: obj[i].id,
							img_url: obj[i].img_url,
							img_url_thumb: obj[i].active_image,
							name: obj[i].name,
							sale_price: obj[i].min_sale_price,
							market_price: obj[i].min_market_price,
							code: obj[i].code,
							spu: obj[i].spu,
						});
					}
