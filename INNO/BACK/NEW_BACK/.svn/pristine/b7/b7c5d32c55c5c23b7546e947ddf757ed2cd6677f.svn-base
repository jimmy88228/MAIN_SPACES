...
<template>
	<div class="goods-list-form-goods-item">
						...
						<div v-for="(item,index) in formItem.goodsList" :name="index" :key="index" class="goods-item-list">
							...
							<Row :gutter="10">
								...
								<div class="goods-name clamp">
									{{item.goods_sn!='' ? '['+item.goods_sn+']' : ''}} {{item.name}}
								</div>
								<div>
									
									<span v-if="item.market_price != item.sale_price" style="text-decoration: line-through;">￥{{item.view_market_price}}</span>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.formItem = dataList[this.currIndex].setting.goodsGroup[this.groupIndex];
			...
			onSelectGoods() {
				...
				let goodsList = this.formItem.goodsList || [];
				...
				this.$selectModule({
					...
					getList:(data)=>{
						...
						for (var i in data) {
							if (arrGoodsIds.indexOf(data[i].id) == -1) {
								let item = data[i] || {};
			...
			// 			this.formItem.goodsList.push({
			// 				id: obj[i].id,
			// 				goods_thumb: obj[i].goods_thumb2,
			// 				name: obj[i].name,
			// 				sale_price: obj[i].shop_price,
			// 				market_price: obj[i].market_price,
			// 				goods_sn: obj[i].goods_sn,
			// 				goods_brief: obj[i].goods_brief,
			// 				tag: '',
			// 			});
			// 		}
			// 	}
