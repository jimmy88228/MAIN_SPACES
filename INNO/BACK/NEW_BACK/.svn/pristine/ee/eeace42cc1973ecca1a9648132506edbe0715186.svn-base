			...
			<Row type="flex">
				...
				<div class="coupon-basic-info">
					...
					<Form ref="formValidate" :model="formItem" :rules="ruleBasicValidate" :label-width="140" v-friendly-errors>
						...
						<FormItem label="领券冻结时间" prop="fullDiscount" v-if="formItem.validityTime.validityLimitType != 1">
							<div class="discount_wrapper">
								<FormItem prop="fullDiscount.amount" :rules="freezeTimeRule">
									...
									<Checkbox v-if="this.$route.query.pid != 0" v-model="formItem.validityTime.invalid_time_show_code" true-value="1" false-value="0">券冻结期间前端不隐藏优惠券号</Checkbox>
								</FormItem>
							</div>
							<p class="strong_tips">*券冻结时间从领券开始计算，优惠券冻结时间内不能使用</p>
						</FormItem>

						<FormItem label="是否开启自叠加" prop="canOverlying" v-if="formShowList.canOverlying">
						...
						<!--是否特定券-->
						<FormItem label="是否特定券" prop="noOverlyingBonus" v-if="formShowList.noOverlyingBonus">
							<RadioGroup v-model="formItem.noOverlyingBonus">
								<Radio label="1" :disabled="isEditStaus">是</Radio>
								<Radio label="0" :disabled="isEditStaus">否</Radio>
							</RadioGroup>
							<p class="strong_tips">如果90元券、100元券、5元券；仅允许90元+5元或100元+5元叠加使用；则90元和100元券配置为特定券</p>
						</FormItem>
						...
						if (res.code) {
							...
							this.formItem = {
								...
								validityTime: {
                                    ...
                                    invalid_time_show_code: invalid_time_show_code
								}
							};
