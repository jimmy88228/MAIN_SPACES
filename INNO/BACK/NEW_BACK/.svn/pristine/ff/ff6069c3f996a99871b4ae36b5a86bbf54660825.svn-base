...
<template>
    <div class="goods-price-explain">
        <Card>
            <div slot="title" class="title">
               	<Icon type="md-cube"></Icon> 商品价格说明
            </div>
            ...
            <div class="info-body">
		        <Form ref="formValidate" :model="formItem" :rules="ruleValidate" :label-width="120">
			        ...
			        <Row :gutter="30">
		        		<Col :span="12">
					       <FormItem label="是否开启：" prop="is_show_price_explain">
								<RadioGroup v-model="formItem.is_show_price_explain">
			        ...
			        <Row :gutter="30">
						<Col :span="12">
					        <FormItem label="广告位：" prop="logo">
					        	<div class="image-box" @click="openImagesModal('goods_price_explain', formItem.goods_price_explain )" :style="'background-image: url('+formItem.goods_price_explain+');'">
						            <Icon type="md-add" size="30" v-show="(formItem.goods_price_explain==''?true:false)"></Icon>
						        </div>
					        </FormItem>
			        	</Col>
			        </Row>
...
export default {
  ...
  methods: {
      ...
      // ajax 请求获取初始化数据，然后动态更新下面数据源
        	this.$ajax.post(this.$api.goodsPriceExplain, {
    ...
    modalOk () {
        ...
        if (valid) {
                	...
                	// ajax 保存编辑数据
                	this.$ajax.post(this.$api.goodsPriceExplainSave, {
        				is_show_price_explain: this.formItem.is_show_price_explain,
						goods_price_explain: this.formItem.goods_price_explain
	            	})
		    		.then((response) => {
		    			var res = response.data;
		    			this.save_loading = false;
