		...
		<div>
			<Form ref="formValidate" :model="formItem" :rules="ruleValidate" :label-width="140">
				...
				<FormItem label="分销身份" prop="dstb_staff_level">
				 ...
				 <FormItem label="分销等级" prop="staff_dstb_rank_id" show-word-limit>
					<Select v-model="formItem.staff_dstb_rank_id" class="basic_select">
						<Option v-for="item in this.formItem.staffRankData" :value="item.id" :key="item.id">{{ item.name }}</Option>
					</Select>
				</FormItem>
...
<script>
	...
	export default {
		...
		methods: {
			loadData() {
				this.spinShow = true;
				return this.$ajax.post(this.$api.distributionStaffDetailsInfo, {
					...
					.then(response => {
						...
						if (res.code) {
							...
							let staffRankData = res.data && res.data.staff_rank_data;
							...
							if (data) {
								this.formItem = Object.assign({}, data, {
                                    ...
                                    staff_dstb_rank_id: Number(data.staff_dstb_rank_id),
								});
							}
						}
						this.spinShow = false;
			...
			confirm() {
				this.$refs.formValidate.validate((valid) => {
					if (valid) {
						this.spinShow = true;
						this.$ajax.post(this.id ? this.$api.distributionStaffEdit : this.$api.distributionStaffAdd, {
								...this.formItem,
								dstb_staff_id: this.id,
