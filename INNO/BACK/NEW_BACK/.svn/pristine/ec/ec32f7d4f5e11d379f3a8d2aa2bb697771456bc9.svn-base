<template>
	<div class="order-info">
		<Card>
				...
				<Row class="order_header_inner">
					...
					<div class="order_steps">
						<Steps :current="currentOrderType">
							<Step title="用户领取" :content="firstStep"></Step>
							<Step title="商家发货" :content="secondStep"></Step>
							<Step title="确认收货" :content="thirdStep"></Step>
			...
			<action-details :action-list="actionList" :order-button="orderButton" :refund-message="refundMessage" :sn="sn"
...
<script>
	...
	export default {
		...
		methods: {
			loadData() {
				this.spinShow = true;
				return this.$ajax.post(this.$api.MatrixOrderInfo, {
					order_id: this.sn,
					orderTypeValue: ''
				}).then(response => {
					...
					if (res.code) {
						...
						this.orderMoney = orderMoney instanceof Array ? {} : orderMoney;
