<template>
	<div class="coupon-list">
		...
		<Table :loading="tableLoading" :height="tableHeight" :columns="tableColumns" :data="proxyData" ref="myTable"
		 @on-selection-change="getSelectGoods" @on-select-all="handleSelectAll">
			<template slot-scope="{ row }" slot="name">
				<div class="img_list_wrap">
					<div class="img_fixed">
						...
						<img src="@rs/images/default-img.jpg" :alt="row.type_name" v-viewer v-else/>
					</div>
					<div>
						<p class="name">{{row.type_name}}</p>
						<p class="name" v-if="row.bonus_type_id == 1 || row.bonus_type_id == 4">({{row.bonus_type_id == 1 ? '满减券' : (row.bonus_type_id == 4 ? '折扣券' : '实物券')}})</p>
					</div>
				</div>
			</template>
			...
			<template slot-scope="{ row }" slot="sendType">
				<p v-if="row.send_type_id == 0">微商城优惠卷</p>
				<p v-else-if="row.send_type_id == 3">扫码支付优惠卷</p>
				<p v-else-if="row.send_type_id == 4">ERP券</p>
				<p v-else-if="row.send_type_id == 5">通用券</p>
			</template>
			...
			<template slot-scope="{ row }" slot="handle">
				<div v-if="$route.query.act === 'coupon-list'">
					<div>
						...
						<span v-show="row.handle.edit" @click="handleEdit(row.type_id, row.send_type_id, row.bonus_type_id)"><a>编辑</a></span>
						<Divider type="vertical" v-show="row.handle.copy" />
						<span v-show="row.handle.copy" @click="handleCopy(row.type_id, row.send_type_id, row.bonus_type_id)"><a>复制</a></span>
					</div>
					<div>
					...
					<div>
						...
						<!--<span v-show="row.handle.reel" @click="exportCopous(row.type_id)"><a>导出券号</a></span>--> <!--handleExport(row.type_id)-->
					</div>
				</div>
				<div v-else>
		...
		<Modal
				v-model="exportCoupon.exportCouponShow"
				title="导出券号"
				@on-ok="makeSureCoupon"
				@on-cancel="cancelCoupon">
			<p style="margin: 10px auto;">确定进行导出券号操作</p>
			<RadioGroup v-model="exportCoupon.exportCouponType">
				<Radio label="1">全量导出券号</Radio>
				<Radio label="2">仅导出后续追加券号</Radio>
			</RadioGroup>

			<div style="margin: 15px auto;" v-if="exportCoupon.isShowTimeRange">
				<span style="color: red;">*</span>
				<span>追加日期：</span>
				<DatePicker v-model="exportCoupon.validTimeRange" style="width: 350px;" type="datetimerange" placeholder="请选择追加日期" class="time_range" @on-change="handleChange"></DatePicker>
			</div>
		</Modal>
	</div>
</template>
<script>
	...
	export default {
		...
		watch:{
            'exportCoupon.exportCouponType': function (newVal,oldVal) {
             
                this.exportCoupon.isShowTimeRange = newVal == 2 ? true : false;
            },
		},
		...
		methods: {
            ...
            handleChange ([start_time, end_time]) {
                this.exportCoupon.validTimeRange = [start_time, end_time];
            },
            makeSureCoupon () {
                return this.$ajax.post(this.$api.exportCoupons, {
                    id: this.exportCoupon.type_id,
                    type: this.exportCoupon.exportCouponType,
                    begin_end_time: this.exportCoupon.validTimeRange,
				})
				.then(response => {
			...
			handleEdit(typeId, pid, id) {
				window.open("/plugins/coupon-details/"+typeId+"?pid="+pid+"&id="+id+"&isEdit=1");
				/*
				this.$router.push({
					name: 'coupon-details',
					params: {
            ...
            exportCopous(type_id){
				this.exportCoupon.type_id = type_id;
				this.exportCoupon.exportCouponShow = true;
			},
			handleExport(id) {
				this.$Modal.confirm({
					title: '操作提示',
