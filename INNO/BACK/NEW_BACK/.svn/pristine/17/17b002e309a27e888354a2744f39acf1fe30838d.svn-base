<template>
  <div class="label-list">
    <Card>
      <list-head title="基础标签" ref="listHead" @on-labelHandle="labelHandle" @on-groupHandle="groupHandle" @on-success="successHandle" :catArr="catArr" :canAction="canCreate"></list-head>
      <div>
...
export default {
  ...
  methods: {
    onLoadData(page, reqData){
        ...
        return util.ajax.post(util.apiUrl.BasicLabelList, params).then(e =>{
            let res = e.data || {};
            if(res.code) {
        let data = res.data || {};
