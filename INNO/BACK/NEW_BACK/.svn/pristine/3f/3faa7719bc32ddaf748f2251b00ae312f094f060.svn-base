      ...
      set (val) {
        if (this.checkType === 1) {
          this.formDynamic.pageData.forEach((item,index) => {
            item._checked = val;
            console.log("index", index,'val: ', val, this.formDynamic.pageData);
    ...
    confirm () {
      this.$refs.formDynamic.validate(valid => {
        if (valid) {
          ...
          console.log('confirm: ', this.allData, 'hahah :', flatten);
    ...
    setData () {
      ...
      this.$nextTick(() => {
          ...
          for(let i = 0; i < data.length; i++){
              this.$set(this.rowData[i], '_checked', false);
              this.$set(this.rowData[i], 'inventory', Number(data[i].inventory));
          }
          this.$set(this.formDynamic, 'pageData', this.rowData);
      ...
      if (this.currentBatchType !== 'enable' && !this.batchValue) {
        ...
        for (let i = 0; i < pageDatas.length; i++) {
            if (ids.includes(pageDatas[i]['id'])) {
                pageDatas[i]['inventory'] = this.batchValue;
            }
        }
          this.$emit('edit-norms', pageDatas);
