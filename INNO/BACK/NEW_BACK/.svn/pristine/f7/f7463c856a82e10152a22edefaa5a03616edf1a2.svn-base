...
export default {
  ...
  methods: {
    onLoadData (page, data) {
      return this.$ajax.post(this.$api.goodsServiceList, data)
      .then(response => {
    ...
    onDelItem (row) {
      return this.$ajax.post(this.$api.goodsServiceRemove, {
        id: row.id
      });
    },
    // 开启/关闭
    updateEnabled (index, row) {
      ...
      return this.$ajax.post(this.$api.goodsServiceIsenable, {
        id: row.id,
