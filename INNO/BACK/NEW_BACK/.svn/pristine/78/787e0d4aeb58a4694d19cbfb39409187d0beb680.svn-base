        ...
        <div>
          ...
          <template v-if="orderGoodsTotal.order_type !== 2">
            <div style="margin-bottom: 20px;">
              ...
              <InputNumber  :max="Number(orderGoodsTotal.return_totalamount)" :step="1.2" v-model="returnMoney"  placeholder="请输入金额" class="basic_input"></InputNumber>
            </div>
          </template>
          <!-- 预售订单 -->
          <template v-if="orderGoodsTotal.order_type == 2">
...
export default {
  ...
  methods: {
    ...
    handleForBack (key, params) {
      this.loading = true;
      return this.$ajax.post(this.$api.ShopReturnOrderUpdate, {
        return_id: this.sn,
        button_type: key,
  ...
  watch: {
    ...
    orderGoodsTotal (nV) {
      this.returnMoney = parseFloat(nV.return_totalamount);
