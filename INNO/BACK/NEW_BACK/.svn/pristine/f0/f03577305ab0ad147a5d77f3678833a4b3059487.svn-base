    ...
    <div class="content">
      ...
      <div class="footer">
        <div class="action">
          ...
          <div style="display: inline-block;" v-if="formItem.is_enabled == '0'">
            <span>状态：</span>
            <Button type="primary" @click="batching('enable')">批量开启</Button>
            <Button type="primary" @click="batching('unenable')">批量关闭</Button>
          </div>
  ...
  methods: {
    ...
    batching(type){
        if (!this.selectData.length) {
            ...
            let ids = this.selectData.map(item => item.goods_id);
            let goodsData = this.formDynamic.pageData || [];
            for(let i = 0; i < goodsData.length; i++){
                let products = goodsData[i].get_products || [];
                for(let j = 0; j < products.length; j++){
                    ...
                    this.$set(this.formDynamic.pageData[i].get_products[j], 'is_enabled', is_enabled);
                    this.$set(this.formDynamic.pageData[i].get_products[j], '_enabled', is_enabled);
                }
            }
        }
    },
