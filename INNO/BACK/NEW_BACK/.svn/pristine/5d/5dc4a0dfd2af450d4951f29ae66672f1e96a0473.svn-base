  ...
  <PageTopBase>
    ...
    <transition-group name="fade">
      ...
      <div v-show="!showBaisc" key="goods" class="bargin-activity-select-wrapper">
        ...
        <goodsSelect :data="formItem.goodsSelect" :storeGoods="formItem.storeGoodsSelect" @get-spec-data="handleSpecData" @clear-goods-data="handleClear" v-show="formItem.goodsSelect.length"/>
      </div>
    </transition-group>
...
export default {
  ...
  methods: {
    loadData () {
      this.spinShow = true;
      this.$ajax.post(this.$api.cloudBargainActivityInfo, {
        id: this.id
      })
      .then(response => {
    ...
    confirm () {
      ...
      this.spinShow = true;
      return this.$ajax.post(this.id ? this.$api.cloudBargainActivityEdit : this.$api.cloudBargainActivityAdd, {
        ...this.formItem,
        id: this.id,
        goods_data_state: 2,
        ...
        page_id: this.formItem.pages.id,
