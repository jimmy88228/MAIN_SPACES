<template>
	<div class="big-order-search-form">
		<Form ref="formSearch" :model="formSearch" inline :label-width="90">
			<FormItem :label-width="0" class="search_wrapper">
				<Input v-model="formSearch.keywords" style="width:320px;" placeholder="关键词搜索" clearable search enter-button
				...
				<div v-show="isShowExtra">
					...
					<Row>
						...
						<FormItem label="所属店铺" prop="store_id">
							<Select v-model="formSearch.store_id" class="basic_select">
								<Option v-for="(val, key) in storeList" :value="key" :key="key">{{ val }}</Option>
							</Select>
						</FormItem>
						</Col>
					</Row>
