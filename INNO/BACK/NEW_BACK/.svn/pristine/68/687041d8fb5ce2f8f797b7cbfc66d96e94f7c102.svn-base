		...
		<Row type="flex" style="margin-bottom:10px;">
			<Col style="flex:1 1 0%;">
				<Tooltip content="返回" placement="bottom-start">
					...
					<span style="margin-left:30px">云店满减规则关联店铺列表</span>
				</Tooltip>
			</Col>
...
export default {
	...
	methods: {
		...
		init() {
			...
			this.$ajax.post(this.$api.cloudFullReductionStoreList, {
				isInit: 1,
				rule_id: this.currPageId,
			})
			.then(response => {
		...
		removeItem( index, row ){
			this.$Modal.confirm({
				...
				onOk: () => {
					// ajax提交数据
					this.tableLoading = true;
					this.$ajax.post(this.$api.cloudFullReductionStoreRemove, {
		...
		onStoreOk( items ){
			...
			// ajax提交数据
			this.tableLoading = true;
			this.$ajax.post(this.$api.cloudFullReductionStoreAdd, {
				store_ids: storeIds,
				rule_id: this.currPageId,
			})
			.then(response => {
