<template>
  <div class="label-list">
    <Card>
      <list-head title="商品销售标签" ref="listHead" @on-labelHandle="labelHandle" @on-groupHandle="groupHandle" @on-success="successHandle" :catArr="catArr" :canAction="canCreate"></list-head>
      <div>
        <Table  :height="tableHeight" :columns="columns" :data="tableData" ref="myTable">
          ...
          <template slot-scope="{ row }" slot="is_auto_label">
            <Icon type="ios-close-circle-outline" size="25" color="#CE3636" v-if="row.is_auto_label == '0'"/>
...
export default {
  ...
  methods: {
    onLoadData(page, reqData){
        ...
        return util.ajax.post(util.apiUrl.SaleLabelList, params).then(e =>{
            let res = e.data || {};
            if(res.code) {
        let data = res.data || {};
...
function submitGroupReq(data){
  this.showSpan = true;
  return util.ajax.post(util.apiUrl.SaleLabelAddGroup, {
...
function changeStatusReq(data){
  this.showSpan = true;
  return util.ajax.post(util.apiUrl.SaleLabelStatus, data).then(e =>{
    let res = e.data || {};
    if(res.code && res.data) {
...
function submitEditReq(data, type){
  ...
  return util.ajax.post(util.apiUrl.SaleLabelEdit, params).then(e =>{
    let res = e.data || {};
    if(res.code && res.data) {
...
function submitRemoveReq(data, type){
  ...
  return util.ajax.post(util.apiUrl.SaleLabelRemove, params).then(e =>{
    let res = e.data || {};
    if(res.code && res.data) {
      this.$Message.info(res.message || '编辑成功!');
      return this.loadData();
