<template>
  <div class="cat-label-goods">
    <Card>
      <Row type="flex" style="margin-bottom: 10px;">
        <Col style="flex:1 1 0%;">
        <Button type="info"  @click="backLink">返回</Button>
        <span style="color:red">*手动导入商品、自定义标签商品各为独立模块，互不影响</span>
          <!-- <Input
            class="basic_input"
            v-model="condition.searchq"
            placeholder="请输入款号，多个用英文逗号拼接"
            clearable
            search
            enter-button
...
export default {
  ...
  methods: {
    onLoadData (page, data) {
      ...
      params.vcat_id =this.vcat_id;
      return this.$ajax.post(this.$api.goodsVcatTagGoods, params)
      .then(response => {
    ...
    onDelItem (row) {
      return this.$ajax.post(this.$api.goodsVcatTagGoods, {
        id: row.id
      });
    },
    createActivity () {
  ...
  mounted () {
    ...
    // console.log(data)
    // this.vcat_id = data.id;
    let query = this.$route.query || {};
		this.vcat_id = query.id || this.vcat_id;
    this.loadData();
  }
}
</script>
