...
<template>
	<div class="user-audio-modal-body">
    	<!--上传按钮-->
    	<div v-if="can.canUpload" class="upload-btn-box">
			...
			:on-success="posterUploadSuccess">
				<div>
		            <Button type="info" icon="md-add">上传音频</Button>
		       	</div>
		    </Upload>

    		<Input v-model="searchq" placeholder="搜索音频备注关键词"  style="width:200px;float:right;" search enter-button clearable
				@on-search="goSearch"
				@on-clear="goSearch"
    	...
    	<Row :gutter="8">
			<Col span="4">
        		...
        		<div class="image-cat" style="border-top:1px solid #ddd;padding-top:5px;">
        			...
        			<!--添加分类气泡-->
        			<Poptip v-model="showAddCatPop" placement="bottom-start" width="220" style="margin-left:10px;">
	        			<div class="cat-item" style="background:#2db7f5;color:#fff;font-size:12px;padding:4px 10px;" v-if="can.canAddCat" @click="onAddCat">
			...
			<Col span="20">
	        	...
	        	<div class="scroll-content" :style="getContentHeight">
	        		...
	        		<Row class="list-box">
						...
						<!--数据库中的列表（可以操作）-->
		        		<Col v-for="(item,index) in list" :key="('e'+index)" :id="('axx'+index)" span="6"
		        			:class="itemClass(item)" @click.native="selectedAudio('axx'+index, item.img_src_format)">
				        	<div class="act-thumb" title="请点击选择音频">
				        		<Icon type="ios-play" color="#fff" size="35"></Icon>
				        		<Icon v-if="can.canRemove" type="md-close-circle" class="close" @click="removeAudio(item.id, 'list', index)" title="删除音频"></Icon>
				        		<Icon type="md-arrow-dropright-circle" class="play" @click="playAudio(item.id, item.img_src_format)" title="播放音频"></Icon>
				        		<Icon class="checked-icon ionmy ion-my-checked"></Icon>
				        	</div>

				        	<div class="img-remark-box" title="请点击选择音频">
		...
		<Modal
	        ...
	        title=""
	        :styles="{top: '20px'}"
	        class="audio-list-modal"
	        :loading="modalPlayLoading"
	        footer-hide>
			<!--播放器-->
			<video-player ref="audioPlayer" :options="playerOptions"></video-player>
		</Modal>
    </div>
</template>
...
export default {
  ...
  methods: {
    	init () {
      // 初始化上传地址(提供给组件使用)
    		this.posterUploadUrl = util.apiUrl.uploadImages + '?access-token=' + Cookies.get('accessToken');

    		this.fromMenu == 1 ? this.pageSize = 40 : '';
    	},
    	// 初始化数据
    	initData (res, multi, multiSelectedUrlBind, selectedUrl) {
    		this.searchq = '';
      this.multi = multi;
      this.multiSelectedUrlBind = multiSelectedUrlBind;
    ...
    itemClass (item) {
        	return 'list-item ' + (this.multi == 1 ? 'multi' : '') +
        			((this.selectedUrl != '' && item.img_src_format == this.selectedUrl) ? ' curr' : '') +
        			((this.multiSelectedUrlBind.length > 0 && this.multiSelectedUrlBind.indexOf(item.img_src_format) !== -1) ? ' disabled' : '');
    },
    // 选中音频
    selectedAudio (index, url) {
        	this.$emit('on-selected-audio', index, url);
    },
    // 更新备注信息
    ...
    posterMaxSize (file) {
      this.$Notice.warning({
        title: '超过了最大文件限制',
        desc: '文件  ' + file.name + ' 超过了 20M，请控制在20M以内.'
      });
    },
    posterFormatError (file) {
        	this.$Notice.warning({
        title: '音频格式错误',
        desc: '文件 ' + file.name + ' 格式不正确, 请选择 mp3 音频文件'
      });
    },
    // ajax上传前
    posterBeforeUpload () {
    ...
    // 删除音频
    removeAudio (id, type, index) {
        	this.$Modal.confirm({
        title: '删除提示',
        content: '如果音频在使用中，删除会影响到使用中的项目，确定无使用再删除，删除吗？',
        onOk: () => {
		        	this.spinShow = true;
    ...
    onRemoveCat (index, id) {
      this.$Modal.confirm({
        title: '删除提示',
        content: '确定删除素材分类吗？无音频文件的分类才能删除成功。',
        okText: '确定删除',
        cancelText: '取消',
        onOk: () => {
          this.spinShow = true;
