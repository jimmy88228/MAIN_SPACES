  ...
  <PageTopBase>
    ...
    <transition-group name="fade">
      ...
      <div v-show="showMan.showGoods" key="goods" class="seckill-activity-select-wrapper">
        ...
        <goods-page-table ref="goodsTable" :id="id" :goods-data="selectedData" @get-data="handlePageData" @get-edit-data="handleEditData"></goods-page-table>
      </div>
    ...
    <multi-norms ref="multiNorms" title="选择可用商品" :selected="selectedData" :customSpec="false" @get-data="handleData"></multi-norms>
...
export default {
  ...
  methods: {
    loadAdData () {
      this.spinShow = true;
      return this.$ajax.post(this.$api.seckillActivityAdvertInfo, {
        id: this.id
      })
      .then(response => {
    ...
    loadData () {
      this.spinShow = true;
      return this.$ajax.post(this.$api.seckillActivityInfo, {
        id: this.id
      })
      .then(response => {
        const res = response.data;
      ...
      this.$refs.adPage.getData((adData) => {
        this.$refs.formValidate.validate(valid => {
          if (valid) {
            ...
            this.spinShow = true;
            return this.$ajax.post(this.id ? this.$api.seckillActivityEdit : this.$api.seckillActivityAdd, {
              ...this.formItem,
