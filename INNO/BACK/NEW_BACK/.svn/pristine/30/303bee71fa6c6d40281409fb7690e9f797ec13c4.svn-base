          ...
          <Form ref="formSearch" :model="formSearch" inline>
            <FormItem>
              <Input
                ...
                <Select v-model="formSearch.type" @on-change="editplaceholder" style="width:150px;" slot="prepend" class="goods-search_select">
                  <Option :value="1">颜色分类</Option>
                  <Option :value="2">颜色名称</Option>
                </Select>
                </Input>
            </FormItem>
          </Form>
...
export default {
  ...
  methods: {
    ...
    init () {
      ...
      util.ajax.post(util.apiUrl.goodsColorList, {
        isInit: 1,
        page: this.curPage,
    ...
    editplaceholder(){
      if(this.formSearch.type==1){
        this.formSearch.note="颜色分类 模糊查询";
      }else{
        this.formSearch.note="颜色名称 模糊查询";
      }
    },
    // 搜索
    searchPage () {
    ...
    removeSpec (index, row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
            this.tableLoading = true;
            // ajax 请求获取数据，然后动态更新下面数据源
            util.ajax.post(util.apiUrl.goodsColorRemove, {
    ...
    confirmExport(){
				return this.$ajax.post(this.$api.goodsColorList,{
					is_export: 1
				}).then((response) => {
						var res = response.data;
						if (res.code) {
