	...
	export default {
		...
		methods: {
			...
			mapArrow(dom) {
				...
				const parent = children[0].childNodes || [];
				const arrow = (children[0].children && children[0].children[0]) || {};
				let title = null;
				try{
					title = children[0].children[1].children[0].children[0];
				} catch(error){
					
				}
				if(!title) return;
				// const title = children[0].children[1].children[0].children[0];
				let childrenCount = arrow.children || [];
				if (childrenCount.length === 0 && Number(title.dataset.index) === 1) {
					// 当前不存在箭头，需要手动添加横杆
