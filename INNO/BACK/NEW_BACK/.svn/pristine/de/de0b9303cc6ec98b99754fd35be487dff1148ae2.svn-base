        ...
        <Form ref="formDynamic" :model="formDynamic">
          <Table :columns="columns" :data="formDynamic.pageData" ref="myTable">
            <template slot-scope="{ row, index }" slot="inventory">
              <FormItem
                :prop="'pageData.' + index + '.inventory'"
                :rules="[{required: true, message: '活动库存不能为空', trigger: 'blur', type: 'number'}]">
                <InputNumber v-model="row.inventory" :min="0" @on-change="e => handleSetData('inventory', index, e)"/>
              </FormItem>
            </template>
            <template slot-scope="{ row, index }" slot="market_price">
              <FormItem
                :prop="'pageData.' + index + '.market_price'"
                :rules="[{required: true, message: '原价不能为空', trigger: 'blur', type: 'number'}]">
                <InputNumber v-model="row.market_price" :min="0" @on-change="e => handleSetData('market_price', index, e)"/>
              </FormItem>
            </template>
            <template slot-scope="{ row, index }" slot="price">
              <FormItem
                :prop="'pageData.' + index + '.price'"
                :rules="[{required: true, message: '秒杀价不能为空', trigger: 'blur', type: 'number'}]">
                <InputNumber v-model="row.price" :min="0" @on-change="e => handleSetData('price', index, e)"/>
              </FormItem>
            </template>
            <template slot-scope="{ row, index }" slot="enable">
              <i-switch :true-value="1" :false-value="0" v-model="row.enable" @on-change="val => handleSetData('enable', index, val ? 1 : 0)">
                <span slot="open">是</span>
                <span slot="close">否</span>
              </i-switch>
            </template>
            <template slot-scope="{ row, index }" slot="is_sellout">
              <i-switch :true-value="1" :false-value="0" v-model="row.is_sellout" @on-change="val => handleSetData('is_sellout', index, val ? 1 : 0)">
                <span slot="open">是</span>
                <span slot="close">否</span>
              </i-switch>
            </template>
          ...
          <div class="action">
            ...
            <template v-if="!idDisabled">
              ...
              <Poptip v-model="showBatchInput" placement="bottom-end">
                <!-- <Button type="primary" @click.native="handleSetBatchType('activeNum')">设置售卖价</Button>
