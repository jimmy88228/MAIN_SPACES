				        ...
				        <Row :gutter="10" class="select-body" :style="getContentHeight">
				        	<Col :span="22" v-for="(item,index) in selectItems" :name="index" :key="index">
				        		<div class="select-box">
				        			...
				        			<div><Avatar :src="item.wx_avatar" icon="md-person" size="large"></Avatar></div>
				        			<div>{{item.wx_nick_name}}{{item.name != null ? item.name : ''}}</div>
				        			<div>{{item.mobile}}</div>
				        		</div>
				        	</Col>
	    		...
	    		<Col :span="20">
	    			...
	    			<div class="table-topbar">
			        	<Form ref="formSearch" :model="formSearch" inline>
			        		...
			        		<FormItem>
					            <Input v-model="formSearch.searchq" style="width:300px;"
                        placeholder="请选择对应类型查询"
...
export default {
  ...
  methods: {
    ...
    searchPage () {
        	...
        	util.ajax.post(util.apiUrl.userList, {
        		isInit: 2,
        		search: this.formSearch.searchq,
            type: this.formSearch.searchqType,
