    ...
    <Row type="flex" style="margin-bottom: 10px;">
      <Col style="flex:1 1 0%;">
        <searchForm
          ...
          :cat-list="catList"
          :vcat-list="vcatList"
          :good-sale-type="goodSaleType"
          :good-sale-kind="goodSaleKind"
          :good-image="goodImage"
          :plat-form="platForm"></searchForm>
      </Col>
...
export default {
  ...
  methods: {
    ...
    // tabs 点击事件
    onTabsClick (name) {
      this.$router.push('/cloud-shop/shop-goods-list#' + name);
      this.tabsName = name;
      // 动态加载列表数据
      this.$nextTick(() => {
    ...
    getData ({goodSaleType, goodSaleKind, goodImage, platForm, canCreate}) {
