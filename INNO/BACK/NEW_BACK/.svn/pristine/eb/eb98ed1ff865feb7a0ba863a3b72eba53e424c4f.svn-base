	...
	export default {
		...
		methods: {
			onLoadData(page, data) {
				...
				return this.$ajax.post(this.$api.cloudPresaleActivityList, params)
					.then(response => {
			...
			handleBatchStatus(val) {
				...
				this.spinShow = true;
				return this.$ajax.post(this.$api.cloudPresaleActivityEditStatus, {
			...
			handleImport(row){
				...
				let DownLoadApi='';
				if(Number(row)==1){
					 UploadApi=this.$api.cloudPresaleActivityUpload;
					 DownLoadApi=this.$api.cloudPresaleActivityDownload;
				}else if(Number(row)==2){
					UploadApi=this.$api.cloudPresaleActivityGoodsUpload;
					DownLoadApi=this.$api.cloudPresaleActivityGoodsDownload;
				}else{
					UploadApi=this.$api.cloudPresaleActivityGoodsUploadByProductSn;
					DownLoadApi=this.$api.cloudPresaleActivityGoodsDownloadByProductSn;
				}
			...
			handleExport(row) {
				this.$Modal.confirm({
					...
					onOk: () => {
						...
						let exportApi=this.$api.cloudPresaleActivityExport;
						//1 活动导出 2 商品导出
						if(Number(row)==1){
							exportApi=this.$api.cloudPresaleActivityExport;
						}else{
							exportApi=this.$api.cloudPresaleActivityGoodsExport;
						}
