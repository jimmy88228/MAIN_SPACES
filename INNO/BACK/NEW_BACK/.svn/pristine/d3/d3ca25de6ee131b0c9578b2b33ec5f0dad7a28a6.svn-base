...
export default {
  ...
  methods: {
    ...
    initData () {
      ...
      return this.$ajax.post(this.$api.ShopGoodsList, {
    ...
    loadExtraData () {
      this.$ajax.post(this.type == 'cat' ? this.$api.ShopCatTree : this.$api.ShopVcatTree)
      .then(response => {
    ...
    modalOk () {
      ...
      this.spinShow = true;
        	this.$ajax.post(this.type == 'cat' ? this.$api.ShopRapidAllocation : this.$api.ShopGoodsVcatRapidAllocation, params)
    		.then((response) => {
    			var res = response.data;
    			if (res.code) {
            // 初始化数据
