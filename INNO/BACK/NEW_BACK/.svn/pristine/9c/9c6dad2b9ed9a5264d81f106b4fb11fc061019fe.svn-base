  ...
  <div class="discount-type-list">
      ...
      <Table :loading="tableLoading" :height="tableHeight" :columns="tableColumns" :data="tableData" ref="myTable">
        ...
        <template slot-scope="{ row, index }" slot="handle">
          ...
          <span v-show="row.handle.remove" @click="delItem(row, '删除提示', '确定删除该优惠券平台类型吗？')"><a>删除</a></span>
        </template>
      </Table>
    ...
    <plat-form-type ref="platFormType" @on-success="onLoadData"/>
  </div>
</template>
<script>
import PlatFormType from './platform-type-form';
...
export default {
  ...
  methods: {
    onLoadData (page, data) {
      ...
      return this.$ajax.post(this.$api.platformTypeList, params)
      .then(response => {
    ...
    openModal (row) {
      ...
      this.$refs.platFormType.setData(row).show();
    },
    editItem (index, row) {
      this.openModal(row);
    },
    onDelItem (row) {
      return this.$ajax.post(this.$api.platformTypeRemove, {
        id: row.id
      });
    }
  }
