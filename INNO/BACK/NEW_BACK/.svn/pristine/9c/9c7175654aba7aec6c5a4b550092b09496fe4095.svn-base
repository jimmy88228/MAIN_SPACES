  ...
  <Card class="visit-page">
      <div class="flex f-just-between f-align-center m-bottom-10">
		<div class="flex f-align-center">
			日期&nbsp;&nbsp;
			<date-select ref="dateSelect" defaultTime="week" dateType="report" class="inline-b space-nowrap" @sT="handleStart" @eT="handleEnd" />
			&nbsp;&nbsp;
			<Input
			class=""
			style="width:250px"
...
export default {
	...
	methods: {
		onLoadData(page, data){
			...
			return util.ajax.post(util.apiUrl.behaviorVisitList, params).then(e =>{
				let res = e.data || {};
				if(res.code) {
					let data = res.data || {};
		...
		handleExport () {
			this.$Modal.confirm({
				...
				onOk: () => {
					...
					return this.$ajax.post(this.$api.BehaviorVisitListExport,{
						...formSearch,
						page: 1,
						pageSize: 20
					}).then((response) => {
