		...
		<PageTopBase isSave @save="confirm">
			<Form ref="formValidate" :model="formItem" :rules="ruleValidate" :label-width="140" label-colon>
				...
				<FormItem label="微企欢迎语" prop="subscribe_notice">
					...
					<div v-for="(item, index) in formItem.activity_msg" style="width: 650px;" :key="index">
						<div v-if="item.msgtype == 'image'" class="msg-type">
							...
							<FormItem label="发送图片" prop="shareImage">
								<image-edit style="display: inline-block;" :img="item.pic_url" @selectImg="openImagesModal('pic_url', item.pic_url, index )" @delImg="handleDelImg('pic_url',index)">
					...
					<div class="add_msg_types">
						<Poptip word-wrap trigger="hover" content="Steven Paul Jobs was an American entrepreneur and business magnate. ">
							<a href="javascript:;">+ 添加图片/网页/小程序</a>
				...
				<FormItem label="企微号限制">
					<wechat-select :data="formItem.wechatData" type="checkbox" @del-tag="handleWechatTag">
						<Button type="dashed" @click="handleSelect('wechatWorker', 'wechatData')" class="basic_select">{{wechatText}}</Button>
					</wechat-select>
					<p class="strong_tips">只有加新的企微号在选择的范围内才执行赠送规则</p>
				</FormItem>
			</Form>
		</PageTopBase>
		<!--加载提示-->
...
<script>
    ...
    export default {
        ...
        computed: {
            couponText () {
                let len = this.formItem.coupons.length;
                return len > 0 ? `选择优惠券(已选${len}张)` : '点我选择优惠券';
            },
			storeText () {
                let len = this.formItem.storeSelect.length;
                return len > 0 ? `选择所属店铺(已选${len}个)` : `选择所属店铺`;
			},
			wechatText () {
                let len = this.formItem.wechatData.length;
                return len > 0 ? `选择企微号(已选${len}个)` : `选择企微号`;
			},
        },
        methods: {
            //删除
            ...
            handleWechatTag (data) {
                this.formItem.wechatData = data;
            },
            loadData() {
				if (this.formItem.id) {
                    this.spinShow = true;
            ...
            confirm() {
                this.$refs.formValidate.validate((valid) => {
                    if (valid) {
                        this.spinShow = true;
                        this.$ajax.post( Number(this.formItem.id)>0?this.$api.addwxAwardEdit:this.$api.addwxAwardAdd, {
							id: this.formItem.id,
