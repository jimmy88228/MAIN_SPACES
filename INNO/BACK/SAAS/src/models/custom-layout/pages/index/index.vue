            ...
            <div class="bg-shadow padding20 list-box">
                <div class="cev-toolbar">
                    ...
                    <div class="right" v-if="true">
                        <Button type="primary" @click="addAd" v-if="$store.state.authJson.btn_add_layout">
                ...
                <Table ref="table" class="table" :columns="columns" :data="list" border>
                    <template slot="action" slot-scope="p">
                        <Button class="item-table-action" type="primary" size="small" @click="edit(p.index)" v-if="$store.state.authJson.btn_edit_layout">编辑</Button>
                        <Poptip confirm title="确定删除该布局？" @on-ok="del(p.index)" transfer v-if="$store.state.authJson.btn_remove_layout">
