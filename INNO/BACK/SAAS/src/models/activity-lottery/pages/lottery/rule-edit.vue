                    ...
                    <!-- <Col class="" span="12"> -->
                        <div class="edit-body">
                            ...
                            <EditItem name="备注事项" label="必填" description="请于xxxx年xx月xx日前，去往门店购买" v-if="ruleType == 'GOODS'">
                                <div slot="edit">
                                    <Input size="large" placeholder="" style="width:300px;" v-model="usageTips" clearable />
                                </div>
                            </EditItem>
                            ...
                            <EditItem name="奖项说明" label="必填">
                                <div slot="edit">
                                    <Select size="large" v-model="articleId" >
                                        <Option v-for="item in ArticleList" :value="item.id" :key="item.id">{{ item.title }}</Option>
                                    </Select>
                                </div>
                            </EditItem>
                        </div>
    ...
    methods: {
        ...
        check() {
            ...
            if (!StringHelper.trim(this.ruleName)) {
                ...
                warn = "请输入奖品等级";
            } else if (!StringHelper.trim(this.articleId)) {
                warn = "请选择开奖说明";
            };
            if (warn) {
                this.showError(warn);
            ...
            return MainApi.getRule({
                ...
                .then(res => {
                    if (res.code === "1") {
                        ...
                        this.articleId = data.articleId || (data.detailInfo && data.detailInfo.detailId) || "";
        ...
        submit() {
            ...
            param.append("articleId", this.articleId);
