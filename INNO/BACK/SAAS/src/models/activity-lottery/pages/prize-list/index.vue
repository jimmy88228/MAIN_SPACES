        ...
        <div class="cev-root" v-bar>
            <div class="bg-shadow padding20 list-box">
                <div class="cev-toolbar">
                    ...
                    <div class="right">
                        <Button type="primary" @click="exportClick" v-if="$store.state.authJson.btn_export_lottery_user">导出名单</Button>
                    </div>
                </div>
                <Table ref="table" class="table" :columns="columns" :data="list" border>
                    <template slot="action" slot-scope="p">
                        <div class="padding10"><Button class="item-table-action" type="error" @click="delPrizeUser(p.row.id)" size="small" >移除</Button></div>
                    </template>
                </Table>
                <Page
                    v-if="showPage"
