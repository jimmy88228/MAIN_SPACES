<template>
    <div class="bg-page cev-root spin-box flex-column">
        <div class="cev-root bg-shadow padding10 flex-auto tabs-box">
            <div class="tabs">
                <Tabs value="base" type="card" v-model="selectTab">
...
export default {
    ...
    methods: {
        ...
        check() {
            ...
            if (!actInfo.picture) {
                warn = "请上传活动主图";
            } else if (!actInfo.name) {
                warn = "请输入活动名称";
            } else if (!actInfo.startTime || !actInfo.endTime) {
                warn = "请选择活动时间";
                ...
                for(let i = 0; i < specList.length; i++){
                    ...
                    if(specItem.valueList.length == 0){
                        warn = "请填写完整的规格值";
                        break;
                    }
                    for(let j = 0; j < specItem.valueList.length; j++){
                        let valueItem  = specItem.valueList[j];
                        if(!valueItem.specValue){
                            warn = "请填写完整的规格值";
                            break;
                        }
                    }
            ...
            if(!warn && (!this.pictureList || this.pictureList.length === 0)) {
                warn = "至少增加一个轮播图";
