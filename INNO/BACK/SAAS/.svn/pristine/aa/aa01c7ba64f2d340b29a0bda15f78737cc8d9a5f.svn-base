        ...
        <div class="cev-root" v-bar>
            <div class="bg-shadow padding20 list-box">
                <div class="cev-toolbar">
                    ...
                    <div class="right">
                        <Poptip confirm title="活动开奖后小程序用户将看到相关中奖信息" @on-ok="activeActPrize" v-if="authJson.btn_announce_activity">
                            <Button type="primary" >活动开奖</Button>
                        </Poptip>
                        <Poptip confirm title="是否推送开奖结果" @on-ok="sendResult" v-if="authJson.btn_push_lottery_result">
                            <Button type="primary" >推送结果</Button>
                        </Poptip>
                        <Button type="primary" @click="getPrizeList" v-if="authJson.btn_check_lottery_user">开奖名单</Button>
                        <Button type="primary" @click="addRule()" v-if="authJson.btn_add_lottery_rule">
                ...
                <Table ref="table" class="table" :columns="columns" :data="list" border>
                    ...
                    <template slot="action" slot-scope="p">
                        <div class="padding10" v-if="authJson.btn_edit_lottery_rule"><Button class="item-table-action " type="primary" size="small" @click="editRule(p.row.ruleId)">编辑</Button></div>
                        <div v-if="authJson.btn_random_lottery"><Button class="item-table-action" type="primary" @click="getRandomList(p.row)" size="small" >随机生成</Button></div>
                        <div class="padding10" v-if="authJson.btn_check_enroll_user"><Button class="item-table-action " type="primary" size="small" @click="getScreenList(p.row.ruleId)">筛选用户</Button></div>
                    </template>
                </Table>
                <Page
                    v-if="showPage"
...
export default {
    ...
    methods: {
        ...
        activeActPrize() {
            this.loading = true;
            return MainApi.drawActivityAnnounce({
                data: {
                    activityId: this.actId
                }
            }).then(res => {
                if (res.code === "1") {
                    this.$Message.success(res.msg || "开奖成功");
