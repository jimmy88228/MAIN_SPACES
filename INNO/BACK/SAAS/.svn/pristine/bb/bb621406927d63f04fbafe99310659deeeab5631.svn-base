<template>
    ...
    >
        <div class="dialog-edit-group center">
            ...
            <div class="dialog-edit-item">
                <span class="name">绑定角色权限：</span>
                <Select
                v-model="roleIds"
                multiple
                >
                    <Option v-for="(option, index) in roleList" :value="option.roleId" :key="index">{{option.roleName}}</Option>
                </Select>
            </div>
            <div v-if="isAdd" class="dialog-edit-item">
...
export default {
    ...
    methods: {
        setData(row, infos) {
            ...
            let roleList = (this.infos && this.infos.roleList) || [];
            this.roleIds = [];
            for(let i = 0; i < roleList.length; i++){
                this.roleIds.push(roleList[i].roleId);
            }
            this.password = "";
            return this;
        },
