...
import { menuKeyAuth } from "@/config/menu-auth-key";
...
//     let authJson = LM.mAuthJson || {};
//     console.log("filterMenu authJson0000", authJson)
//     menus || (menus = []);
//     menus.map(e => {
//         let key = menuKeyAuth[e.name] || "";
//         e.isHide = (!senior && e.extra && e.extra.senior) || !authJson[key];
//     });
//     return menus;
// }
function filterMenus(menus, authJson) {
    authJson = authJson || {};
    menus || (menus = []);
    menus.map(e => {
        let key = menuKeyAuth[e.name] || "";
        e.isHide = !authJson[key];//(!senior && e.extra && e.extra.senior) || !authJson[key];
    });
    return menus;
}
const PageHelper = new Vue({
    data: {
        menus: filterMenus(Models.menus, LM.mAuthJson), //filterMenus(Models.menus, LM.isSenior),
        menuCurrent: {},
        breadcrumbList: [],
