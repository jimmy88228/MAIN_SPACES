                            ...
                            <div class="tabs-page animated" v-if="nowTabs===0" key="base">
                                <div class="edit-body cev-max-width">
                                    ...
                                    <EditItem name="发售店铺">
                                        <div slot="edit">
                                            <div>
                                                <span v-for="item in editData.storeList" :key="item">{{ item }}</span>
                                            </div>
                                            <Input slot="edit" size="large" v-model="inputStore" clearable/>
                                            <Button type="default" @click="addStore(inputStore)">添加店铺</Button>
                                        </div>
                                    </EditItem>
                                    <EditItem name="商品信息" >
                                        <div slot="edit">
                                            <Button type="default" @click="addSpecType">添加类型</Button>
                                        </div>
                                    </EditItem>
                                </div>
                            </div>
...
export default {
    ...
    methods: {
        ...
        addStore(inputStore) {
            ...
            if(inputStore){
                let storeStr = "," + this.editData.storeList.join(",") + ","
                if(storeStr.indexOf(","+ inputStore +",") != "-1"){
                    this.$Message.warning("已添加了该店铺");
                    return;
                }
                this.editData.storeList.push(inputStore)
            }
        },
        addSpecType() {
