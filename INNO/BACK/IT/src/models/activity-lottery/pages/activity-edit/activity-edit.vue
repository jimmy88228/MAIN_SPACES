...
export default {
    ...
    computed: {
        pictureList: {
            get() {
                let actInfo = JSON.parse(JSON.stringify(this.actInfo || {}));
                let list = []
                for ( let i = 0; i < actInfo.pictureList.length; i++) {
                    let picture = actInfo.pictureList[i].picture;
                    list.push(picture);
                }
                this.submitPics = actInfo.pictureList || [];
                return list;
            },
            set(val) {
    ...
    watch: {
       ...
       productList: {
            handler: function(val) {
                // console.log("productList", JSON.parse(JSON.stringify(val)));
    ...
    methods: {
        ...
        submit() {
            ...
            if (parseInt(this.actId)) {
                ...
                console.log("specProduct", specProduct);
                console.log("productJson", this.productJson);
...
function getProduct(specList = []) {
    ...
    // console.log("productJson", JSON.parse(JSON.stringify(this.productJson)));
    // console.log("productList", JSON.parse(JSON.stringify(this.productList)));
...
function loopData(specI, specList, addItem = {}, result = [], type) {
    ...
    let sEditType = specList[specI].editType;
    ...
    for (let i = 0; i < valueList.length; i++) {
        ...
        let vEditType = valueList[i].editType;
        ...
        if (nextValues && nextValues.length > 0) { // 递归拼装数据
            ...
            // console.log("keyStr", keyStr, "editType", vEditType);
            // console.log("keyStr值", this.productJson);
...
function getSubmitSpec(submitSpec, delAddSpec) {
    submitSpec = JSON.parse(JSON.stringify(submitSpec))
    let productList, specList = [];
    // console.log("delAddSpec submitSpec", submitSpec)
    if (delAddSpec) {
