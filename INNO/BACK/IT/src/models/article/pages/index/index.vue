        ...
        <div class="cev-root" v-bar>
            <div class="bg-shadow padding20 list-box">
                <div class="cev-toolbar">
                    ...
                    <div class="right">
                        <Button type="primary" :to="{ name: 'ArticleAdd' }">新建文章</Button>
                    </div>
                </div>
                <Table ref="table" class="table" :columns="columns" :data="list" @on-sort-change="handleSortChange" border>
                    <template slot="action" slot-scope="p">
                        <Button class="copy-url item-table-action" type="primary" size="small" @click="copyUrl($event,p.index)">复制地址</Button>
                        <Button class="item-table-action" type="primary" size="small" @click="edit(p.index)">编辑</Button>
                        <Poptip confirm title="确定删除该文章？" @on-ok="del(p.index)" transfer>
