...
<script>
    ...
    import userListForm from '../components/user-list-form';
    ...
    export default{
        ...
        methods: {
            ...
            onLoadData(page, reqData){
                ...
                return util.ajax.post(util.apiUrl.ManualLabelUserList, reqData).then(e =>{
                    let res = e.data || {};
                    if(res.code) {
                        let data = res.data || {};
            ...
            addUserReq(data){
                if(!this.tag_id || data.length == 0) return;
                this.showSpan = true;
                return util.ajax.post(util.apiUrl.ManualLabelUserAdd, {
                    tag_id: this.tag_id,
                    user_ids: data
                }).then(e =>{
                    let res = e.data || {};
            ...
            removeUser(){
                let seletUser = this.$refs["myTable"].getSelection() || [];
                let idsArr = [];
                if(!seletUser || seletUser.length == 0){
                    this.$Message.warning("没有选择用户！")
                } else {
                    for(let i = 0; i < seletUser.length; i++){
                        idsArr.push(seletUser[i].user_id);
                    }
                    ManualLabelUserRemove.call(this, idsArr);
                }
            },
            sendCouponReq(data){
                ManualLabelUserSend.call(this, data);
            },
            sendWechatReq(data){
                ManualLabelUserSend.call(this, data);
            },
            ...
            handleExport () {
                this.$Modal.confirm({
                    ...
                    cancelText: '取消',
                    onOk: () => {
                        return this.$ajax.post(this.$api.ManualLabelUserExport,{
                            tag_id: this.tag_id,
                            searchq: this.formData.searchq,
                            page: 1,
                            pageSize: 20
    ...
    //
    function ManualLabelUserSend(data){
       if(!this.tag_id) return;
        this.showSpan = true;
        return util.ajax.post(util.apiUrl.ManualLabelUserSend, {
            tag_id: this.tag_id,
            ...data
        }).then(e =>{
            let res = e.data || {};
    ...
    //删除用户
    function ManualLabelUserRemove(data){
       if(!this.tag_id) return;
        this.showSpan = true;
        return util.ajax.post(util.apiUrl.ManualLabelUserRemove, {
            tag_id: this.tag_id,
            user_ids: data
        }).then(e =>{
            let res = e.data || {};
