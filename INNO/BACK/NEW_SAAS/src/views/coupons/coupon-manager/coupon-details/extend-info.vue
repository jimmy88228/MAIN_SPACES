    ...
    <Form ref="formValidate" :model="formItem" :rules="ruleExtendValidate" :label-width="140" v-friendly-errors>
      ...
      <FormItem label="商品限制" prop="limitedType" v-if="formShowList.limitTypeList">
        <RadioGroup v-model="formItem.limitedType" vertical>
          <Radio :label="item.value" v-for="item in formShowList.limitTypeList" :key="item.value" class="limit_type" :disabled="isEditStaus">
            <span>{{item.name}}</span>
          </Radio>
        </RadioGroup>
        ...
        <Input
          ...
          :rows="3"
           :disabled="formItem.limitedType != 4 || isEditStaus"/>
        </div>
      </FormItem>
      <!-- 编辑状态并且是扫码支付优惠券的直接隐藏 -->
      <FormItem label="排除商品" prop="excludeGoods" v-if="!(isEditStaus && formItem.sendType == 3)">
        <Input
          type="textarea"
          class="basic_textarea basic_textarea_fixed"
  ...
  watch: {
    data(nV) {
      if (nV && Object.keys(nV).length > 0) {
        const { discount_type, payment, vcat, category, goods_brand } = nV;
    ...
    editData(nV) {
      if (nV && Object.keys(nV).length > 0) {
        ...
        const catRs = binding_cat.map(Number);
        const vcatRs = binding_vcat.map(Number);
