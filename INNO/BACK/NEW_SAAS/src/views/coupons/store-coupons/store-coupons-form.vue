  ...
  <div class="store-coupons-form">
    <PageTopBase isSave @save="confirm">
      <Form ref="formValidate" :model="formItem" :rules="ruleValidate" :label-width="160" label-colon v-friendly-errors>
        ...
        <FormItem label="指定商品国标码" prop="limitGoodsSns">
          <Input v-model="formItem.limitGoodsSns" type="textarea" :rows="3" placeholder="请输入指定商品国标码" class="basic_input"/>
          <p class="strong_tips">多商品国标码用逗号分隔，不能有空格，不填写则表示不做限制；</p>
        </FormItem>
        <FormItem label="排除商品国标码" prop="notLimitGoodsSns">
          <Input v-model="formItem.notLimitGoodsSns" type="textarea" :rows="3" placeholder="请输入指定排除商品国标码" class="basic_input"/>
          <p class="strong_tips">多商品国标码用逗号(英文)分隔</p>
        </FormItem>
