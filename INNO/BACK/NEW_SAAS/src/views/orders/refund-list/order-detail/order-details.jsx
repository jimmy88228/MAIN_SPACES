...
export default {
  ...
  methods: {
    ...
    confirmPop(id, remark) {
      ...
      this.spinShow = true;
      return this.$ajax.post(this.$api.orderRefundRemark, {
        id,
        remark
      }).then(response => {
            ...
            scopedSlots: {
              ...
              handle: ({ row }) => {
                return <div class="handle_content">
                  {this.rowData.handle.edit ? <router-link tag="a" to={{ name: 'refund-info', params: { sn: this.rowData.id } }} target="_blank" class="edit_item">查看</router-link> : <span></span>}
                  {row.handle.remark ?
