    ...
    <transition-group name="fade">
      <div class="presale-activity-form" v-show="showBaisc" key="basic">
        <Form ref="formValidate" :model="formItem" :rules="ruleValidate" :label-width="140" v-friendly-errors>
          ...
          <FormItem label="运费" prop="shippingFree">
            <InputNumber :min="0" v-model="formItem.shippingFree"></InputNumber>
            <label>元</label>
          </FormItem>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      ruleValidate: {
        ...
        shippingFree: [{required: true, message: '运费不能为空', trigger: 'blur', type: 'number'}]
      },
  ...
  methods: {
    loadData (page, data) {
      this.spinShow = true;
      return this.$ajax.post(this.$api.presaleActivityInfo, {
        id: this.id
      })
      .then(response => {
        ...
        if (res.code) {
          ...
          this.formItem = {
            ...
            shippingFree: Number(data.shipping_free),
    ...
    confirm () {
      ...
      this.$refs.formValidate.validate(valid => {
        if (valid && this.isValidTable) {
          this.spinShow = true;
          return this.$ajax.post(this.id ? this.$api.presaleActivityEdit : this.$api.presaleActivityAdd, {
            activity_id: this.id,
            ...
            shipping_free: this.formItem.shippingFree,
