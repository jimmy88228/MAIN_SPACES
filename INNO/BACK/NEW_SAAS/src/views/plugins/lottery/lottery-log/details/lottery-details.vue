      ...
      <Table :loading="tableLoading" :height="tableHeight" :columns="tableColumns" :data="tableData" ref="myTable">
        <template slot-scope="{ row }" slot="prize_img">
          <div class="img_list_wrap">
            <div class="img_fixed">
              <img :src="row.prize_img" v-if="row.prize_img" :alt="row.prizes_name" v-viewer/>
              <img src="@rs/images/default-img.jpg" :alt="row.prizes_name" v-viewer v-else></img>
            </div>
          </div>
        </template>
...
export default {
  ...
  methods: {
    onLoadData (page, data) {
      ...
      return this.$ajax.post(this.$api.lotteryLogViewDetails, params)
      .then(response => {
    ...
    handleExport () {
			this.$Modal.confirm({
				...
				cancelText: '取消',
				onOk: () => {
					return this.$ajax.post(this.$api.lotteryLogViewDetailsExport, {
            ...this.condition,
            activity_id: this.id
          }).then((response) => {
