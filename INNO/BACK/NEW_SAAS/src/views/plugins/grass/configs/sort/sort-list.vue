<template>
  <div class="activity-list">
    <Card>
      <Row>
        ...
        <Col span="6" class="btn-group">
          <Button type="primary" icon="md-add" @click="createActivity" v-if="canCreate.add">添加分类</Button>
        </Col>
      </Row>
    ...
    <SortForm ref="sortForm" @on-success="loadData()"></SortForm>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    onLoadData (page, data) {
      ...
      return this.$ajax.post(this.$api.communitySettingCatList, params)
      .then(response => {
    ...
    onDelItem (row) {
      return this.$ajax.post(this.$api.communitySettingCatRemove, {
        id: row.id
      });
    },
    createActivity() {
