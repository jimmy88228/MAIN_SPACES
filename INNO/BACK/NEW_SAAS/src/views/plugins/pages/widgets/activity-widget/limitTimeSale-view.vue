...
<template>
	<div class="limit-time-sale-view">
		<!--统一活动头-->
		...
		<div v-if="typeof(goodsList) != 'undefined' && goodsList.length > 0">
			...
			<template v-if="info.layout == 'one' || info.layout == 'two' || info.layout == 'three' ">
				<Row type="flex" style="flex-wrap:wrap;" :gutter="info.gutter">
					<Col v-for="(item,index) in goodsList" :key="index" v-if="info.open_slide == null || info.open_slide == false || ( info.open_slide == true && index +1 <= (info.layout == 'one' ? 1 : (info.layout == 'two' ? 2 : 3)) ) "
			...
			<template v-else-if="info.layout == 'bigSmall'">
				<Row type="flex" style="flex-wrap:wrap;" :gutter="info.gutter">
					<Col v-for="(item,index) in goodsList" :key="index" :span="index%3 == 0 ? 24 : 12 ">
			...
			<template v-else-if="info.layout == 'list' ">
				<div v-for="(item,index) in goodsList" :key="index" class="goods-item" style="margin-bottom:3px;padding:5px;">
					<Tag :color="styleColor" class="tag">限时</Tag>
