		...
		<Form ref="formValidate" :model="formItem" label-position="top">
			<FormItem label="logo图片( 为了美观，请尽量使用统一大小规格的图片 )">
					...
					<div class="item-list">
						...
						<Row :gutter="16">
							<Col span="6">
								<div class="image-box" :style="'background-image: url('+formItem.winningLogo.img+');'">
									<Icon type="ios-close-circle" class="close" title="移除" @click="removeImg('winningLogo')" />
									<a class="image-box-text" @click="selectSingleImage('winningLogo')">{{formItem.winningLogo.img ? '更换图片' : '选择图片'}}</a>
								</div>
							</Col>
							<Col span="18">
								<div class="flex f-align-center" style="margin-top:35px;">
									<div class="f-shrink0" style="margin-right:15px;">图片占比</div> 
									<Slider v-model="formItem.winningLogo.mix" 
									:min="0" :max="100" 
									show-input 
	...
	export default {
		...
		methods: {
			...
			init() {
				...
				if(!this.formItem.winningLogo) {
					this.$set(this.formItem, 'winningLogo', {img: "", mix: 0})
				}
			},
			// 移除图片
			removeImg(type) {
				this.$Modal.confirm({
					...
					cancelText: '取消',
					onOk: () => {
						this.$delete(this.formItem[type], 'img', '');
					},
				});
			},
			// 选择图片（单选）
