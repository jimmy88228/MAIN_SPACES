						...
						<div v-for="(item,index) in dataList" :key="t+'|'+index" :class="'com-item '+ ( item.selected === true ? 'selected' : '' )"
						 :style="getCompStyle(item.setting)" @click="itemChange(index, true)">
							...
							<div class="com-tips">
								...
								<span class="txt space-nowrap inline-b">
									<template v-if="item.code == null || typeof( componentsList[item.code] ) == 'undefined'">未知组件</template>
									<template v-else>{{componentsList[item.code]}}</template>
								</span>
							...
							<!--用户头部（个人中心用）-->
							<userCenterHeaderView v-else-if=" item.code == 'userCenterHeader' " :currIndex="index"></userCenterHeaderView>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 拖动事件
			dragChange(e) {
				console.log("dragChange", JSON.parse(JSON.stringify(e)))
