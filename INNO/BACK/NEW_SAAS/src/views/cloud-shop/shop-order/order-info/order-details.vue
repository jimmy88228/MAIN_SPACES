    ...
    <Form ref="form" :label-width="72" label-colon>
      <Row>
        ...
        <Col span="6" class="order_details_item">
          ...
          <FormItem label="物流单号">
            <template v-if="addressInfo.logistics_sn && addressInfo.logistics_sn.length > 0">
              <p v-for="(item, index) in addressInfo.logistics_sn" :key="index">
         ...
         <!-- <FormItem label="配送时间"><p>{{addressInfo.delivery_time}}</p></FormItem> -->
          <FormItem label="发货单号">
            <p  style="font-size:12px;" v-for="(item, index) in addressInfo.deliver_sn" :key="index">
              <router-link
                tag="a"
                :to="{name: 'shop-delivery-order-info', params: { sn:index}}"
                target="_blank"
                v-if="item">
                {{item}}
              </router-link>
        ...
        <Col span="6" class="order_details_item">
          ...
          <FormItem label="实付金额"><p>{{orderMoney.money_paid}}</p></FormItem>
          <FormItem label="支付方式">
            <p>{{ordreMessage.pay_name}}</p>
          </FormItem>
  ...
  methods: {
    ...
    confirmEdit() {
      this.$refs.formItem.validate(valid => {
        if (valid) {
          this.spinShow = true;
          return this.$ajax.post(this.$api.ShopOrderAdminRemark, {
            order_id: this.sn,
            remark: '',
