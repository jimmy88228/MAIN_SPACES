    ...
    <Form ref="form" :label-width="72" label-colon>
      <Row>
        ...
        <Col span="8" class="order_details_item">
          ...
          <FormItem label="发货门店"><p>{{ordreMessage.delivery_store_name || '--'}}</p></FormItem>
          <FormItem label="快递公司">
            <p>
        ...
        <Col span="8" class="order_details_item">
          ...
          <FormItem label="会员卡号"><p>{{ordreMessage.card_num ? ordreMessage.card_num : ''}}</p></FormItem>
...
export default {
  ...
  methods: {
    ...
    loadshippingList () {
      this.spinShow = true;
      return this.$ajax.post(this.$api.ShopShippingSimpleList, {
        delivery_id: this.sn
      }).then(response => {
    ...
    confirmEdit() {
      ...
      this.spinShow = true;
      return this.$ajax.post(this.$api.ShopChangeDelivery, {
        delivery_id: this.sn,
