          ...
          <Col span="8" class="order_intro">
            <div class="order_status">
              <Form ref="form" :label-width="72" label-colon>
                ...
                <FormItem label="订单状态"><p>{{currentOrderType==2 ?'已取消':currentOrderType==1?'已发货':'待发货'}}</p></FormItem>
              </Form>
            </div>
          </Col>
          <Col span="16">
            <div class="order_steps">
              <Steps :current="currentOrderType">
                ...
                <Step title="已取消" :content="threeStep"></Step>
              </Steps>
            </div>
          </Col>
        </Row>
...
export default {
  ...
  methods: {
    loadData() {
      this.spinShow = true;
      return this.$ajax.post(this.$api.ShopShipmentInfo, {
        delivery_id: this.sn
      }).then(response => {
