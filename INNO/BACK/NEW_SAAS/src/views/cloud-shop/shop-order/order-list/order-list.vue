	...
	<Card class="order-list">
		<Row type="flex" style="margin-bottom: 10px;">
			...
			<searchForm ref="search-form" :order-status-list="orderStatusList" :store-list="storeList" :order-from="orderFrom"
			 ...
			 :check-order="checkOrder" :search-type-list="searchTypeList" 
			 :order-status-select="order_status_select" 
			 @on-search="searchPage"></searchForm>
			</Col>
...
<script>
	...
	const DEFAULT_TAB = [0, 1, 2, 8, 3, 4, 5, 6,11,12];
	export default {
		props: ['userId'],
		components: {
			orderListTable,
		...
		methods: {
			...
			onTabsClick(status) {
				...
				this.$refs['list-' + status].initData(this.order_status, {
					...this.searchForm,
					user_id: this.userId || 0
				});
			...
			loadExtraData() {
				...
				return this.$ajax.all(
					[
						this.$ajax.post(this.$api.ShopSearchList),
						// this.$ajax.post(this.$api.storeTree)
					]
				).then(
					//storeList
					this.$ajax.spread((searchList) => {
			...
			searchPage(searchForm) {
				...
				this.order_status = DEFAULT_TAB.indexOf(Number(searchForm.orderStatus)) === -1 ? '0' : String( searchForm.orderStatus );
			...
			confirmRst() {
				...
				return this.$ajax.post(this.$api.ShopResetOrderCommission, {
					// 过滤得出合法的字符串
			...
			handleExport(type) {
				...
				// ajax 请求获取数据
				this.$ajax.post(this.$api.ShopOrderExport, this.searchForm)
					.then((response) => {
