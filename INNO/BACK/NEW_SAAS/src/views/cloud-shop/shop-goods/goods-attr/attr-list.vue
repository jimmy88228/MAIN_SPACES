...
export default {
  ...
  methods: {
    ...
    onDelItem (row) {
      return this.$ajax.post(this.$api.ShopGoodsAttrGroupRemove, {
        cat_id: row.cat_id
      });
    },
    // 开启/关闭
    updateEnabled (index, row) {
      ...
      return this.$ajax.post(this.$api.ShopGoodsAttrGroupIsenable, {
        cat_id: row.cat_id,
    ...
    exportData (row) {
      this.tableLoading = true;
      return this.$ajax.post(this.$api.ShopGoodsTypeExport, {
        cat_id: row.cat_id
      })
      .then((response) => {
        var res = response.data;
