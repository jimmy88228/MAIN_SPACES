	...
	<div class="goods-editor-basic">
		<Form ref="formValidate" :model="formItem" :rules="ruleValidate" :label-width="120">
			...
			<FormItem label="商品名称" prop="goods_name">
				<Input v-model="formItem.goods_name" placeholder="请输入商品名称" class="basic_input" :disabled="disabled"/>
			</FormItem>
			<FormItem label="商品货号" prop="goods_sn">
				<Input v-model="formItem.goods_sn" placeholder="请输入商品货号" class="basic_input" :disabled="disabled"/>
			</FormItem>
      <FormItem label="商品类别" prop="sale_type">
				<!-- 新建页面 -->
				<template v-if="isAdd">
					<Select v-model="formItem.sale_type" class="basic_select" :disabled="disabled">
        ...
        <img :src="formItem.goods_img" v-viewer style="width: 80px; height: 80px; cursor: pointer; border-radius: 4px;"/>
      ...
      <FormItem label="主图视频" prop="goods_videos" class="video_form">
        ...
        <div class="strong_tips">视频大小不能超过10M,建议使用mp4格式</div>
			...
			<FormItem label="商品品牌" prop="goods_brand_id">
				<Select v-model="formItem.goods_brand_id" class="basic_select" filterable clearable :disabled="disabled">
			...
			<FormItem label="商品服务" prop="goods_service">
				<Select v-model="formItem.goods_service" class="basic_select" multiple filterable :disabled="disabled">
					<Option v-for="item in serviceList" :value="item.id" :key="item.id">{{ item.name }}</Option>
				</Select>
			</FormItem>
			<FormItem label="商品卖点" prop="goods_brief">
				<Input v-model="formItem.goods_brief" placeholder="请输入商品卖点" class="basic_input" maxlength="13" show-word-limit :disabled="disabled"/>
			</FormItem>
			<FormItem label="关键字" prop="keywords">
				<Input v-model="formItem.keywords" placeholder="请输入关键字" class="basic_textarea" type="textarea" :disabled="disabled"/>
        ...
        <div v-for="(item, index) in selectedVcat" :key="item.value" class="vact_item">
          ...
          <Icon type="ios-close-circle-outline" size="20" class="del_vcat" title="删除" @click="delVcat(item.value)"/>
				...
				<Button type="primary" icon="md-add" :disabled="disabled" @click="addVcat">添加</Button>
			</FormItem>
			<!-- <FormItem label="上架平台" prop="goods_platform">
          ...
          <Radio label="0" :disabled="disabled">
              ...
              <span>暂不上架，放入仓库</span>
				...
				<!-- <span class="auto_sale">
					<Checkbox v-model="formItem.goods_auto_on_sale_enable" :disabled="disabled">自动上架</Checkbox>
					<DatePicker
        ...
        <RadioGroup v-model="formItem.self_get">
					<Radio label="0" :disabled="disabled">无限制</Radio>
					<Radio label="1" :disabled="disabled">强制自提</Radio>
          <Radio label="2" :disabled="disabled">强制配送</Radio>
				</RadioGroup>
			</FormItem>
  ...
  methods: {
    ...
    saveGoods () {
      ...
      this.$refs.formValidate.validate((valid) => {
        ...
        if (valid) {
          this.spinShow = true;
          this.$ajax.post((this.formItem.goods_id == 0 ? this.$api.goodsAdd : this.$api.goodsEdit), {
            ...
            self_get: this.formItem.self_get,
