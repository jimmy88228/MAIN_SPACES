...
export default {
	...
	methods: {
		...
		remove( root, node, data ){
			...
			this.$Modal.confirm({
		        title: '删除提示',
		        content: '确定删除素材分类吗？无素材的分类才能删除成功。',
		        okText: '确定删除',
		        cancelText: '取消',
		        onOk: () => {

					// ajax 提交数据
		        	util.ajax.post( util.apiUrl.csMaterialCatRemove, {
		        		id: data.id,
		        	})
		    		.then( (response) => {
		...
		onSaveCat( ){
			...
			this.$refs['formValidate'].validate((valid) => {
		        if (valid) {
		        	...
		        	// ajax 提交数据
		        	util.ajax.post( ( this.formItem.id == 0 ? util.apiUrl.csMaterialCatAdd : util.apiUrl.csMaterialCatUpdate ), {
						id: this.formItem.id,
						parent_id: parentId,
