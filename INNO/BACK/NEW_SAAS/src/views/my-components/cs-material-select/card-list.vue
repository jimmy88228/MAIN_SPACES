			...
			<Col style="flex: 1 1 0%;">
	        	...
	        	<div class="scroll-content" :style="getContentHeight">
	        		...
	        		<Row class="list-box">
						...
						<!--数据库中的图片列表（可以操作）-->
		        		<Col v-for="(item,index) in list" :key="('e'+index)" :id="('cs-card'+timestamp+index)" span="6" 
		        			:class="itemClass(item)" @click.native="selectedImg('cs-card'+timestamp+index, item)">
				        	
							<div class="img-remark-box" title="请点击选择图片">
								<Row v-show="item.edit==false">
							    	<Col class="img-remark" span="20">{{item.content}}</Col>
									<Col span="4" v-if="can.canEdit">
										<Icon class="icon remark" type="md-create" title="修改备注信息" @click.stop="editRemark(index,item.id,'list')" />
									</Col>
								</Row>
								<Row v-show="item.edit" :gutter="5">
									<Col span="16"><Input size="small" v-model="item.content" placeholder="备注"/></Col>
									<Col span="8">
								...
								<Row v-show="item.edit==false">
									<Col span="4">路由:</Col>
									<Col class="img-remark" span="20">{{item.link_url}}</Col>
								</Row>
								<Row v-show="item.edit" :gutter="5">
									<Col span="23"><Input size="small" v-model="item.link_url" placeholder="小程序路由地址"/></Col>
								</Row>
							</div>
    ...
    methods: {
        ...
        saveRemark(index, id, type){
        	...
        	if( type == 'list' ){
				...
				link_url = this.list[index].link_url;
        		img_id = this.list[index].id;
        	}
        	else if( type == 'uploadList' ){
        		img_remark = this.uploadList[index].content;
				link_url = this.uploadList[index].link_url;
