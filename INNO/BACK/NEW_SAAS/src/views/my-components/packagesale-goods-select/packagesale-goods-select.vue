...
<template>
	<div>
		<Modal v-model="showModal" :loading="modalLoading" :title="modalTitle" :width="880" :styles="{top:'20px'}" class="packagesale-goods-select"
		 @on-ok="onOk">
						...
						<div class="select-box">
							...
							<div>
								<div class="goods-img-box" title="点击查看大图">
									<img :src="( typeof(item.package_bg_image) != 'undefined' ? item.package_bg_image : '')" :alt="item.package_name"
	...
	export default {
		...
		methods: {
			...
			initData() {
				...
				// ajax 请求获取初始化数据
				util.ajax.post(util.apiUrl.goodsPackageList, {
						isInit: 2,
			...
			initTable(res) {
				...
				this.columns[(this.columns.length - 1)].render = (h, params) => {
					...
					const color = row.is_enabled == 1 ? 'green' : 'red';
					const text = row.is_enabled_str;
			...
			handlePageSize(pageSize) {
				...
				// ajax 请求获取数据，
				util.ajax.post(util.apiUrl.presaleActivityList, {
						isInit: 2,
						search: this.formSearch.search,
						page: this.currentpage,
						pageSize: this.pageSize
