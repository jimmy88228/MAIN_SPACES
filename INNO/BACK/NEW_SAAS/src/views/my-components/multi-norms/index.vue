<template>
  <Modal
    ...
    <div class="search">
      <SearchForm ref="search" :data="data" @on-search="searchPage"></SearchForm>
    </div>
    <div class="content">
      <Table
        ...
        <template slot-scope="{ row, index }" slot="norms">
          <Poptip placement="right" width="400" transfer v-model="row.isVisible">
            <p><a @click.stop="showPop(index)">已选规格({{row._selectLen}})</a></p>
            <div slot="content">
              <row-norms
                :row-data="row.get_products"
...
export default {
  ...
  computed: {
    ...
    checkTableAll: {
      get () {
        let {checked, indeterminate} = this.handleCheckStatus(this.allLen, this.selectedLen);
        ...
        if (val) {
          ...
          res.forEach(item => {
            if (hasIds.includes(Object.keys(item)[0])) {
              let index = this.selectData.findIndex(selectItem => Object.keys(selectItem)[0] === Object.keys(item)[0]);
          ...
          this.freezeData.forEach(item => {
            let listIndex = this.selectList.map(item => item.goods_id).findIndex(t => Number(t) === Number(item.goods_id));
            if (listIndex > -1) {
  ...
  methods: {
    onLoadData (page, data) {
      ...
      return this.$ajax.post(this.$api.goodsList, params)
      .then(response => {
              ...
              if (selectedLen === originLen) {
                ...
                item.get_products.forEach(originNorm => {
                  if(!(originNorm.color_id in originTemp)) {
                    ...
                    originTemp[originNorm.color_id].push(originNorm.size_id);
                  } else {
                    originTemp[originNorm.color_id].push(originNorm.size_id);
                  }
                });
                ...
                selectedCollection.forEach(selectedNorm => {
                  if(!(selectedNorm.color_id in newTemp)) {
                    ...
                    newTemp[selectedNorm.color_id].push(selectedNorm.size_id);
                  } else {
                    newTemp[selectedNorm.color_id].push(selectedNorm.size_id);
                  }
                });
                ...
                item.get_products.forEach(normItem => {
                  ...
                  if (checkLostArr.includes(normItem.color_id)) {
                    ...
                    normItem._sizeChecked = newTemp[normItem.color_id].includes(normItem.size_id);
                  }
