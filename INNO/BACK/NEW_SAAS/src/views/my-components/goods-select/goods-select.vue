...
<template>
	<div>
		<Modal v-model="showModal" :loading="modalLoading" :title="modalTitle" :width="880" :styles="{top:'20px'}" class="goods-select"
		 @on-ok="onOk">
...
<script>
	...
	export default {
		...
		computed: {
			// 获取内容框高度
			getContentHeight() {
				let tHeight = this.tableHeight = document.body.clientHeight - 320;
		...
		methods: {
			...
			changePage(page) {
				...
				this.$ajax.post((this.$route.fullPath.indexOf('cloud-shop') != -1 ? this.$api.ShopGoodsList : this.$api.goodsList), {
						...
						vcat_id: this.formSearch.vcat_id,
						is_delete: this.formSearch.isDelete,
						is_on_sale: this.formSearch.isOnSale,
						sale_type: this.formSearch.saleType,
						sale_kind: this.formSearch.saleKind,
						is_on_image: this.formSearch.isOnImage,
						platform_src: this.formSearch.platformSrc.join(','),
						only_store_goods: this.formSearch.onlyStoreGoods == true ? 1 : 0,
						page: this.currentpage,
						pageSize: this.pageSize
					})
					.then((response) => {
