...
<template>
  <Card class="visit-page span-parent statistics-page">
	...
	<div class="span-parent">
		...
		<Table :columns="dataCol" :data="sourceData" :max-height="tableHeight"></Table>
		<Divider orientation="left">商品转化数据</Divider>
		<Table :columns="goodsCol" :data="goodsList" :max-height="tableHeight">
...
export default {
	...
	methods: {
		...
		onLoadData(page, data){
			this.showSpin = true;
			return util.ajax.post(util.apiUrl.basicChannelDetails, {
				...
				if(res.code) {
					...
					this.goodsList = goods_data.list || [];
					this.data = {
						items: this.goodsList,
						total: goods_data.totalCount || 0
					}
		...
		getFilterX(){
			let XData = [];
			let startDate = Date.parse(this.start_time);
			let endDate = Date.parse(this.end_time);
			let _date = startDate;
