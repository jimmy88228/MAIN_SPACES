	...
	<div class="goods-editor-basic">
      ...
      <FormItem label="商品类别" prop="sale_type">
			...
			<FormItem label="售卖类型" prop="sale_kind">
				<Select v-model="formItem.sale_kind" class="basic_select" :disabled="formItem.goods_id != 0">
					<Option v-for="(item, index) in saleKindList" :value="index" :key="index">{{item}}</Option>
				</Select>
			</FormItem>
      ...
      <FormItem label="限购" prop="is_limit_buy" v-show="isProduct">
        ...
        <p class="strong_tips">限制单个会员购买数量</p>
			...
			<FormItem label="分销提成比例" prop="staff_dstb_goods_commate" class="staff_goods_commate">
  ...
  methods: {
    ...
    initData (res) {
      ...
      this.isAdd = basicData.length === 0;
        ...
        // 商品分类
        cat_id_list: basicData.cat_id_list.map(Number) || [],
        ...
        recount_inventory: basicData.recount_inventory || '0'
      };
      // 非赠品
