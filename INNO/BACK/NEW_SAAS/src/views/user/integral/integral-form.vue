	...
	<div>
		<Modal
	        ...
	        <Form ref="formValidate" :model="formItem" :rules="ruleValidate" :label-width="150">
				...
				<FormItem v-if="formItem.key=='integral_expire'" :label="formItem.name" prop="val">
					<!--积分有效期-->
					<RadioGroup v-model="formItem.val" @on-change="radioChange" vertical>
				        ...
				        <div v-show="nyearsShow">
							...
							<div>示例：若设置每笔积分有效期为1年，则用户今年1月1日获得的积分，将在明年1月1日过期</div>
				        </div>
				    </RadioGroup>
				</FormItem>

		        <FormItem v-else :label="formItem.name" prop="val">
		            <Input v-model="formItem.val" placeholder="请输入可获得的积分" style="width:200px"></Input>
		        </FormItem>
...
export default {
  ...
  methods: {
    ...
    modalOk () {
        ...
        if (valid) {
                	// ajax 保存编辑数据
                	util.ajax.post(util.apiUrl.integralEdit, {
