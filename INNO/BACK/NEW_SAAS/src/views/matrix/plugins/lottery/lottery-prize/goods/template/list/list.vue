	...
	<div>
		...
		<Table :loading="tableLoading" :height="tableHeight" :columns="tableColumns" :data="tableData" ref="myTable" @on-selection-change="changeSelect">
		  <template slot-scope="{ row }" slot="handle">
				<div class="v-lines">
					<template v-if="row.handle.edit"><a @click="addGoods(row)">编辑</a>&nbsp;<span class="v-line">|&nbsp;</span></template>
					<template v-if="row.handle.remove">
						<Poptip
						:transfer="true"
						confirm
						title="确定删除该实物么?"
						@on-ok="removeGoods(row)">
						<a >删除</a>
		...
		<editGoods ref="editGoods" @search="search"></editGoods>
	</div>
</template>
<script>
