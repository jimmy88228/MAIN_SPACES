    ...
    <Form ref="form" :label-width="72" label-colon>
      <Row>
        ...
        <Col span="8" class="order_details_item">
          ...
          <FormItem label="配送方式"><p>{{addressInfo.delivery_way}}</p></FormItem>
          <FormItem label="物流单号">
          ...
          <FormItem label="发货单号">
            <p>
							<template v-if="addressInfo.deliver_sn">
									<router-link
										tag="a"
										:to="{name: 'matrix-delivery-order-info', params: { sn: index }}"
										target="_blank"
										v-for="(item, index) in addressInfo.deliver_sn"
										:key="index"
										>
										{{item}}
									</router-link>
          ...
          <!-- <FormItem label="退货单号">
						<template v-if="ordreMessage.return_data && ordreMessage.return_data.length > 0">
            ...
            <span v-else-if="!ordreMessage.return_data">--</span>
          </FormItem> -->
        </Col>
  ...
  methods: {
    ...
    confirmEdit() {
      this.$refs.formItem.validate(valid => {
        if (valid) {
          this.spinShow = true;
          return this.$ajax.post(this.$api.MatrixOrderAddressChange, {
            order_id: this.sn,
            remark: '',
						goodsAddress: {
		...
		getAddressInfo(){
			...
			let district = formItem.district	
			let info = district;
			for(let i = 0; i < this.provinceList.length; i++){
				if(district.province == this.provinceList[i].value){
					info.province = this.provinceList[i].value;
					info.province_name = this.provinceList[i].label;
					break;
				}
			}
			for(let i = 0; i < this.cityList.length; i++){
				if(district.city == this.cityList[i].value){
					info.city = this.cityList[i].value;
					info.city_name = this.cityList[i].label;
					break;
				}
			}
			for(let i = 0; i < this.districtList.length; i++){
				if(district.street == this.districtList[i].value){
					info.district = this.districtList[i].value;
					info.district_name = this.districtList[i].label;
					break;
				}
			}
			return info
