...
export default {
  ...
  methods: {
    ...
    // ajax 请求获取初始化数据，然后动态更新下面数据源
      util.ajax.post(util.apiUrl.ShopGoodsSizeList, {
        isInit: 1,
        page: this.curPage,
    ...
    removeSpec (index, row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
              this.tableLoading = true;
            // ajax 请求获取数据，然后动态更新下面数据源
            util.ajax.post(util.apiUrl.ShopGoodsSizeRemove, {
    ...
    handleImport () {
      this.$refs.batchImport.openModal(this.canCreate, this.$api.ShopGoodsSizeUpload, this.$api.ShopGoodsSizeDownload);
    },
    onImportSuccess () {
      this.init();
    }
