			...
			<div class="goods-cat-inner">
				<Row type="flex">
					...
					<Col style="width:320px;text-align: right;">
						<Button v-if="canCreate" type="info" icon="md-add" @click="createCat">添加自定义分类</Button>
...
<script>
	...
	export default {
		...
		methods: {
			onLoadData(page) {
				this.showSpin = true;
				return this.$ajax.post(this.$api.ShopGoodsVcatList, {
						page,
						isInit: 1
					})
					.then(response => {
						...
						if (res.code) {
							...
							this.columns = [{
									...
									key: "vcat_name",
									align: "left",
									span: document.body.offsetWidth <= 1600 ? 10 : 12
								},
								{
									title: "商品数",
									key: "get_goods_count",
			...
			searchPage() {
				this.showSpin = true;
				return this.$ajax.post(this.$api.ShopGoodsVcatList, this.formSearch)
					.then((response) => {
						this.showSpin = false;
			...
			onDelItem(row) {
				this.showSpin = true;
				return this.$ajax.post(this.$api.ShopGoodsVcatRemove, {
						vcat_id: row.id
					})
					.then((response) => {
						this.showSpin = false;
