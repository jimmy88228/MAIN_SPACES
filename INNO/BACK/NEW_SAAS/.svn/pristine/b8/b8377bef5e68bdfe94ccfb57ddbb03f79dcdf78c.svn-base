	...
	<div class="order-info">
		<Card>
			...
			<action-details :action-list="actionList" :order-button="orderButton" :refund-message="refundMessage" :sn="sn"
			 :deliver-sn="orderAddress.deliver_sn" :order-sn="ordreMessage.order_sn" :todo-refund="todoRefund" :handle="handle" @reload="loadData"></action-details>
			<!--加载提示-->
...
<script>
	...
	export default {
		...
		methods: {
			loadData() {
				this.spinShow = true;
				return this.$ajax.post(this.$api.ShopOrderInfo, {
					order_id: this.sn,
					orderTypeValue: ''
				}).then(response => {
