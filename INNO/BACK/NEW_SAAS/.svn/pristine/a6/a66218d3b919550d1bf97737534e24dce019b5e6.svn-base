...
export default {
  ...
  methods: {
    onLoadData (page, data) {
      return this.$ajax.post(this.$api.seckillGroupList, data)
      .then(response => {
    ...
    onDelItem (row) {
      return this.$ajax.post(this.$api.seckillGroupRemove, {
        id: row.id
      });
    },
    handleToggle (row) {
      this.spinShow = true;
      return this.$ajax.post(this.$api.seckillGroupStatus, {
        id: row.id,
