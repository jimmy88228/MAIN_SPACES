		...
		<Card v-show="showMessageList">
			...
			<div class="table-topbar">
				<Row type="flex">
					...
					<Col stype="width:160px;text-align:center">
						<Button v-if="canCreate" type="info" icon="md-add" @click="openModal">添加消息</Button>
	...
	export default {
		...
		methods: {
			...
			initData() {
				...
				util.ajax.post(util.apiUrl.systemMessageList, {
						isInit: 1
					})
					.then((response) => {
			...
			searchPage(searchForm) {
				this.tableLoading = true;
				// ajax 请求获取数据，然后动态更新下面数据源
				util.ajax.post(util.apiUrl.systemMessageList, searchForm)
					.then((response) => {
						this.tableLoading = false;
						var res = response.data;
						if (res.code) {
			...
			sendMessage(index, row) {
				this.$Modal.confirm({
					...
					onOk: () => {
						...
						// ajax 请求获取数据，然后动态更新下面数据源
						util.ajax.post(util.apiUrl.systemMessageSend, {
								messageId: row.id
							})
							.then((response) => {
								var res = response.data;
			...
			removeMessage(index, row) {
				this.$Modal.confirm({
					...
					onOk: () => {
						this.tableLoading = true;
						// ajax 请求获取数据，然后动态更新下面数据源
						util.ajax.post(util.apiUrl.systemMessageRemove, {
								messageId: row.id
							})
							.then((response) => {
								var res = response.data;
