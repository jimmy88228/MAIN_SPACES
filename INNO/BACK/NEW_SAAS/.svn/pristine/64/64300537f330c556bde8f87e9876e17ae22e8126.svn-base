...
<template>
	<div>
		<Modal v-model="showModal" :loading="modalLoading" :title="modalTitle" :width="880" :styles="{top:'20px'}" class="goods-vcat-select"
		 @on-ok="onOk">
			...
			<Row :gutter="10">
				...
				<!--选中的区域-->
				<Card>
					<p slot="title" class="selected-title">已选项
						<span v-if="selectModel=='radio'" style="color:red;">[单选模式]</span>
						<span v-else>[多选模式]</span>
					</p>
					...
					<Row :gutter="10" class="select-body" :style="getContentHeight">
						<Col :span="22" v-for="(item,index) in selectItems" :name="item.vcat_id" :key="item.vcat_id">
						<div class="select-box">
							<div class="close" @click="onCloseSelect(index)">
								<Icon type="md-close-circle" class="close-icon"></Icon>
							</div>
							<div class="clamp2">{{item.vcat_name}}</div>
						</div>
						</Col>
				...
				<div class="table-topbar">
					<Form ref="formSearch" :model="formSearch" inline>
						...
						<FormItem>
							<Input v-model="formSearch.searchq" style="width:280px;" placeholder="" clearable search enter-button @on-search="searchPage"
				...
				<Table ref="select-table" row-key="vcat_name" :loading="tableLoading" :height="tableHeight" :columns="columns"
				 :data="data" @on-select="onTableSelect" @on-select-cancel="onTableSelectCancel" @on-select-all="onSelectAll"
	...
	export default {
		...
		methods: {
			...
			initData() {
				...
				// ajax 请求获取初始化数据
				this.$ajax.post((this.$route.fullPath.indexOf('cloud-shop') != -1 ? this.$api.ShopGoodsVCatListForSelect : this.$api
						.goodsVCatListForSelect), {
						isInit: 2,
					})
					.then((response) => {
			...
			initTable(res) {
				...
				this.columns[(this.columns.length - 2)]['render'] = (h, params) => {
					...
					var color = row.status == 1 ? 'success' : 'error';
					var text = row.status == 1 ? '显示' : '隐藏';
					return h('Tag', {
						props: {
							color: color,
							type: 'dot',
