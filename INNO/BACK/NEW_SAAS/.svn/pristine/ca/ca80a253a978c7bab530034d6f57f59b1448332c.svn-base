            ...
            <transition name="fade">
                <div v-show="isShowExtra">
                    <Row>
                        <Col span="6">
                            <FormItem label="查询类型" >
                                <RadioGroup class="flex" v-model="formSearch.styleType" type="button" btn-type="primary" @on-change="search">
                        ...
                        <Col span="6">
                            <FormItem label="商品品牌">
                                <store-select type="radio" :data="brandInfo" @del-tag="delBrand">
																		<div slot="content">
																			<Tag
																			  color="primary"
																			  type="dot"
                        ...
                        <Col span="6">
                            <FormItem label="显示数量">
                                <Select v-model="formSearch.topCount" class="basic-width" style="width:100px;">
     ...
     //    },
        ...
        handleStoreSelected (selected) {
					...
					this.$selectContent({
						mode: 'store',
						type: 'checkbox',
						extraAddtion: { agentId: (agentId[0] || 0), brand_id: ( this.brandId || 0 ) },
						data: this.storeData,
						getList: (data) => {
							this.storeData = data;
				...
				delBrand(data){
					this.brandInfo = [];
					this.formSearch.goodsBrandId = 0;
				},
        handleBrandSelect(){
            this.$selectContent({
							mode: 'goods-brand',
							...
							getList: (data) => {
								this.brandInfo = data;
								this.formSearch.goodsBrandId = data[0].id;
							}
						});
        },
        handleCatData(catId){
            this.formSearch.catId = catId;
        },
        handleStart (date) {
