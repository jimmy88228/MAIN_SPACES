...
<template>
	<div class="admin-message">
		<Card v-show="modalShow">
	        ...
	        <Form ref="formValidate" :model="formItem" :rules="ruleValidate" :label-width="100" class="message-form">
		        ...
		        <FormItem label="发送对象" prop="userType">
					<RadioGroup v-model="formItem.userType" type="button" @on-change="userTypeChange">
				        <Radio label="all">所有管理员</Radio>
				        <Radio label="related">指定品牌管理员</Radio>
				    </RadioGroup>
				    （注意：信息只发送给品牌管理员，如果要发送给C端用户，请用管理员站内信息发送）
			    </FormItem>

			    <transition name="tran-resetform">
			    <FormItem label="选择品牌" prop="userIds" v-show="formItem.showAdminIds">
			    	<Row v-if="formItem.brand.length > 0">
			    		<Col v-for="(brand,index) in formItem.brand" :name="index" :key="index" :span="4">
					    	{{brand.name}} ({{brand.id}})
			    		</Col>
			    	</Row>
		        	<Button @click="onSelectBrand" size="small" type="info">选择品牌...</Button>
			    </FormItem>
			    </transition>
	        ...
	        <!--绑定微信用户的选择器-->
	   		<brandSelect ref="brand-select" @on-ok="onBrandOk"></brandSelect>
	    </Card>
...
export default {
  ...
  methods: {
    ...
    modalOk () {
        ...
        if (valid) {
          ...
          this.formItem.messageContent = this.$refs.ueditor.getUEContent();
                	...
                	util.ajax.post((this.formItem.messageId == 0 ? util.apiUrl.systemMessageAdd : util.apiUrl.systemMessageEdit), {
    ...
    onSelectBrand () {
      ...
      //   }
      // })
        	this.$refs['brand-select'].openModal((this.formItem.user == null ? [] : [this.formItem.user]), 'multi');
    },
    // 选用户的组件的 回调
    onBrandOk (items) {
      if (items.length > 0) {
        ...
        for (var i in items) {
          if (typeof (items[i].id) !== 'undefined') {
            this.formItem.brand.push(items[i]);
