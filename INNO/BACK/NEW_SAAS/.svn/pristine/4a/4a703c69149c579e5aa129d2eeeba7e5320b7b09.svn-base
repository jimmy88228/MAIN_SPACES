<template>
  <div class="label-list">
    <Card>
      <list-head title="营销标签" ref="listHead" @on-labelHandle="labelHandle" @on-groupHandle="groupHandle" :canAction="canCreate"></list-head>
      <div>
...
export default {
  ...
  methods: {
    onLoadData(page, reqData){
        this.showSpan = true;
        return util.ajax.post(util.apiUrl.MarketingLabelList, reqData).then(e =>{
            let res = e.data || {};
            if(res.code) {
        let data = res.data || {};
        let items = data.items || [];
        for(let i = 0; i < items.length; i++){
            let children = items[i].get_usertag_mkt || [];
