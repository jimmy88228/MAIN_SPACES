						...
						<div v-else style="padding:5px 10px 10px 10px;">
							<div slot="content">
								<div v-if="item.goodsListType != '' ">
									...
									<div>
										已选择项：<Tag v-for="(em,eindex) in item.goodsList" :key="eindex" size="medium" closable @on-close="itemClose(index,eindex)">
											{{em.name}}
										</Tag>
									</div>
								</div>
		...
		methods: {
			...
			selectGoodsList(type) {
				switch (type) {
					...
					case 'goodsCat':
						this.$refs['goods-cat-select'].openModal([], 'checkbox');
						break;

					case 'vcat':
						this.$refs['goods-vcat-select'].openModal([], 'checkbox');
						break;

					case 'goodsBrand':
						this.$refs['goods-brand-select'].openModal([], 'checkbox');
						break;
				}
			},
			// 商品品牌选择的，回调
