	...
	export default {
		...
		methods: {
			onLoadData (page, data) {
			  return this.$ajax.post(this.$api.CloudGoodsCommentList, {
					...this.formSearch,
					type: this.typeIndex,
					...data
				})
			...
			changeCommentType(val, row){
				...
				this.changing = true;
				return this.$ajax.post(this.$api.CloudGoodsUpdateCommentType, {
					id: row.id,
					val: val,
					type: this.typeIndex,
				})
			...
			changeCommentShow(val, row){
				...
				if(this.changing || !row.id) return;
				this.changing = true;
				return this.$ajax.post(this.$api.CloudGoodsUpdateCommentShow, {
					id: row.id,
					val: val,
					type: this.typeIndex,
				}).then(response => {
			...
			examineEvent(row){
				...
				this.$Modal.confirm({
					...
					onOk:()=>{
						return this.$ajax.post(this.$api.CloudGoodsExamineComment, {
							id: ids,
							type: this.typeIndex,
						})
						.then(response => {
			...
			removeComment(row){
				...
				this.changing = true;
				return this.$ajax.post(this.$api.CloudGoodsDelComment, {
					id: row.id,
					type: this.typeIndex,
				})
				.then(response => {
