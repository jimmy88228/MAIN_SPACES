		...
		<Row type="flex" style="margin-bottom:10px;">
			<Col style="flex:1 1 0%;">
				<Tooltip content="返回" placement="bottom-start">
					...
					<span style="margin-left:30px">云店微页面关联店铺列表</span>
				</Tooltip>
			</Col>
...
export default {
	...
	methods: {
		...
		init() {
			...
			this.currPageId = this.$route.query.page_id;
			
			this.tableLoading = true;
			this.$ajax.post(this.$api.cloudGoodsPageStoreList, {
				isInit: 1,
				page_id: this.currPageId,
			})
			.then(response => {
		...
		removeItem( index, row ){
			this.$Modal.confirm({
				...
				onOk: () => {
					// ajax提交数据
					this.tableLoading = true;
					this.$ajax.post(this.$api.cloudGoodsPageStoreRemove, {
		...
		onStoreOk( items ){
			...
			// ajax提交数据
			this.tableLoading = true;
			this.$ajax.post(this.$api.cloudGoodsPageStoreAdd, {
				store_ids: storeIds,
				page_id: this.currPageId,
			})
			.then(response => {
		...
		goBack(){
			this.$router.push('/cloud-shop/cloud-goods-page');
		},
	},
	mounted() {
		this.init();
