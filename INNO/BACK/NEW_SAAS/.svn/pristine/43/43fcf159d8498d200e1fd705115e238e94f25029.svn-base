      ...
      @on-ok="confirm">
        <Form ref="formValidate" :model="formItem"  :rules="ruleValidate" :label-width="80">
          <FormItem label="标签名称" prop="label_name">
            <Input v-model="formItem.label_name" placeholder="请输入标签名称"></Input>
          </FormItem>
          <FormItem label="活动排序">
  ...
  methods: {
    ...
    confirm () {
      this.$refs.formValidate.validate((valid) => {
        if (valid) {
          ...
          this.$ajax.post((this.formItem.id === 0 ? this.$api.communitySettingLableAdd : this.$api.communitySettingLableEdit), this.formItem)
		    		.then((response) => {
    ...
    // 初始化表单数据
      ...
      if (this.formItem.id === 0) {
        ...
        this.formItem.label_name = row.label_name;
