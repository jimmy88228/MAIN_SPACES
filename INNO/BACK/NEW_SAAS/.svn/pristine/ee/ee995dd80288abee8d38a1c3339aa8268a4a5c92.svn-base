<template>
    <div class="main-view">
        <div class="label-node" v-for="(item, index) in loopData" :key="index">
            <div class="label-item" 
            :class="['label' + item._level]" 
            :tagType="item.tagType"
            @mouseenter="switchOperate('enter', index)" 
            @mouseleave="switchOperate('out', index)"
            >
                <div class="label-info" @dblclick="checkSaveMsg(item.tagType, item._level, item.chart_setting, item.name)">
                    <template v-if="item.tagType || item.tagType === 0">
                        <img :src="labelList[item.tagType].img" />
                        <p class="label-name">{{ labelList[item.tagType].name }}</p>
                    </template>
                    <template v-else>
                ...
                <div class="info_operate">
                    ...
                    <div class="create_branch flex f-align-center" :class="[item.showCreate]">
                        <div class="branch_type">
                            <a class="y_branch" v-if="checkBranch(item, 'yes')" @click="creatBranch('yes', item._level)">是分支</a>
                            <a class="n_branch" v-if="checkBranch(item, 'no')" @click="creatBranch('no', item._level)">否分支</a>
                        </div>
    ...
    methods: {
        ...
        switchOperate(type, index){
            ...
            if(type == "enter"){
                ...
                this.$emit("labelEnter", { level: editData._level });
            }else {
               editData.showOperate = "";
               editData.showCreate = "";
               this.$emit("labelOut", { level: editData._level });
            }
