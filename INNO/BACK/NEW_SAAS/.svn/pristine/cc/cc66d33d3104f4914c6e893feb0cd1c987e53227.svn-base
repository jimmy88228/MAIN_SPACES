		...
		<Form ref="formValidate" :model="formItem" label-position="top">
			<FormItem label="logo图片( 为了美观，请尽量使用统一大小规格的图片 )">
					<div class="item-list">
						<Row :gutter="16">
							<Col span="6">
								<div class="image-box" :style="'background-image: url('+formItem.logo.img+');'">
									<Icon type="ios-close-circle" class="close" title="移除" @click="removeImg('logo')" />
									<a class="image-box-text" @click="selectSingleImage('logo')">{{formItem.logo.img ? '更换图片':'选择图片'}}</a>
								</div>
							</Col>
							<Col span="18">
								<div class="flex f-align-center" style="margin-top:35px;">
									<div class="f-shrink0" style="margin-right:15px;">图片占比</div> 
									<Slider v-model="formItem.logo.mix" 
									:min="0" :max="100" 
									show-input 
	...
	export default {
		...
		methods: {
			...
			init() {
				...
				if (typeof(this.formItem.logo) == 'undefined') {
					this.$set(this.formItem, 'logo', {img: '', mix: 0});
				}
			},
			// 移除图片
			removeImg(type) {
			...
			selectSingleImage(type) {
				...
				this.$refs['user-images'].showModal({
					name: 'single-image',
					multi: 0,
					selectedImages: this.formItem[type].img,
				});
			},
			// 图片选择组件的回调
			returnImageUrl(obj) {
