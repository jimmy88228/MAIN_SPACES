					...
					<div class="filter_stay">
						<div class="_filter_stay first_filter_stay">
							<div class="item_switch">
									<i-switch size="large" :true-value="trueValue" :false-value="falseValue" v-model="screenData.relation">
							...
							<div class="filter_item first_filter_item" v-for="(lItem, lIndex) in screenData.list" :key="lIndex">
								...
								<div class="basic_filter" v-for="(iItem, iIndex) in lItem" :key="iIndex">
									<div>
										<h4 class="inline_b">{{ iItem.exclusion_condition == 'and' ? '会员基础信息' : '排除条件'}}</h4>
									</div>
									<a href="javascript:;" class="del_filter_point btn_file" @click="removeScreen(lIndex, iIndex)"
									...
									<div class="filter_stay ">
											<div class="_filter_stay">
												<div class="item_switch filter_item_switch">
													<i-switch class="" size="large" :true-value="trueValue" :false-value="falseValue" v-model="iItem.inner_condition">
														<span slot="open">且</span>
														<span slot="close">或</span>
													</i-switch>
												</div>
												<div class="filter_select_screen">
													<!--增加的筛选模块-->
													<div class="select_item" v-for="(item, index) in iItem.screenList" :key="index">
														<Select class="select_screen" transfer :value="item.label" @on-change="(val)=>{changeSelect(val, lIndex, iIndex, index)}" style="width:200px;">
															<Option value="">--请选择--</Option> 
															<Option v-for="(sItem, sIndex) in screenSelect" :key="sIndex" :value="sItem.key">{{ sItem.name }}</Option> 
														</Select>
														<div class="inline_b select_set_list">
															<!--性别-->
															<div class="select_set sex_select_set" v-if="item.label == 'userSex_val'">
															...
															<!--绑定微信-->
															<div class="select_set bind_wx_select_set" v-else-if="item.label == 'userBindWX'">
																<RadioGroup v-model="item.val">
															...
															<!--绑定手机-->
															<div class="select_set phone_select_set" v-else-if="item.label == 'userBindTel_val'">
																<RadioGroup v-model="item.val">
																	<Radio :label="1">已绑定</Radio>
																	<Radio :label="0">未绑定</Radio>
																</RadioGroup>
															...
															<!--注册门店-->
															<div class="select_set store_select_set" v-else-if="item.label == 'store_id'">
																<a href="javascript:;" class="show_datalist" @click="setRegisterStore(lIndex, iIndex, index, item.val)">
																	{{getNumStr(item.label, item.val)}}
																</a>
															</div>
															<!--注册时间-->
															<div class="select_set time_select_set" v-else-if="item.label == 'register_val'">
																<dateSelect class="inline-b space-nowrap" :customDate="item.val" @sT="(date)=>{handleStart(date, lIndex, iIndex, index)}" @eT="(date)=>{handleEnd(date, lIndex, iIndex, index)}" @/>
															</div>
															<!--指定标签-->
															<div class="select_set label_select_set" v-else-if="item.label == 'userTag_id'">
																<a href="javascript:;" class="show_datalist" @click="setTags(lIndex, iIndex, index, item.val)">
																	{{getNumStr(item.label, item.val)}}
																</a>
															</div>
															<!--最近消费时间-->
															<div class="select_set last_pay_select_set" v-else-if="item.label == 'lastPay_val'">
																<dateSelect class="inline-b space-nowrap" :customDate="item.val" @sT="(date)=>{handleStart(date, lIndex, iIndex, index)}" @eT="(date)=>{handleEnd(date, lIndex, iIndex, index)}" />
															</div>
															<!--登录时间-->
															<div class="select_set login_select_set" v-else-if="item.label == 'lastLogin_val'">
																<dateSelect class="inline-b space-nowrap" :customDate="item.val" @sT="(date)=>{handleStart(date, lIndex, iIndex, index)}" @eT="(date)=>{handleEnd(date, lIndex, iIndex, index)}" />
															</div>
															<!--购买指定商品-->
															<div class="select_set goods_select_set" v-else-if="item.label == 'buyGoods_id'">
																<a href="javascript:;" class="show_datalist" @click="setGoods(lIndex, iIndex, index, item.val)">
																	{{getNumStr(item.label, item.val)}}
																</a>
															</div>
															<!--查看优惠券-->
															<div class="select_set coupon_select_set" v-else-if="item.label == 'coupon'">
																<a href="javascript:;" class="show_datalist" @click="setCoupon(lIndex, iIndex, index, item.val)">{{getNumStr(item.label, item.val)}}</a>
															</div>
															<!--领取优惠券-->
															<div class="select_set get_coupon_select_set" v-else-if="item.label == 'couponGet_id'">
																<a href="javascript:;" class="show_datalist" @click="setCoupon(lIndex, iIndex, index, item.val)">{{getNumStr(item.label, item.val)}}</a>
															</div>
															<!--使用优惠券-->
															<div class="select_set check_coupon_select_set" v-else-if="item.label == 'couponUse_id'">
																<a href="javascript:;" class="show_datalist" @click="setCoupon(lIndex, iIndex, index, item.val)">{{getNumStr(item.label, item.val)}}</a>
															</div>
															<div class="select_set rfm_select_set" v-else-if="item.label == 'rfm_id'">
																<a href="javascript:;" class="show_datalist" @click="setRFM(lIndex, iIndex, index, item.val)">
																	{{item.val ? '查看设置' : '点击设置'}}
																</a>
															</div>
															<div class="select_set isSubscribe_select_set" v-else-if="item.label == 'isSubscribe_val'">
																<RadioGroup v-model="item.val">
																	<Radio :label="1">已关注</Radio>
																	<Radio :label="0">未关注</Radio>
																</RadioGroup>
															</div>
															<div class="select_set isApplet_select_set" v-else-if="item.label == 'isApplet_val'">
																<RadioGroup v-model="item.val">
																	<Radio :label="1">小程序会员</Radio>
																	<Radio :label="0">非小程序会员</Radio>
																</RadioGroup>
															</div>
															<div class="select_set profile_select_set" v-else-if="item.label == 'profile_val'">
																<RadioGroup v-model="item.val">
																	<Radio :label="1">已完善</Radio>
																	<Radio :label="0">未完善</Radio>
																</RadioGroup>
															</div>
															<div class="select_set isErpRegister_select_set" v-else-if="item.label == 'isErpRegister_val'">
																<RadioGroup v-model="item.val">
															...
															<!--指定会员分组-->
															<div class="select_set userGroup_select_set" v-else-if="item.label == 'userGroup_id'">
																<a href="javascript:;" class="show_datalist" @click="setGroup(lIndex, iIndex, index, item.val)">{{getNumStr(item.label, item.val)}}</a>
															</div>
															<div class="select_set userRank_select_set" v-else-if="item.label == 'rank_ids'">
																<a href="javascript:;" class="show_datalist" @click="setLevel(lIndex, iIndex, index, item.val)">{{getNumStr(item.label, item.val)}}</a>
															</div>
														</div>
														<a href="javascript:;" class="del_filter_point btn_file" v-if="iItem.screenList.length > 1" @click="removeScreen(lIndex, iIndex, index)">
															<Icon type="md-close-circle" color="#2D8CF0"/>
														</a>
													</div>
													<!--增加的筛选模块-->
											...
											<div class="add_filter_link add_basic_select flex f-align-center" @click="addScreenSelect(lIndex, iIndex)">
												<Icon type="ios-add-circle" color="#2F8CEE" size="20"/>
												<span class="add_filter_txt">添加筛选条件,系统将筛选符合条件的会员</span>
											</div>
										</div>
									</div>
									<div class="add_filter_link add_basic_area flex f-align-center" @click="addScreenArea(lIndex)">
										<Icon type="ios-add-circle" color="#2F8CEE" size="20"/>
						...
						<div class="filter_operate">
							<Button type="primary" class="add_new_filter add_new_basic_filter" @click="addScreenGroup()">新增基础信息条件组</Button>
						</div>
					</div>
				</div>
			</div>
			...
			*/
			...
			saveSetting(){
				...
				if(this.checkSave(screenData)){
					...
					this.$emit("saveMsg", {setting: screenData, level: this.config.level, _params: _params})
				}
			},
				...
				for(let j = 0; j < list.length; j++){
					...
					for(let k = 0; k < jItems.length; k++){
						...
						for(let n = 0; n < this.screenSelect.length; n++){
							...
							if(!json[_key]){
								json[_key] = "";
								let _item = this.installParams({label: _key, val: ""});
								json = {
									...json,
									..._item
								}
